...
#include "cobiwm-monitor-manager-dummy.h"
...
struct _CobiwmMonitorManagerDummy
{
  CobiwmMonitorManager parent_instance;
};

struct _CobiwmMonitorManagerDummyClass
{
  CobiwmMonitorManagerClass parent_class;
};

G_DEFINE_TYPE (CobiwmMonitorManagerDummy, cobiwm_monitor_manager_dummy, COBIWM_TYPE_MONITOR_MANAGER);

static void
cobiwm_monitor_manager_dummy_read_current (CobiwmMonitorManager *manager)
{
   ...
   *
   * COBIWM_DEBUG_NUM_DUMMY_MONITORS
   *
   ...
   *
   * COBIWM_DEBUG_DUMMY_MONITOR_SCALES
   *
   ...
   *
   * COBIWM_DEBUG_NUM_DUMMY_MONITORS=2
   * COBIWM_DEBUG_DUMMY_MONITOR_SCALES=1,2
   */
  num_monitors_str = getenv ("COBIWM_DEBUG_NUM_DUMMY_MONITORS");
  if (num_monitors_str)
    {
      num_monitors = g_ascii_strtoll (num_monitors_str, NULL, 10);
      if (num_monitors <= 0)
        {
          cobiwm_warning ("Invalid number of dummy monitors");
          num_monitors = 1;
        }
    }
  ...
  monitor_scales_str = getenv ("COBIWM_DEBUG_DUMMY_MONITOR_SCALES");
  if (monitor_scales_str)
    {
      ...
      if (g_strv_length (scales_str_list) != num_monitors)
        cobiwm_warning ("Number of specified monitor scales differ from number "
      ...
      for (i = 0; i < num_monitors && scales_str_list[i]; i++)
        {
          ...
          if (scale == 1 || scale == 2)
            ...
            cobiwm_warning ("Invalid dummy monitor scale");
        }
  ...
  manager->modes = g_new0 (CobiwmMonitorMode, 1);
  manager->n_modes = 1;
  ...
  manager->crtcs = g_new0 (CobiwmCRTC, num_monitors);
  manager->n_crtcs = num_monitors;
  manager->outputs = g_new0 (CobiwmOutput, num_monitors);
  ...
  for (i = 0; i < num_monitors; i++)
    {
      ...
      manager->crtcs[i].transform = COBIWM_MONITOR_TRANSFORM_NORMAL;
      ...
      manager->outputs[i].vendor = g_strdup ("CobiwmProducts Inc.");
...
static void
cobiwm_monitor_manager_dummy_class_init (CobiwmMonitorManagerDummyClass *klass)
{
  CobiwmMonitorManagerClass *manager_class = COBIWM_MONITOR_MANAGER_CLASS (klass);

  manager_class->read_current = cobiwm_monitor_manager_dummy_read_current;
  manager_class->apply_configuration = cobiwm_monitor_manager_dummy_apply_config;
}

static void
cobiwm_monitor_manager_dummy_init (CobiwmMonitorManagerDummy *manager)
{
}
