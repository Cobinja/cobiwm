...
session_unpause (void)
{
...

  {
    ...
    CobiwmCursorRenderer *renderer = cobiwm_backend_get_cursor_renderer (backend);
    ...
    cobiwm_cursor_renderer_native_force_update (COBIWM_CURSOR_RENDERER_NATIVE (renderer));
...
static void
sync_active (CobiwmLauncher *self)
{
...
CobiwmLauncher *
cobiwm_launcher_new (GError **error)
{
  CobiwmLauncher *self = NULL;
  ...
  self = g_slice_new0 (CobiwmLauncher);
...
void
cobiwm_launcher_free (CobiwmLauncher *self)
{
  g_object_unref (self->seat_proxy);
  g_object_unref (self->session_proxy);
  g_slice_free (CobiwmLauncher, self);
}

gboolean
cobiwm_launcher_activate_session (CobiwmLauncher  *launcher,
                                GError       **error)
{
...
gboolean
cobiwm_launcher_activate_vt (CobiwmLauncher  *launcher,
                           signed char    vt,
                           GError       **error)
{
