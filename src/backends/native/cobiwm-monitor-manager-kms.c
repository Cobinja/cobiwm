...
#define ALL_TRANSFORMS (COBIWM_MONITOR_TRANSFORM_FLIPPED_270 + 1)
...
struct _CobiwmMonitorManagerKms
{
...
struct _CobiwmMonitorManagerKmsClass
{
  CobiwmMonitorManagerClass parent_class;
};

G_DEFINE_TYPE (CobiwmMonitorManagerKms, cobiwm_monitor_manager_kms, COBIWM_TYPE_MONITOR_MANAGER);

static void
free_resources (CobiwmMonitorManagerKms *manager_kms)
{
  unsigned i;
...
static void
cobiwm_output_destroy_notify (CobiwmOutput *output)
{
  CobiwmOutputKms *output_kms;
  unsigned i;
  ...
  g_slice_free (CobiwmOutputKms, output_kms);
}

static void
cobiwm_monitor_mode_destroy_notify (CobiwmMonitorMode *output)
{
  g_slice_free (drmModeModeInfo, output->driver_private);
}

static void
cobiwm_crtc_destroy_notify (CobiwmCRTC *crtc)
{
...
static void
find_connector_properties (CobiwmMonitorManagerKms *manager_kms,
                           CobiwmOutputKms         *output_kms)
{
  int i;
...
static void
find_crtc_properties (CobiwmMonitorManagerKms *manager_kms,
                      CobiwmCRTC *cobiwm_crtc)
{
  CobiwmCRTCKms *crtc_kms;
  drmModeObjectPropertiesPtr props;
  size_t i;

  crtc_kms = cobiwm_crtc->driver_private;

  props = drmModeObjectGetProperties (manager_kms->fd, cobiwm_crtc->crtc_id, DRM_MODE_OBJECT_CRTC);
  if (!props)
    return;
...
static GBytes *
read_output_edid (CobiwmMonitorManagerKms *manager_kms,
                  CobiwmOutput            *output)
{
  CobiwmOutputKms *output_kms = output->driver_private;
  ...
  if (!edid_blob)
    {
      cobiwm_warning ("Failed to read EDID of output %s: %s\n", output->name, strerror(errno));
      return NULL;
    }
...
static gboolean
output_get_tile_info (CobiwmMonitorManagerKms *manager_kms,
                      CobiwmOutput            *output)
{
  CobiwmOutputKms *output_kms = output->driver_private;
  ...
  if (!tile_blob)
    {
      cobiwm_warning ("Failed to read TILE of output %s: %s\n", output->name, strerror(errno));
      return FALSE;
    }
...
static CobiwmMonitorMode *
find_cobiwm_mode (CobiwmMonitorManager    *manager,
...
static CobiwmOutput *
find_output_by_id (CobiwmOutput *outputs,
                   unsigned    n_outputs,
                   glong       id)
{
  unsigned i;
...
static int
compute_scale (CobiwmOutput *output)
{
  int scale = 1;
...
static int
get_output_scale (CobiwmMonitorManager *manager,
                  CobiwmOutput         *output)
{
  CobiwmMonitorManagerKms *manager_kms = COBIWM_MONITOR_MANAGER_KMS (manager);
...
static int
find_property_index (CobiwmMonitorManager         *manager,
                     drmModeObjectPropertiesPtr  props,
...
static void
parse_transforms (CobiwmMonitorManager *manager,
                  drmModePropertyPtr  prop,
                  CobiwmCRTC           *crtc)
{
  CobiwmCRTCKms *crtc_kms = crtc->driver_private;
  int i;
  ...
  for (i = 0; i < prop->count_enums; i++)
    {
      ...
      if (strcmp (prop->enums[i].name, "rotate-0") == 0)
        cur = COBIWM_MONITOR_TRANSFORM_NORMAL;
      else if (strcmp (prop->enums[i].name, "rotate-90") == 0)
        cur = COBIWM_MONITOR_TRANSFORM_90;
      else if (strcmp (prop->enums[i].name, "rotate-180") == 0)
        cur = COBIWM_MONITOR_TRANSFORM_180;
      else if (strcmp (prop->enums[i].name, "rotate-270") == 0)
        cur = COBIWM_MONITOR_TRANSFORM_270;
...
static gboolean
is_primary_plane (CobiwmMonitorManager         *manager,
                  drmModeObjectPropertiesPtr  props)
{
  drmModePropertyPtr prop;
  int idx;
...
static void
init_crtc_rotations (CobiwmMonitorManager *manager,
                     CobiwmCRTC           *crtc,
                     unsigned int        idx)
{
  CobiwmMonitorManagerKms *manager_kms = COBIWM_MONITOR_MANAGER_KMS (manager);
...
static void
cobiwm_monitor_manager_kms_read_current (CobiwmMonitorManager *manager)
{
  CobiwmMonitorManagerKms *manager_kms = COBIWM_MONITOR_MANAGER_KMS (manager);
  ...
  while (g_hash_table_iter_next (&iter, NULL, (gpointer)&mode))
    {
      CobiwmMonitorMode *cobiwm_mode;

      cobiwm_mode = &manager->modes[i];

      cobiwm_mode->mode_id = i;
      cobiwm_mode->name = g_strndup (mode->name, DRM_DISPLAY_MODE_LEN);
      cobiwm_mode->width = mode->hdisplay;
      cobiwm_mode->height = mode->vdisplay;

      /* Calculate refresh rate in milliHz first for extra precision. */
      cobiwm_mode->refresh_rate = (mode->clock * 1000000LL) / mode->htotal;
      cobiwm_mode->refresh_rate += (mode->vtotal / 2);
      cobiwm_mode->refresh_rate /= mode->vtotal;
      if (mode->flags & DRM_MODE_FLAG_INTERLACE)
	cobiwm_mode->refresh_rate *= 2;
      if (mode->flags & DRM_MODE_FLAG_DBLSCAN)
        cobiwm_mode->refresh_rate /= 2;
      if (mode->vscan > 1)
        cobiwm_mode->refresh_rate /= mode->vscan;
      cobiwm_mode->refresh_rate /= 1000.0;

      cobiwm_mode->driver_private = g_slice_dup (drmModeModeInfo, mode);
      cobiwm_mode->driver_notify = (GDestroyNotify)cobiwm_monitor_mode_destroy_notify;
  ...
  for (i = 0; i < (unsigned)resources->count_crtcs; i++)
    {
      ...
      cobiwm_crtc->crtc_id = crtc->crtc_id;
      ...
      cobiwm_crtc->transform = COBIWM_MONITOR_TRANSFORM_NORMAL;
      /* FIXME: implement! */
      cobiwm_crtc->all_transforms = 1 << COBIWM_MONITOR_TRANSFORM_NORMAL;
      ...
      if (crtc->mode_valid)
        {
          ...
          width = MAX (width, cobiwm_crtc->rect.x + cobiwm_crtc->rect.width);
          height = MAX (height, cobiwm_crtc->rect.y + cobiwm_crtc->rect.height);
        }

      cobiwm_crtc->driver_private = g_new0 (CobiwmCRTCKms, 1);
      cobiwm_crtc->driver_notify = (GDestroyNotify) cobiwm_crtc_destroy_notify;
      find_crtc_properties (manager_kms, cobiwm_crtc);
      init_crtc_rotations (manager, cobiwm_crtc, i);
  ...
  manager->outputs = g_new0 (CobiwmOutput, manager_kms->n_connectors);
  n_actual_outputs = 0;

  for (i = 0; i < manager_kms->n_connectors; i++)
    {
      CobiwmOutput *cobiwm_output, *old_output;
      CobiwmOutputKms *output_kms;
      drmModeConnector *connector;
      GArray *crtcs;
      ...
      if (connector && connector->connection == DRM_MODE_CONNECTED)
	{
          cobiwm_output->driver_private = output_kms = g_slice_new0 (CobiwmOutputKms);
          cobiwm_output->driver_notify = (GDestroyNotify)cobiwm_output_destroy_notify;

	  cobiwm_output->winsys_id = connector->connector_id;
	  cobiwm_output->name = make_output_name (connector);
	  cobiwm_output->width_mm = connector->mmWidth;
	  cobiwm_output->height_mm = connector->mmHeight;

          switch (connector->subpixel)
            {
            case DRM_MODE_SUBPIXEL_NONE:
              cobiwm_output->subpixel_order = COGL_SUBPIXEL_ORDER_NONE;
              break;
            case DRM_MODE_SUBPIXEL_HORIZONTAL_RGB:
              cobiwm_output->subpixel_order = COGL_SUBPIXEL_ORDER_HORIZONTAL_RGB;
              break;
            case DRM_MODE_SUBPIXEL_HORIZONTAL_BGR:
              cobiwm_output->subpixel_order = COGL_SUBPIXEL_ORDER_HORIZONTAL_BGR;
              break;
            case DRM_MODE_SUBPIXEL_VERTICAL_RGB:
              cobiwm_output->subpixel_order = COGL_SUBPIXEL_ORDER_VERTICAL_RGB;
              break;
            case DRM_MODE_SUBPIXEL_VERTICAL_BGR:
              cobiwm_output->subpixel_order = COGL_SUBPIXEL_ORDER_VERTICAL_BGR;
              break;
	  ...
	  cobiwm_output->preferred_mode = NULL;
	  cobiwm_output->n_modes = connector->count_modes;
	  cobiwm_output->modes = g_new0 (CobiwmMonitorMode *, cobiwm_output->n_modes);
	  for (j = 0; j < cobiwm_output->n_modes; j++) {
            cobiwm_output->modes[j] = find_cobiwm_mode (manager, &connector->modes[j]);
            if (connector->modes[j].type & DRM_MODE_TYPE_PREFERRED)
              cobiwm_output->preferred_mode = cobiwm_output->modes[j];
          }

          if (!cobiwm_output->preferred_mode)
	  ...
	  for (j = 0; j < output_kms->n_encoders; j++)
	    {
          ...
          crtcs = g_array_new (FALSE, FALSE, sizeof (CobiwmCRTC*));

          for (j = 0; j < manager->n_crtcs; j++)
            {
              if (crtc_mask & (1 << j))
                {
                  CobiwmCRTC *crtc = &manager->crtcs[j];
                  g_array_append_val (crtcs, crtc);
		}
	    }

	  cobiwm_output->n_possible_crtcs = crtcs->len;
	  cobiwm_output->possible_crtcs = (void*)g_array_free (crtcs, FALSE);
          ...
          old_output = find_output_by_id (old_outputs, n_old_outputs,
                                          cobiwm_output->winsys_id);
          if (old_output)
            {
              cobiwm_output->is_primary = old_output->is_primary;
              cobiwm_output->is_presentation = old_output->is_presentation;
            }
          else
            {
              cobiwm_output->is_primary = FALSE;
              cobiwm_output->is_presentation = FALSE;
            }

          find_connector_properties (manager_kms, output_kms);
          cobiwm_output->suggested_x = output_kms->suggested_x;
          cobiwm_output->suggested_y = output_kms->suggested_y;
          cobiwm_output->hotplug_mode_update = output_kms->hotplug_mode_update;
          
          edid = read_output_edid (manager_kms, cobiwm_output);
          cobiwm_output_parse_edid (cobiwm_output, edid);
          g_bytes_unref (edid);

          /* CobiwmConnectorType matches DRM's connector types */
          cobiwm_output->connector_type = (CobiwmConnectorType) connector->connector_type;

          cobiwm_output->scale = get_output_scale (manager, cobiwm_output);

          output_get_tile_info (manager_kms, cobiwm_output);
          ...
          */
	  cobiwm_output->backlight_min = 0;
          cobiwm_output->backlight_max = 0;
  ...
  manager->outputs = g_renew (CobiwmOutput, manager->outputs, manager->n_outputs);
  ...
  for (i = 0; i < manager->n_outputs; i++)
    {
      ...
      output_kms = cobiwm_output->driver_private;
  ...
  for (i = 0; i < manager->n_outputs; i++)
    {
      ...
      for (j = 0; j < manager->n_outputs; j++)
        {
          CobiwmOutput *cobiwm_clone;
          CobiwmOutputKms *clone_kms;

          cobiwm_clone = &manager->outputs[i];
          clone_kms = cobiwm_clone->driver_private;

          if (cobiwm_clone == cobiwm_output)
            continue;
          ...
          if (clone_kms->encoder_mask == output_kms->enc_clone_mask)
            {
              cobiwm_output->n_possible_clones++;
              cobiwm_output->possible_clones = g_renew (CobiwmOutput *,
                                                      cobiwm_output->possible_clones,
                                                      cobiwm_output->n_possible_clones);
              cobiwm_output->possible_clones[cobiwm_output->n_possible_clones - 1] = cobiwm_clone;
            }
        }
    }
...
static GBytes *
cobiwm_monitor_manager_kms_read_edid (CobiwmMonitorManager *manager,
                                    CobiwmOutput         *output)
{
  CobiwmMonitorManagerKms *manager_kms = COBIWM_MONITOR_MANAGER_KMS (manager);
...
static void
cobiwm_monitor_manager_kms_set_power_save_mode (CobiwmMonitorManager *manager,
  ...
  for (i = 0; i < manager->n_outputs; i++)
    {
      ...
      if (output_kms->dpms_prop_id != 0)
        {
          int ok = drmModeObjectSetProperty (manager_kms->fd, cobiwm_output->winsys_id,
                                             DRM_MODE_OBJECT_CONNECTOR,
                                             output_kms->dpms_prop_id, state);

          if (ok < 0)
            cobiwm_warning ("Failed to set power save mode for output %s: %s\n",
                          cobiwm_output->name, strerror (errno));
        }
    }
...
static void
set_underscan (CobiwmMonitorManagerKms *manager_kms,
               CobiwmOutput *output)
{
  if (!output->crtc)
    return;

  CobiwmCRTC *crtc = output->crtc;
  CobiwmCRTCKms *crtc_kms = crtc->driver_private;
...
static void
cobiwm_monitor_manager_kms_apply_configuration (CobiwmMonitorManager *manager,
                                              ...
                                              unsigned int         n_outputs)
{
      ...
      if (crtc_info->mode == NULL)
      ...
      else
        {
          ...
          for (j = 0; j < n_connectors; j++)
            {
              CobiwmOutput *output = g_ptr_array_index (crtc_info->outputs, j);
  ...
  if (!ok)
    {
      cobiwm_warning ("Applying display configuration failed: %s\n", error->message);
      g_error_free (error);
      return;
    }
...
static void
cobiwm_monitor_manager_kms_get_crtc_gamma (CobiwmMonitorManager  *manager,
                                         CobiwmCRTC            *crtc,
                                         gsize               *size,
                                         unsigned short     **red,
...
static void
cobiwm_monitor_manager_kms_set_crtc_gamma (CobiwmMonitorManager *manager,
                                         CobiwmCRTC           *crtc,
                                         gsize               size,
                                         unsigned short     *red,
                                         unsigned short     *green,
...
on_uevent (GUdevClient *client,
           ...
           gpointer     user_data)
{
  CobiwmMonitorManagerKms *manager_kms = COBIWM_MONITOR_MANAGER_KMS (user_data);
  CobiwmMonitorManager *manager = COBIWM_MONITOR_MANAGER (manager_kms);
...
static void
cobiwm_monitor_manager_kms_init (CobiwmMonitorManagerKms *manager_kms)
{
  ClutterBackend *backend;
  CoglContext *cogl_context;
  CoglDisplay *cogl_display;
  ...
  manager_kms->desktop_settings = g_settings_new ("org.cinnamon.desktop.interface");
}

static void
cobiwm_monitor_manager_kms_dispose (GObject *object)
{
  CobiwmMonitorManagerKms *manager_kms = COBIWM_MONITOR_MANAGER_KMS (object);

  g_clear_object (&manager_kms->udev);
  g_clear_object (&manager_kms->desktop_settings);

  G_OBJECT_CLASS (cobiwm_monitor_manager_kms_parent_class)->dispose (object);
}

static void
cobiwm_monitor_manager_kms_finalize (GObject *object)
{
  CobiwmMonitorManagerKms *manager_kms = COBIWM_MONITOR_MANAGER_KMS (object);

  free_resources (manager_kms);

  G_OBJECT_CLASS (cobiwm_monitor_manager_kms_parent_class)->finalize (object);
}

static void
cobiwm_monitor_manager_kms_class_init (CobiwmMonitorManagerKmsClass *klass)
{
  CobiwmMonitorManagerClass *manager_class = COBIWM_MONITOR_MANAGER_CLASS (klass);
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->dispose = cobiwm_monitor_manager_kms_dispose;
  object_class->finalize = cobiwm_monitor_manager_kms_finalize;

  manager_class->read_current = cobiwm_monitor_manager_kms_read_current;
  manager_class->read_edid = cobiwm_monitor_manager_kms_read_edid;
  manager_class->apply_configuration = cobiwm_monitor_manager_kms_apply_configuration;
  manager_class->set_power_save_mode = cobiwm_monitor_manager_kms_set_power_save_mode;
  manager_class->get_crtc_gamma = cobiwm_monitor_manager_kms_get_crtc_gamma;
  manager_class->set_crtc_gamma = cobiwm_monitor_manager_kms_set_crtc_gamma;
}
