...
struct _CobiwmIdleMonitorXSync
{
...
struct _CobiwmIdleMonitorXSyncClass
{
...
typedef struct {
  ...
  XSyncAlarm xalarm;
} CobiwmIdleMonitorWatchXSync;

G_DEFINE_TYPE (CobiwmIdleMonitorXSync, cobiwm_idle_monitor_xsync, COBIWM_TYPE_IDLE_MONITOR)
...
static XSyncAlarm
_xsync_alarm_set (CobiwmIdleMonitorXSync	*monitor_xsync,
...
static XSyncCounter
find_idletime_counter (CobiwmIdleMonitorXSync *monitor_xsync)
{
  CobiwmIdleMonitor *monitor = COBIWM_IDLE_MONITOR (monitor_xsync);
  int		      i;
  int		      ncounters;
...
static void
init_xsync (CobiwmIdleMonitorXSync *monitor_xsync)
{
...
static void
cobiwm_idle_monitor_xsync_dispose (GObject *object)
{
  CobiwmIdleMonitorXSync *monitor_xsync = COBIWM_IDLE_MONITOR_XSYNC (object);
  ...
  G_OBJECT_CLASS (cobiwm_idle_monitor_xsync_parent_class)->dispose (object);
}

static void
cobiwm_idle_monitor_xsync_constructed (GObject *object)
{
  CobiwmIdleMonitorXSync *monitor_xsync = COBIWM_IDLE_MONITOR_XSYNC (object);
  CobiwmBackendX11 *backend = COBIWM_BACKEND_X11 (cobiwm_get_backend ());

  monitor_xsync->display = cobiwm_backend_x11_get_xdisplay (backend);
  init_xsync (monitor_xsync);

  G_OBJECT_CLASS (cobiwm_idle_monitor_xsync_parent_class)->constructed (object);
}

static gint64
cobiwm_idle_monitor_xsync_get_idletime (CobiwmIdleMonitor *monitor)
{
  CobiwmIdleMonitorXSync *monitor_xsync = COBIWM_IDLE_MONITOR_XSYNC (monitor);
  XSyncValue value;
...
{
  CobiwmIdleMonitorWatch *watch = data;
...
{
  CobiwmIdleMonitorWatchXSync *watch_xsync = data;
  CobiwmIdleMonitorWatch *watch = (CobiwmIdleMonitorWatch *) watch_xsync;
  ...
  g_slice_free (CobiwmIdleMonitorWatchXSync, watch_xsync);
}

static CobiwmIdleMonitorWatch *
cobiwm_idle_monitor_xsync_make_watch (CobiwmIdleMonitor           *monitor,
...
{
  CobiwmIdleMonitorXSync *monitor_xsync = COBIWM_IDLE_MONITOR_XSYNC (monitor);
  CobiwmIdleMonitorWatchXSync *watch_xsync;
  CobiwmIdleMonitorWatch *watch;

  watch_xsync = g_slice_new0 (CobiwmIdleMonitorWatchXSync);
  watch = (CobiwmIdleMonitorWatch *) watch_xsync;
      ...
      if (timeout_msec != 0)
        {
          ...
          if (cobiwm_idle_monitor_get_idletime (monitor) > (gint64)timeout_msec)
            {
              watch->idle_source_id = g_idle_add (fire_watch_idle, watch);
              g_source_set_name_by_id (watch->idle_source_id, "[cobiwm] fire_watch_idle");
            }
        }
      else
        {
...
static void
cobiwm_idle_monitor_xsync_class_init (CobiwmIdleMonitorXSyncClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  CobiwmIdleMonitorClass *idle_monitor_class = COBIWM_IDLE_MONITOR_CLASS (klass);

  object_class->dispose = cobiwm_idle_monitor_xsync_dispose;
  object_class->constructed = cobiwm_idle_monitor_xsync_constructed;

  idle_monitor_class->get_idletime = cobiwm_idle_monitor_xsync_get_idletime;
  idle_monitor_class->make_watch = cobiwm_idle_monitor_xsync_make_watch;
}

static void
cobiwm_idle_monitor_xsync_init (CobiwmIdleMonitorXSync *monitor_xsync)
{
  CobiwmIdleMonitor *monitor = COBIWM_IDLE_MONITOR (monitor_xsync);
...
static void
check_x11_watches (CobiwmIdleMonitor *monitor,
                   XSyncAlarm       alarm)
{
  ...
  for (node = watch_ids; node != NULL; node = node->next)
    {
      ...
      CobiwmIdleMonitorWatchXSync *watch;
...
void
cobiwm_idle_monitor_xsync_handle_xevent (CobiwmIdleMonitor       *monitor,
