...
/**
 * SECTION:cobiwm-background-actor
 * @title: CobiwmBackgroundActor
 ...
 * here is that CobiwmBackground generates a single texture (which
...
#include <errors.h>
#include "cobiwm-background-actor-private.h"
...
typedef struct _CobiwmBackgroundLayer CobiwmBackgroundLayer;
...
struct _CobiwmBackgroundActorPrivate
{
  CobiwmScreen *screen;
  int monitor;
...
G_DEFINE_TYPE_WITH_CODE (CobiwmBackgroundActor, cobiwm_background_actor, CLUTTER_TYPE_ACTOR,
                         G_IMPLEMENT_INTERFACE (COBIWM_TYPE_CULLABLE, cullable_iface_init));

static void
set_clip_region (CobiwmBackgroundActor *self,
                 cairo_region_t      *clip_region)
{
  CobiwmBackgroundActorPrivate *priv = self->priv;
...
static void
cobiwm_background_actor_dispose (GObject *object)
{
  CobiwmBackgroundActor *self = COBIWM_BACKGROUND_ACTOR (object);
  CobiwmBackgroundActorPrivate *priv = self->priv;

  set_clip_region (self, NULL);
  cobiwm_background_actor_set_background (self, NULL);
  if (priv->pipeline)
    {
  ...
  G_OBJECT_CLASS (cobiwm_background_actor_parent_class)->dispose (object);
}

static void
get_preferred_size (CobiwmBackgroundActor *self,
                    gfloat              *width,
                    gfloat              *height)
{
  CobiwmBackgroundActorPrivate *priv = COBIWM_BACKGROUND_ACTOR (self)->priv;
  CobiwmRectangle monitor_geometry;

  cobiwm_screen_get_monitor_geometry (priv->screen, priv->monitor, &monitor_geometry);
...
static void
cobiwm_background_actor_get_preferred_width (ClutterActor *actor,
                                           gfloat        for_height,
                                           gfloat       *min_width_p,
                                           gfloat       *natural_width_p)
{
  ...
  get_preferred_size (COBIWM_BACKGROUND_ACTOR (actor), &width, NULL);
...
static void
cobiwm_background_actor_get_preferred_height (ClutterActor *actor,
                                            gfloat        for_width,
                                            gfloat       *min_height_p,
...

{
  ...
  get_preferred_size (COBIWM_BACKGROUND_ACTOR (actor), NULL, &height);
  ...
  if (*templatep == NULL)
    {
       ...
       */
      *templatep = COGL_PIPELINE (cobiwm_create_texture_pipeline (NULL));
...
static void
setup_pipeline (CobiwmBackgroundActor   *self,
                cairo_rectangle_int_t *actor_pixel_rect)
{
  ...
  if ((priv->changed & CHANGED_BACKGROUND) != 0)
    {
      ...
      CoglTexture *texture = cobiwm_background_get_texture (priv->background,
                                                          priv->monitor,
                                                          &priv->texture_area,
                                                          &wrap_mode);
  ...
  if (!priv->force_bilinear &&
      cobiwm_actor_painting_untransformed (actor_pixel_rect->width, actor_pixel_rect->height, NULL, NULL))
...
static void
set_glsl_parameters (CobiwmBackgroundActor   *self,
                     cairo_rectangle_int_t *actor_pixel_rect)
{
  CobiwmBackgroundActorPrivate *priv = self->priv;
  float scale[2];
...
static gboolean
cobiwm_background_actor_get_paint_volume (ClutterActor       *actor,
...
static void
cobiwm_background_actor_paint (ClutterActor *actor)
{
  CobiwmBackgroundActor *self = COBIWM_BACKGROUND_ACTOR (actor);
  CobiwmBackgroundActorPrivate *priv = self->priv;
...
static void
cobiwm_background_actor_set_property (GObject      *object,
                                    guint         prop_id,
                                    const GValue *value,
                                    GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_BACKGROUND:
      cobiwm_background_actor_set_background (self, g_value_get_object (value));
      break;
    case PROP_VIGNETTE:
      cobiwm_background_actor_set_vignette (self,
...
static void
cobiwm_background_actor_get_property (GObject      *object,
                                    guint         prop_id,
                                    GValue       *value,
                                    GParamSpec   *pspec)
{
  CobiwmBackgroundActorPrivate *priv = COBIWM_BACKGROUND_ACTOR (object)->priv;
...
static void
cobiwm_background_actor_class_init (CobiwmBackgroundActorClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  ClutterActorClass *actor_class = CLUTTER_ACTOR_CLASS (klass);
  GParamSpec *param_spec;

  g_type_class_add_private (klass, sizeof (CobiwmBackgroundActorPrivate));

  object_class->dispose = cobiwm_background_actor_dispose;
  object_class->set_property = cobiwm_background_actor_set_property;
  object_class->get_property = cobiwm_background_actor_get_property;

  actor_class->get_preferred_width = cobiwm_background_actor_get_preferred_width;
  actor_class->get_preferred_height = cobiwm_background_actor_get_preferred_height;
  actor_class->get_paint_volume = cobiwm_background_actor_get_paint_volume;
  actor_class->paint = cobiwm_background_actor_paint;
  ...
  param_spec = g_param_spec_object ("background",
                                    "Background",
                                    "CobiwmBackground object holding background parameters",
...
static void
cobiwm_background_actor_init (CobiwmBackgroundActor *self)
{
  CobiwmBackgroundActorPrivate *priv;

  priv = self->priv = G_TYPE_INSTANCE_GET_PRIVATE (self,
                                                   COBIWM_TYPE_BACKGROUND_ACTOR,
                                                   CobiwmBackgroundActorPrivate);
 ...
 */
ClutterActor *
cobiwm_background_actor_new (CobiwmScreen *screen,
                           int         monitor)
{
  CobiwmBackgroundActor *self;

  self = g_object_new (COBIWM_TYPE_BACKGROUND_ACTOR,
...
static void
cobiwm_background_actor_cull_out (CobiwmCullable   *cullable,
                                cairo_region_t *unobscured_region,
                                cairo_region_t *clip_region)
{
  CobiwmBackgroundActor *self = COBIWM_BACKGROUND_ACTOR (cullable);
  set_clip_region (self, clip_region);
}

static void
cobiwm_background_actor_reset_culling (CobiwmCullable *cullable)
{
  CobiwmBackgroundActor *self = COBIWM_BACKGROUND_ACTOR (cullable);
  set_clip_region (self, NULL);
}
...
{
  iface->cull_out = cobiwm_background_actor_cull_out;
  iface->reset_culling = cobiwm_background_actor_reset_culling;
}

/**
 * cobiwm_background_actor_get_clip_region:
 * @self: a #CobiwmBackgroundActor
 *
 * Return value (transfer none): a #cairo_region_t that represents the part of
 * the background not obscured by other #CobiwmBackgroundActor or
 * #CobiwmWindowActor objects.
 */
cairo_region_t *
cobiwm_background_actor_get_clip_region (CobiwmBackgroundActor *self)
{
  CobiwmBackgroundActorPrivate *priv = self->priv;
...
static void
invalidate_pipeline (CobiwmBackgroundActor *self,
                     ChangedFlags         changed)
{
  CobiwmBackgroundActorPrivate *priv = self->priv;
...
static void
on_background_changed (CobiwmBackground      *background,
                       CobiwmBackgroundActor *self)
{
...
void
cobiwm_background_actor_set_background (CobiwmBackgroundActor *self,
                                      CobiwmBackground      *background)
{
  CobiwmBackgroundActorPrivate *priv;

  g_return_if_fail (COBIWM_IS_BACKGROUND_ACTOR (self));
  g_return_if_fail (background == NULL || COBIWM_IS_BACKGROUND (background));
...
void
cobiwm_background_actor_set_vignette (CobiwmBackgroundActor *self,
                                    gboolean             enabled,
                                    double               brightness,
                                    double               sharpness)
{
