...
G_DEFINE_INTERFACE (CobiwmCullable, cobiwm_cullable, CLUTTER_TYPE_ACTOR);

/**
 * SECTION:cobiwm-cullable
 * @title: CobiwmCullable
 ...
 *
 * #CobiwmCullable is our solution. The basic technique applied here is to
...
/**
 * cobiwm_cullable_cull_out_children:
 * @cullable: The #CobiwmCullable
 ...
 *
 * See #CobiwmCullable and cobiwm_cullable_cull_out() for more details.
 */
void
cobiwm_cullable_cull_out_children (CobiwmCullable   *cullable,
  ...
  while (clutter_actor_iter_prev (&iter, &child))
    {
      ...
      if (needs_culling && !cobiwm_actor_is_untransformed (child, NULL, NULL))
        needs_culling = FALSE;
      ...
      if (needs_culling)
        {
          ...
          cobiwm_cullable_cull_out (COBIWM_CULLABLE (child), unobscured_region, clip_region);
        ...
        {
          cobiwm_cullable_cull_out (COBIWM_CULLABLE (child), NULL, NULL);
        }
    }
}

/**
 * cobiwm_cullable_reset_culling_children:
 * @cullable: The #CobiwmCullable
 *
 * This is a helper method for actors that want to recurse over their
 * child actors, and cull them out.
 *
 * See #CobiwmCullable and cobiwm_cullable_reset_culling() for more details.
 */
void
cobiwm_cullable_reset_culling_children (CobiwmCullable *cullable)
{
  ClutterActor *actor = CLUTTER_ACTOR (cullable);
  ClutterActor *child;
  ClutterActorIter iter;
  ...
  while (clutter_actor_iter_next (&iter, &child))
    {
      ...
      cobiwm_cullable_reset_culling (COBIWM_CULLABLE (child));
    }
}

static void
cobiwm_cullable_default_init (CobiwmCullableInterface *iface)
{
}

/**
 * cobiwm_cullable_cull_out:
 * @cullable: The #CobiwmCullable
 * @unobscured_region: The unobscured region, in @cullable's space.
 * @clip_region: The clip region, in @cullable's space.
 *
 * When #CobiwmWindowGroup is painted, we walk over its direct cullable
 ...
 *
 * Actors that have children can also use the cobiwm_cullable_cull_out_children()
 * helper method to do a simple cull across all their children.
 */
void
cobiwm_cullable_cull_out (CobiwmCullable   *cullable,
                        cairo_region_t *unobscured_region,
                        cairo_region_t *clip_region)
{
  COBIWM_CULLABLE_GET_IFACE (cullable)->cull_out (cullable, unobscured_region, clip_region);
}

/**
 * cobiwm_cullable_reset_culling:
 * @cullable: The #CobiwmCullable
 *
 ...
 */
void
cobiwm_cullable_reset_culling (CobiwmCullable *cullable)
{
  COBIWM_CULLABLE_GET_IFACE (cullable)->reset_culling (cullable);
}
