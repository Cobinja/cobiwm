...
/**
 * SECTION:cobiwm-dnd-actor
 * @title: CobiwmDnDActor
...
typedef struct _CobiwmDnDActorPrivate CobiwmDnDActorPrivate;

struct _CobiwmDnDActorPrivate
{
...
G_DEFINE_TYPE_WITH_PRIVATE (CobiwmDnDActor, cobiwm_dnd_actor, COBIWM_TYPE_FEEDBACK_ACTOR)

static void
cobiwm_dnd_actor_set_property (GObject      *object,
                             guint         prop_id,
                             const GValue *value,
                             GParamSpec   *pspec)
{
  CobiwmDnDActor *self = COBIWM_DND_ACTOR (object);
  CobiwmDnDActorPrivate *priv = cobiwm_dnd_actor_get_instance_private (self);
...
static void
cobiwm_dnd_actor_get_property (GObject      *object,
                             guint         prop_id,
                             GValue       *value,
                             GParamSpec   *pspec)
{
...
static void
cobiwm_dnd_actor_class_init (CobiwmDnDActorClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);
  GParamSpec *pspec;

  object_class->set_property = cobiwm_dnd_actor_set_property;
  object_class->get_property = cobiwm_dnd_actor_get_property;
...
static void
cobiwm_dnd_actor_init (CobiwmDnDActor *self)
{
}

/**
 * cobiwm_dnd_actor_new:
 *
 ...
 */
ClutterActor *
cobiwm_dnd_actor_new (ClutterActor *drag_origin,
                    int           drag_start_x,
                    int           drag_start_y)
{
  CobiwmDnDActor *self;

  self = g_object_new (COBIWM_TYPE_DND_ACTOR,
...
void
cobiwm_dnd_actor_drag_finish (CobiwmDnDActor *self,
                            gboolean      success)
{
  CobiwmDnDActorPrivate *priv;
  ClutterActor *actor;

  g_return_if_fail (COBIWM_IS_DND_ACTOR (self));

  actor = CLUTTER_ACTOR (self);
  priv = cobiwm_dnd_actor_get_instance_private (self);
      ...
      if (CLUTTER_ACTOR_IS_VISIBLE (priv->drag_origin))
        {
          ...
          clutter_actor_get_transformed_position (priv->drag_origin,
                                                  &dest.x, &dest.y);
          cobiwm_feedback_actor_get_anchor (COBIWM_FEEDBACK_ACTOR (self),
                                          &anchor_x, &anchor_y);
