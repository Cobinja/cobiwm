...
/**
 * SECTION:cobiwm-shadow-factory
 * @title: CobiwmShadowFactory
...
typedef struct _CobiwmShadowCacheKey  CobiwmShadowCacheKey;
typedef struct _CobiwmShadowClassInfo CobiwmShadowClassInfo;

struct _CobiwmShadowCacheKey
{
...
struct _CobiwmShadow
{
  int ref_count;

  CobiwmShadowFactory *factory;
  CobiwmShadowCacheKey key;
  CoglTexture *texture;
...
struct _CobiwmShadowClassInfo
{
  const char *name; /* const so we can reuse for static definitions */
  CobiwmShadowParams focused;
  CobiwmShadowParams unfocused;
};

struct _CobiwmShadowFactory
{
  GObject parent_instance;

  /* CobiwmShadowCacheKey => CobiwmShadow; the shadows are not referenced
   * by the factory, they are simply removed from the table when freed */
  GHashTable *shadows;

  /* class name => CobiwmShadowClassInfo */
  GHashTable *shadow_classes;
};

struct _CobiwmShadowFactoryClass
{
...
CobiwmShadowClassInfo default_shadow_classes[] = {
...
G_DEFINE_TYPE (CobiwmShadowFactory, cobiwm_shadow_factory, G_TYPE_OBJECT);

static guint
cobiwm_shadow_cache_key_hash (gconstpointer val)
{
  const CobiwmShadowCacheKey *key = val;

  return 59 * key->radius + 67 * key->top_fade + 73 * cobiwm_window_shape_hash (key->shape);
}

static gboolean
cobiwm_shadow_cache_key_equal (gconstpointer a,
                             gconstpointer b)
{
  const CobiwmShadowCacheKey *key_a = a;
  const CobiwmShadowCacheKey *key_b = b;

  return (key_a->radius == key_b->radius && key_a->top_fade == key_b->top_fade &&
          cobiwm_window_shape_equal (key_a->shape, key_b->shape));
}

CobiwmShadow *
cobiwm_shadow_ref (CobiwmShadow *shadow)
{
  shadow->ref_count++;
...
void
cobiwm_shadow_unref (CobiwmShadow *shadow)
{
  shadow->ref_count--;
  if (shadow->ref_count == 0)
    {
      ...
      cobiwm_window_shape_unref (shadow->key.shape);
      cogl_object_unref (shadow->texture);
      cogl_object_unref (shadow->pipeline);

      g_slice_free (CobiwmShadow, shadow);
    }
}
 ...
 * size of the region. (Since a #CobiwmShadow can be shared between
 * different sizes with the same extracted #CobiwmWindowShape the
 * size needs to be passed in here.)
 */
void
cobiwm_shadow_paint (CobiwmShadow     *shadow,
                   int             window_x,
                   int             window_y,
                   int             window_width,
                   int             window_height,
...
/**
 * cobiwm_shadow_get_bounds:
 * @shadow: a #CobiwmShadow
 * @window_x: x position of the region to paint a shadow for
 * @window_y: y position of the region to paint a shadow for
 * @window_width: actual width of the region to paint a shadow for
 * @window_height: actual height of the region to paint a shadow for
 ...
 */
void
cobiwm_shadow_get_bounds  (CobiwmShadow            *shadow,
                         int                    window_x,
                         int                    window_y,
                         int                    window_width,
                         int                    window_height,
...
static void
cobiwm_shadow_class_info_free (CobiwmShadowClassInfo *class_info)
{
  g_free ((char *)class_info->name);
  g_slice_free (CobiwmShadowClassInfo, class_info);
}

static void
cobiwm_shadow_factory_init (CobiwmShadowFactory *factory)
{
  guint i;

  factory->shadows = g_hash_table_new (cobiwm_shadow_cache_key_hash,
                                       cobiwm_shadow_cache_key_equal);

  factory->shadow_classes = g_hash_table_new_full (g_str_hash,
                                                   g_str_equal,
                                                   NULL,
                                                   (GDestroyNotify)cobiwm_shadow_class_info_free);

  for (i = 0; i < G_N_ELEMENTS (default_shadow_classes); i++)
    {
      CobiwmShadowClassInfo *class_info = g_slice_new (CobiwmShadowClassInfo);
...
static void
cobiwm_shadow_factory_finalize (GObject *object)
{
  CobiwmShadowFactory *factory = COBIWM_SHADOW_FACTORY (object);
  GHashTableIter iter;
  gpointer key, value;
  ...
  G_OBJECT_CLASS (cobiwm_shadow_factory_parent_class)->finalize (object);
}

static void
cobiwm_shadow_factory_class_init (CobiwmShadowFactoryClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->finalize = cobiwm_shadow_factory_finalize;
...
CobiwmShadowFactory *
cobiwm_shadow_factory_new (void)
{
  return g_object_new (COBIWM_TYPE_SHADOW_FACTORY, NULL);
}

/**
 * cobiwm_shadow_factory_get_default:
 *
 * Return value: (transfer none): the global singleton shadow factory
 */
CobiwmShadowFactory *
cobiwm_shadow_factory_get_default (void)
{
  static CobiwmShadowFactory *factory;

  if (factory == NULL)
    factory = cobiwm_shadow_factory_new ();
...
static void
make_shadow (CobiwmShadow     *shadow,
             cairo_region_t *region)
{
   ...
   */
  row_convolve_region = cobiwm_make_border_region (region, spread, spread, FALSE);
  column_convolve_region = cobiwm_make_border_region (region, 0, spread, TRUE);
  ...
  if (error)
    {
      cobiwm_warning ("Failed to allocate shadow texture: %s\n", error->message);
  ...
  shadow->pipeline = cobiwm_create_texture_pipeline (shadow->texture);
}

static CobiwmShadowParams *
get_shadow_params (CobiwmShadowFactory *factory,
                   const char        *class_name,
                   gboolean           focused,
                   gboolean           create)
{
  CobiwmShadowClassInfo *class_info = g_hash_table_lookup (factory->shadow_classes,
                                                         class_name);
  if (class_info == NULL)
    {
      if (create)
        {
          class_info = g_slice_new0 (CobiwmShadowClassInfo);
...
/**
 * cobiwm_shadow_factory_get_shadow:
 * @factory: a #CobiwmShadowFactory
 ...
 *
 * Return value: (transfer full): a newly referenced #CobiwmShadow; unref with
 *  cobiwm_shadow_unref()
 */
CobiwmShadow *
cobiwm_shadow_factory_get_shadow (CobiwmShadowFactory *factory,
                                CobiwmWindowShape   *shape,
                                int                width,
                                int                height,
                                const char        *class_name,
                                gboolean           focused)
{
  ...
  g_return_val_if_fail (COBIWM_IS_SHADOW_FACTORY (factory), NULL);
  ...
  cobiwm_window_shape_get_borders (shape,
                                 &shape_border_top,
                                 &shape_border_right,
                                 &shape_border_bottom,
                                 &shape_border_left);
  ...
  if (cacheable)
    {
      ...
      if (shadow)
        return cobiwm_shadow_ref (shadow);
    }

  shadow = g_slice_new0 (CobiwmShadow);

  shadow->ref_count = 1;
  shadow->factory = factory;
  shadow->key.shape = cobiwm_window_shape_ref (shape);
  ...
  region = cobiwm_window_shape_to_region (shape, center_width, center_height);
...
/**
 * cobiwm_shadow_factory_set_params:
 * @factory: a #CobiwmShadowFactory
 ...
 */
void
cobiwm_shadow_factory_set_params (CobiwmShadowFactory *factory,
                                const char        *class_name,
                                gboolean           focused,
                                CobiwmShadowParams  *params)
{
  CobiwmShadowParams *stored_params;

  g_return_if_fail (COBIWM_IS_SHADOW_FACTORY (factory));
...
/**
 * cobiwm_shadow_factory_get_params:
 * @factory: a #CobiwmShadowFactory
 ...
 */
void
cobiwm_shadow_factory_get_params (CobiwmShadowFactory *factory,
                                const char        *class_name,
                                gboolean           focused,
                                CobiwmShadowParams  *params)
{
...
G_DEFINE_BOXED_TYPE (CobiwmShadow, cobiwm_shadow,
                     cobiwm_shadow_ref, cobiwm_shadow_unref)
