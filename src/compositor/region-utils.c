 ...
 * move all the rectangles after that.) To avoid this behavior, CobiwmRegionBuilder
...
void
cobiwm_region_builder_init (CobiwmRegionBuilder *builder)
{
  int i;
  for (i = 0; i < COBIWM_REGION_BUILDER_MAX_LEVELS; i++)
    builder->levels[i] = NULL;
  builder->n_levels = 1;
}

void
cobiwm_region_builder_add_rectangle (CobiwmRegionBuilder *builder,
                                   int                x,
                                   int                y,
                                   int                width,
                                   int                height)
      ...
      for (i = 1; i < builder->n_levels + 1; i++)
        {
          if (builder->levels[i] == NULL)
            {
              if (i < COBIWM_REGION_BUILDER_MAX_LEVELS)
                {
...
cairo_region_t *
cobiwm_region_builder_finish (CobiwmRegionBuilder *builder)
{
...
void
cobiwm_region_iterator_init (CobiwmRegionIterator *iter,
...
gboolean
cobiwm_region_iterator_at_end (CobiwmRegionIterator *iter)
{
  return iter->i >= iter->n_rectangles;
}

void
cobiwm_region_iterator_next (CobiwmRegionIterator *iter)
{
  iter->i++;
...
cairo_region_t *
cobiwm_region_scale (cairo_region_t *region, int scale)
{
  int n_rects, i;
...
static void
add_expanded_rect (CobiwmRegionBuilder  *builder,
                   int                 x,
                   int                 y,
                   int                 width,
                   int                 height,
...
{
  if (flip)
    cobiwm_region_builder_add_rectangle (builder,
 ...
 */
cairo_region_t *
cobiwm_make_border_region (cairo_region_t *region,
                         int             x_amount,
                         int             y_amount,
                         gboolean        flip)
{
