 ...
 *
 * Sometimes, X programs "ring the bell", whatever that means. Cobiwm lets
 * the user configure the bell to be audible or visible (aka visual), and
 * if it's visual it can be configured to be frame-flash or fullscreen-flash.
 * We never get told about audible bells; X handles them just fine by itself.
 *
 * Visual bells come in at cobiwm_bell_notify(), which checks we are actually
 ...
 */
static void
bell_flash_fullscreen (CobiwmDisplay *display)
{
  cobiwm_compositor_flash_screen (display->compositor, display->screen);
}
 ...
 * really be called cobiwm_bell_unflash_window_frame().
 */
static gboolean
bell_unflash_frame (gpointer data)
{
  CobiwmFrame *frame = (CobiwmFrame *) data;
  frame->is_flashing = 0;
  cobiwm_frame_queue_draw (frame);
  return FALSE;
}
 ...
 */
static void
bell_flash_window_frame (CobiwmWindow *window)
{
  guint id;
   ...
   */
  id = g_timeout_add_full (COBIWM_PRIORITY_REDRAW, 100,
        bell_unflash_frame, window->frame, NULL);
  g_source_set_name_by_id (id, "[cobiwm] bell_unflash_frame");
}

static void
bell_flash_window (CobiwmWindow *window)
{
  cobiwm_compositor_flash_window (window->display->compositor, window);
}
 ...
 */
static void
bell_flash_frame (CobiwmDisplay *display,
                  CobiwmWindow  *window)
{
 ...
 */
static void
bell_visual_notify (CobiwmDisplay *display,
                    CobiwmWindow  *window)
{
  switch (cobiwm_prefs_get_visual_bell_type ())
    {
...
static gboolean
bell_audible_notify (CobiwmDisplay *display,
                     CobiwmWindow  *window)
{
#ifdef HAVE_LIBCANBERRA
  ca_proplist *p;
...
gboolean
cobiwm_bell_notify (CobiwmDisplay *display,
                  CobiwmWindow  *window)
{
  /* flash something */
  if (cobiwm_prefs_get_visual_bell ())
    bell_visual_notify (display, window);

  if (cobiwm_prefs_bell_is_audible ())
...
void
cobiwm_bell_set_audible (CobiwmDisplay *display, gboolean audible)
{
#ifdef HAVE_LIBCANBERRA
...
gboolean
cobiwm_bell_init (CobiwmDisplay *display)
{
...
void
cobiwm_bell_shutdown (CobiwmDisplay *display)
{
...
/**
 * cobiwm_bell_notify_frame_destroy:
 ...
 */
void
cobiwm_bell_notify_frame_destroy (CobiwmFrame *frame)
{
