...
/**
 * SECTION:boxes
 * @Title: CobiwmRectangle
 * @Short_Description: Simple box operations
 */

/*
 * Copyright (C) 2005, 2006 Elijah Newren
 * [cobiwm_rectangle_intersect() is copyright the GTK+ Team according to Havoc,
 * see gdkrectangle.c.  As far as Havoc knows, he probably wrote
 * cobiwm_rectangle_equal(), and I'm guessing it's (C) Red Hat.  So...]
...
CobiwmRectangle *
cobiwm_rectangle_copy (const CobiwmRectangle *rect)
{
  return g_memdup (rect, sizeof (CobiwmRectangle));
}

void
cobiwm_rectangle_free (CobiwmRectangle *rect)
{
  g_free (rect);
}

GType
cobiwm_rectangle_get_type (void)
{
  static GType type_id = 0;

  if (!type_id)
    type_id = g_boxed_type_register_static (g_intern_static_string ("CobiwmRectangle"),
					    (GBoxedCopyFunc) cobiwm_rectangle_copy,
					    (GBoxedFreeFunc) cobiwm_rectangle_free);
  return type_id;
}

char*
cobiwm_rectangle_to_string (const CobiwmRectangle *rect,
                          char                *output)
{
...
char*
cobiwm_rectangle_region_to_string (GList      *region,
...
char*
cobiwm_rectangle_edge_to_string (const CobiwmEdge *edge,
                               char           *output)
{
  /* 25 chars: 2 commas, space, plus, trailing \0 + 5 for each digit.
   * Should be more than enough space.  Note that of this space, the
...
char*
cobiwm_rectangle_edge_list_to_string (GList      *edge_list,
                                    const char *separator_string,
                                    char       *output)
{
  ...
  while (tmp)
    {
      CobiwmEdge      *edge = tmp->data;
      CobiwmRectangle *rect = &edge->rect;
...
CobiwmRectangle
cobiwm_rect (int x, int y, int width, int height)
{
...
int
cobiwm_rectangle_area (const CobiwmRectangle *rect)
{
...
/**
 * cobiwm_rectangle_intersect:
 * @src1: a #CobiwmRectangle
 * @src2: another #CobiwmRectangle
 * @dest: (out caller-allocates): an empty #CobiwmRectangle, to be filled
 ...
 */
gboolean
cobiwm_rectangle_intersect (const CobiwmRectangle *src1,
...
gboolean
cobiwm_rectangle_equal (const CobiwmRectangle *src1,
                      const CobiwmRectangle *src2)
{
...
/**
 * cobiwm_rectangle_union:
 * @rect1: a #CobiwmRectangle
 * @rect2: another #CobiwmRectangle
 * @dest: (out caller-allocates): an empty #CobiwmRectangle, to be filled
 *   with the coordinates of the bounding box.
 */
void
cobiwm_rectangle_union (const CobiwmRectangle *rect1,
...
gboolean
cobiwm_rectangle_overlap (const CobiwmRectangle *rect1,
                        const CobiwmRectangle *rect2)
{
...
gboolean
cobiwm_rectangle_vert_overlap (const CobiwmRectangle *rect1,
                             const CobiwmRectangle *rect2)
{
...
gboolean
cobiwm_rectangle_horiz_overlap (const CobiwmRectangle *rect1,
                              const CobiwmRectangle *rect2)
{
...
gboolean
cobiwm_rectangle_could_fit_rect (const CobiwmRectangle *outer_rect,
...
gboolean
cobiwm_rectangle_contains_rect  (const CobiwmRectangle *outer_rect,
                               const CobiwmRectangle *inner_rect)
{
  return
...
void
cobiwm_rectangle_resize_with_gravity (const CobiwmRectangle *old_rect,
...
merge_spanning_rects_in_region (GList *region)
{
  ...
  if (region == NULL)
    {
      cobiwm_warning ("Region to merge was empty!  Either you have a some "
...
{
  const CobiwmRectangle *a_rect = (gconstpointer) a;
  const CobiwmRectangle *b_rect = (gconstpointer) b;

  int a_area = cobiwm_rectangle_area (a_rect);
  int b_area = cobiwm_rectangle_area (b_rect);
...
static gboolean
check_strut_align (CobiwmStrut *strut, const CobiwmRectangle *rect)
{
 ...
 * @all_struts: (element-type Cobiwm.Rectangle): List of struts
 *
 ...
 * rectangles.
 *
 * Returns: (transfer full) (element-type Cobiwm.Rectangle): Minimal spanning set
 */
GList*
cobiwm_rectangle_get_minimal_spanning_set_for_region (
  ...
  const GSList  *all_struts)
{
   ...
   *     /cobiwmcity-devel-list/2005-November/msg00028.html.  (Sorry for the
   *     URL splitting.)
   */
  ...
  temp_rect = g_new (CobiwmRectangle, 1);
  *temp_rect = *basic_rect;
  ...
  for (strut_iter = all_struts; strut_iter; strut_iter = strut_iter->next)
    {
      GList *rect_iter;
      CobiwmStrut *strut = (CobiwmStrut*)strut_iter->data;
      CobiwmRectangle *strut_rect = &strut->rect;
      ...
      while (rect_iter)
        {
          CobiwmRectangle *rect = (CobiwmRectangle*) rect_iter->data;

          if (!cobiwm_rectangle_overlap (strut_rect, rect) ||
...
/**
 * cobiwm_rectangle_expand_region: (skip)
 *
 */
GList*
cobiwm_rectangle_expand_region (GList     *region,
                              const int  left_expand,
                              const int  right_expand,
                              const int  top_expand,
                              const int  bottom_expand)
{
  return cobiwm_rectangle_expand_region_conditionally (region,
                                                     left_expand,
                                                     right_expand,
                                                     top_expand,
                                                     bottom_expand,
...
/**
 * cobiwm_rectangle_expand_region_conditionally: (skip)
 *
 */
GList*
cobiwm_rectangle_expand_region_conditionally (GList     *region,
                                            const int  left_expand,
                                            const int  right_expand,
                                            const int  top_expand,
                                            const int  bottom_expand,
                                            ...
                                            const int  min_y)
{
  ...
  while (tmp_list)
    {
      CobiwmRectangle *rect = (CobiwmRectangle*) tmp_list->data;
      if (rect->width >= min_x)
        {
          rect->x      -= left_expand;
...
void
cobiwm_rectangle_expand_to_avoiding_struts (CobiwmRectangle       *rect,
                                          ...
                                          const GSList        *all_struts)
{
   ...
   */
  g_assert ((direction == COBIWM_DIRECTION_HORIZONTAL) ^
            (direction == COBIWM_DIRECTION_VERTICAL  ));

  if (direction == COBIWM_DIRECTION_HORIZONTAL)
    {
      rect->x      = expand_to->x;
  ...
  for (strut_iter = all_struts; strut_iter; strut_iter = strut_iter->next)
    {
      ...
      if (!cobiwm_rectangle_overlap (&strut->rect, rect))
        continue;

      if (direction == COBIWM_DIRECTION_HORIZONTAL)
        {
          if (strut->side == COBIWM_SIDE_LEFT)
            {
              int offset = BOX_RIGHT(strut->rect) - BOX_LEFT(*rect);
              rect->x     += offset;
              rect->width -= offset;
            }
          else if (strut->side == COBIWM_SIDE_RIGHT)
            {
      ...
      else /* direction == COBIWM_DIRECTION_VERTICAL */
        {
          if (strut->side == COBIWM_SIDE_TOP)
            {
              int offset = BOX_BOTTOM(strut->rect) - BOX_TOP(*rect);
              rect->y      += offset;
              rect->height -= offset;
            }
          else if (strut->side == COBIWM_SIDE_BOTTOM)
            {
...
} /* end cobiwm_rectangle_expand_to_avoiding_struts */

void
cobiwm_rectangle_free_list_and_elements (GList *filled_list)
{
...
gboolean
cobiwm_rectangle_could_fit_in_region (const GList         *spanning_rects,
                                    const CobiwmRectangle *rect)
{
  ...
  while (!could_fit && temp != NULL)
    {
      could_fit = could_fit || cobiwm_rectangle_could_fit_rect (temp->data, rect);
      temp = temp->next;
    }
...
gboolean
cobiwm_rectangle_contained_in_region (const GList         *spanning_rects,
                                    const CobiwmRectangle *rect)
{
  const GList *temp;
  gboolean     contained;
  ...
  while (!contained && temp != NULL)
    {
      contained = contained || cobiwm_rectangle_contains_rect (temp->data, rect);
      temp = temp->next;
    }
...
gboolean
cobiwm_rectangle_overlaps_with_region (const GList         *spanning_rects,
                                     const CobiwmRectangle *rect)
{
  const GList *temp;
  gboolean     overlaps;
...
void
cobiwm_rectangle_clamp_to_fit_into_region (const GList         *spanning_rects,
                                         FixedDirections      fixed_directions,
                                         CobiwmRectangle       *rect,
                                         const CobiwmRectangle *min_size)
{
  const GList *temp;
  const CobiwmRectangle *best_rect = NULL;
  int                  best_overlap = 0;
  ...
  for (temp = spanning_rects; temp; temp = temp->next)
    {
      CobiwmRectangle *compare_rect = temp->data;
  ...
  if (best_rect == NULL)
    {
      cobiwm_warning ("No rect whose size to clamp to found!\n");
...
void
cobiwm_rectangle_clip_to_region (const GList         *spanning_rects,
                               FixedDirections      fixed_directions,
                               CobiwmRectangle       *rect)
{
  const GList *temp;
  ...
  for (temp = spanning_rects; temp; temp = temp->next)
    {
      ...
      /* Determine maximal overlap amount */
      cobiwm_rectangle_intersect (rect, compare_rect, &overlap);
      maximal_overlap_amount_for_compare = cobiwm_rectangle_area (&overlap);
  ...
  if (best_rect == NULL)
    cobiwm_warning ("No rect to clip to found!\n");
  else
    {
...
void
cobiwm_rectangle_shove_into_region (const GList         *spanning_rects,
                                  FixedDirections      fixed_directions,
                                  CobiwmRectangle       *rect)
{
  const GList *temp;
  ...
  if (best_rect == NULL)
    cobiwm_warning ("No rect to shove into found!\n");
  else
    {
      /* Extra precaution with checking fixed direction shouldn't be needed
       * due to logic above, but it shouldn't hurt either.
...
void
cobiwm_rectangle_find_linepoint_closest_to_point (double x1,
                                                double y1,
                                                double x2,
                                                double y2,
                                                double px,
...
gboolean
cobiwm_rectangle_edge_aligns (const CobiwmRectangle *rect, const CobiwmEdge *edge)
{
                        ...
                        CobiwmRectangle *overlap)
{
  ...
  CobiwmRectangle *rect = rect_in_list->data;
  GList *ret = NULL;

  if (BOX_LEFT (*rect) < BOX_LEFT (*overlap))
    {
      temp = g_new (CobiwmRectangle, 1);
      *temp = *rect;
                                        ...
                                        const CobiwmRectangle *region)
{
  ...
  while (old_struts)
    {
      CobiwmRectangle *cur = &((CobiwmStrut*)old_struts->data)->rect;
      CobiwmRectangle *copy = g_new (CobiwmRectangle, 1);
      *copy = *cur;
      if (cobiwm_rectangle_intersect (copy, region, copy))
  ...
  while (tmp)
    {
      ...
      CobiwmRectangle *cur = tmp->data;

      compare = tmp->next;
      while (compare)
        {
          CobiwmRectangle *comp = compare->data;
          CobiwmRectangle overlap;

          if (cobiwm_rectangle_intersect (cur, comp, &overlap))
            {
              ...
              CobiwmRectangle *overlap_allocated = g_new (CobiwmRectangle, 1);
...
gint
cobiwm_rectangle_edge_cmp_ignore_type (gconstpointer a, gconstpointer b)
{
  const CobiwmEdge *a_edge_rect = (gconstpointer) a;
  const CobiwmEdge *b_edge_rect = (gconstpointer) b;
  ...
  if (a_edge_rect->side_type == COBIWM_SIDE_LEFT ||
      a_edge_rect->side_type == COBIWM_SIDE_RIGHT)
    {
  ...
  else if (a_edge_rect->side_type == COBIWM_SIDE_TOP ||
           a_edge_rect->side_type == COBIWM_SIDE_BOTTOM)
    {
      a_compare = a_edge_rect->rect.y;
      b_compare = b_edge_rect->rect.y;
      if (a_compare == b_compare)
...
gint
cobiwm_rectangle_edge_cmp (gconstpointer a, gconstpointer b)
{
  const CobiwmEdge *a_edge_rect = (gconstpointer) a;
  const CobiwmEdge *b_edge_rect = (gconstpointer) b;
  ...
  if (a_compare == b_compare)
    return cobiwm_rectangle_edge_cmp_ignore_type (a, b);
...
static gboolean
edges_overlap (const CobiwmEdge *edge1,
               const CobiwmEdge *edge2)
{
  if (edge1->rect.width == 0 && edge2->rect.width == 0)
    {
      return cobiwm_rectangle_vert_overlap (&edge1->rect, &edge2->rect) &&
             edge1->rect.x == edge2->rect.x;
    }
  else if (edge1->rect.height == 0 && edge2->rect.height == 0)
    {
      return cobiwm_rectangle_horiz_overlap (&edge1->rect, &edge2->rect) &&
...
static gboolean
rectangle_and_edge_intersection (const CobiwmRectangle *rect,
                                 const CobiwmEdge      *edge,
                                 CobiwmEdge            *overlap,
                                 int                 *handle_type)
{
  const CobiwmRectangle *rect2  = &edge->rect;
  CobiwmRectangle *result = &overlap->rect;
...
{
  CobiwmEdge *temp_edge;
  int i;

  for (i=0; i<4; i++)
    {
      temp_edge = g_new (CobiwmEdge, 1);
      temp_edge->rect = *rect;
      switch (i)
        {
        case 0:
          temp_edge->side_type =
            rect_is_internal ? COBIWM_SIDE_LEFT : COBIWM_SIDE_RIGHT;
          temp_edge->rect.width = 0;
          break;
        case 1:
          temp_edge->side_type =
            rect_is_internal ? COBIWM_SIDE_RIGHT : COBIWM_SIDE_LEFT;
          temp_edge->rect.x     += temp_edge->rect.width;
          temp_edge->rect.width  = 0;
          break;
        case 2:
          temp_edge->side_type =
            rect_is_internal ? COBIWM_SIDE_TOP : COBIWM_SIDE_BOTTOM;
          temp_edge->rect.height = 0;
          break;
        case 3:
          temp_edge->side_type =
            rect_is_internal ? COBIWM_SIDE_BOTTOM : COBIWM_SIDE_TOP;
          temp_edge->rect.y      += temp_edge->rect.height;
          temp_edge->rect.height  = 0;
          break;
        }
      temp_edge->edge_type = COBIWM_EDGE_SCREEN;
...
split_edge (GList *cur_list,
            const CobiwmEdge *old_edge,
            const CobiwmEdge *remove)
{
  CobiwmEdge *temp_edge;
  switch (old_edge->side_type)
    ...
    case COBIWM_SIDE_LEFT:
    case COBIWM_SIDE_RIGHT:
      g_assert (cobiwm_rectangle_vert_overlap (&old_edge->rect, &remove->rect));
    ...
    case COBIWM_SIDE_TOP:
    case COBIWM_SIDE_BOTTOM:
      g_assert (cobiwm_rectangle_horiz_overlap (&old_edge->rect, &remove->rect));
 ...
 */
static void
fix_up_edges (CobiwmRectangle *rect,         CobiwmEdge *edge,
...
/**
 * cobiwm_rectangle_remove_intersections_with_boxes_from_edges: (skip)
 *
 * This function removes intersections of edges with the rectangles from the
 * list of edges.
 */
GList*
cobiwm_rectangle_remove_intersections_with_boxes_from_edges (
  GList        *edges,
  const GSList *rectangles)
{
  ...
  while (rect_iter)
    {
      CobiwmRectangle *rect = rect_iter->data;
      GList *edge_iter = edges;
      while (edge_iter)
        {
          CobiwmEdge *edge = edge_iter->data;
          CobiwmEdge overlap;
          int      handle;
...
/**
 * cobiwm_rectangle_find_onscreen_edges: (skip)
 *
 * This function is trying to find all the edges of an onscreen region.
 */
GList*
cobiwm_rectangle_find_onscreen_edges (const CobiwmRectangle *basic_rect,
                                    const GSList        *all_struts)
{
  GList        *ret;
  ...
  while (strut_rect_iter)
    {
      CobiwmRectangle *strut_rect = (CobiwmRectangle*) strut_rect_iter->data;
      ...
      while (edge_iter)
        {
          CobiwmEdge *cur_edge = edge_iter->data;
  ...
  /* Sort the list */
  ret = g_list_sort (ret, cobiwm_rectangle_edge_cmp);

  /* Free the fixed struts list */
  cobiwm_rectangle_free_list_and_elements (fixed_strut_rects);

  return ret;
}

/**
 * cobiwm_rectangle_find_nonintersected_monitor_edges: (skip)
 *
 */
GList*
                                    ...
                                    const GSList        *all_struts)
{
   ...
   * cobiwm_rectangle_find_onscreen_edges() because real screen edges
  ...
  while (cur)
    {
      CobiwmRectangle *cur_rect = cur->data;
      const GList *compare = monitor_rects;
      while (compare)
        {
          CobiwmRectangle *compare_rect = compare->data;

          /* Check if cur might be horizontally adjacent to compare */
          if (cobiwm_rectangle_vert_overlap(cur_rect, compare_rect))
            {
              ...
              if (x != INT_MIN)
                {
                   ...
                   */
                  CobiwmEdge *new_edge  = g_new (CobiwmEdge, 1);

                  new_edge->rect = cobiwm_rect (x, y, width, height);
                  new_edge->side_type = side_type;
                  new_edge->edge_type = COBIWM_EDGE_MONITOR;
          ...
          if (cobiwm_rectangle_horiz_overlap(cur_rect, compare_rect))
            {
              CobiwmSide side_type;
  ...
  for (; all_struts; all_struts = all_struts->next)
    temp_rects = g_slist_prepend (temp_rects,
                                  &((CobiwmStrut*)all_struts->data)->rect);
  ret = cobiwm_rectangle_remove_intersections_with_boxes_from_edges (ret,
                                                                   temp_rects);
