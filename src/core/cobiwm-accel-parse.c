...
/* This is copied from GTK+ and modified to work with cobiwm's
          ...
          if (len >= 9 && is_primary (accelerator))
            {
              ...
              accelerator += 9;
              len -= 9;
              mods |= COBIWM_VIRTUAL_CONTROL_MASK;
            }
          ...
          else if (len >= 7 && is_shift (accelerator))
            {
              accelerator += 7;
              len -= 7;
              mods |= COBIWM_VIRTUAL_SHIFT_MASK;
            }
          ...
          else if (len >= 6 && is_modx (accelerator))
            {
              static const guint mod_vals[] = {
                COBIWM_VIRTUAL_ALT_MASK,
                COBIWM_VIRTUAL_MOD2_MASK,
                COBIWM_VIRTUAL_MOD3_MASK,
                COBIWM_VIRTUAL_MOD4_MASK,
                COBIWM_VIRTUAL_MOD5_MASK,
              };
          ...
          else if (len >= 5 && is_alt (accelerator))
            {
              accelerator += 5;
              len -= 5;
              mods |= COBIWM_VIRTUAL_ALT_MASK;
            }
          else if (len >= 6 && is_cobiwm (accelerator))
            {
              accelerator += 6;
              len -= 6;
              mods |= COBIWM_VIRTUAL_COBIWM_MASK;
            }
          else if (len >= 7 && is_hyper (accelerator))
            {
              accelerator += 7;
              len -= 7;
              mods |= COBIWM_VIRTUAL_HYPER_MASK;
            }
          else if (len >= 7 && is_super (accelerator))
            {
              accelerator += 7;
              len -= 7;
              mods |= COBIWM_VIRTUAL_SUPER_MASK;
            }
          else
            {
              gchar last_ch;
...
gboolean
cobiwm_parse_accelerator (const char   *accel,
                        CobiwmKeyCombo *combo)
{
  g_return_val_if_fail (combo != NULL, FALSE);

  *combo = (CobiwmKeyCombo) { 0 };
...
gboolean
cobiwm_parse_modifier (const char          *accel,
                     CobiwmVirtualModifier *mask)
{
