...
/* Cobiwm size/position constraints */
...
#include <math.h>
 ...
 //  */
 // static gboolean
 // constrain_whatever (CobiwmWindow         *window,
 //                     ConstraintInfo     *info,
...
typedef struct
{
  CobiwmRectangle        orig;
  CobiwmRectangle        current;
  ...
  CobiwmRectangle        entire_monitor;
...
static gboolean do_screen_and_monitor_relative_constraints (CobiwmWindow     *window,
                                                            GList          *region_spanning_rectangles,
                                                            ConstraintInfo *info,
                                                            gboolean        check_only);
static gboolean constrain_modal_dialog       (CobiwmWindow         *window,
                                              ConstraintInfo     *info,
                                              ConstraintPriority  priority,
                                              gboolean            check_only);
static gboolean constrain_maximization       (CobiwmWindow         *window,
                                              ConstraintInfo     *info,
                                              ConstraintPriority  priority,
                                              gboolean            check_only);
static gboolean constrain_tiling             (CobiwmWindow         *window,
                                              ConstraintInfo     *info,
                                              ConstraintPriority  priority,
                                              gboolean            check_only);
static gboolean constrain_fullscreen         (CobiwmWindow         *window,
                                              ConstraintInfo     *info,
                                              ConstraintPriority  priority,
                                              gboolean            check_only);
static gboolean constrain_size_increments    (CobiwmWindow         *window,
                                              ConstraintInfo     *info,
                                              ConstraintPriority  priority,
                                              gboolean            check_only);
static gboolean constrain_size_limits        (CobiwmWindow         *window,
                                              ConstraintInfo     *info,
                                              ConstraintPriority  priority,
                                              gboolean            check_only);
static gboolean constrain_aspect_ratio       (CobiwmWindow         *window,
                                              ConstraintInfo     *info,
                                              ConstraintPriority  priority,
                                              gboolean            check_only);
static gboolean constrain_to_single_monitor  (CobiwmWindow         *window,
                                              ConstraintInfo     *info,
                                              ConstraintPriority  priority,
                                              gboolean            check_only);
static gboolean constrain_fully_onscreen     (CobiwmWindow         *window,
                                              ConstraintInfo     *info,
                                              ConstraintPriority  priority,
                                              gboolean            check_only);
static gboolean constrain_titlebar_visible   (CobiwmWindow         *window,
                                              ConstraintInfo     *info,
                                              ConstraintPriority  priority,
                                              gboolean            check_only);
static gboolean constrain_partially_onscreen (CobiwmWindow         *window,
                                              ConstraintInfo     *info,
                                              ConstraintPriority  priority,
                                              gboolean            check_only);
...
static void setup_constraint_info        (ConstraintInfo      *info,
                                          ...
                                          int                  resize_gravity,
                                          const CobiwmRectangle *orig,
                                          CobiwmRectangle       *new);
static void place_window_if_needed       (CobiwmWindow     *window,
                                          ConstraintInfo *info);
static void update_onscreen_requirements (CobiwmWindow     *window,
                                          ConstraintInfo *info);

typedef gboolean (* ConstraintFunc) (CobiwmWindow         *window,
                                     ConstraintInfo     *info,
                                     ConstraintPriority  priority,
                                     gboolean            check_only);
...
static gboolean
do_all_constraints (CobiwmWindow         *window,
                    ConstraintInfo     *info,
                    ConstraintPriority  priority,
                    gboolean            check_only)
{
...
void
cobiwm_window_constrain (CobiwmWindow          *window,
                       CobiwmMoveResizeFlags  flags,
                       int                  resize_gravity,
                       const CobiwmRectangle *orig,
                       CobiwmRectangle       *new)
...
setup_constraint_info (ConstraintInfo      *info,
                       ...
                       CobiwmRectangle       *new)
{
  const CobiwmMonitorInfo *monitor_info;
  CobiwmWorkspace *cur_workspace;

  info->orig    = *orig;
  info->current = *new;

  if (flags & COBIWM_MOVE_RESIZE_MOVE_ACTION && flags & COBIWM_MOVE_RESIZE_RESIZE_ACTION)
    ...
    g_error ("BAD, BAD developer!  No treat for you!  (Fix your calls to "
             "cobiwm_window_move_resize_internal()).\n");

  info->is_user_action = (flags & COBIWM_MOVE_RESIZE_USER_ACTION);
  ...
  monitor_info =
    cobiwm_screen_get_monitor_for_rect (window->screen, &info->current);
  ...
  if (!window->fullscreen || window->fullscreen_monitors[0] == -1)
  ...
  else
    {
      ...
      for (i = 1; i <= 3; i++)
        {
          monitor = window->fullscreen_monitors[i];
          cobiwm_rectangle_union (&info->entire_monitor,
                                &window->screen->monitor_infos[monitor].rect,
                                &info->entire_monitor);
        }
    }
  ...
  info->usable_screen_region   =
    cobiwm_workspace_get_onscreen_region (cur_workspace);
  info->usable_monitor_region =
    cobiwm_workspace_get_onmonitor_region (cur_workspace,
                                         monitor_info->number);
  ...
  cobiwm_topic (COBIWM_DEBUG_GEOMETRY,
              ...
              cobiwm_gravity_to_string (info->resize_gravity),
...
static void
place_window_if_needed(CobiwmWindow     *window,
                       ConstraintInfo *info)
{
  ...
  if (!window->placed &&
      ...
      !window->fullscreen)
    {
      CobiwmRectangle orig_rect;
      CobiwmRectangle placed_rect;
      CobiwmWorkspace *cur_workspace;
      const CobiwmMonitorInfo *monitor_info;

      cobiwm_window_get_frame_rect (window, &placed_rect);

      orig_rect = info->orig;

      cobiwm_window_place (window, orig_rect.x, orig_rect.y,
       ...
       */
      monitor_info =
        cobiwm_screen_get_monitor_for_rect (window->screen, &placed_rect);
      info->entire_monitor = monitor_info->rect;
      cobiwm_window_get_work_area_for_monitor (window,
                                             monitor_info->number,
                                             &info->work_area_monitor);
          ...
          if (window->maximize_horizontally_after_placement ||
            ...
            cobiwm_window_maximize_internal (window,
                (window->maximize_horizontally_after_placement ?
                 COBIWM_MAXIMIZE_HORIZONTAL : 0 ) |
                (window->maximize_vertically_after_placement ?
                 COBIWM_MAXIMIZE_VERTICAL : 0), &info->current);
...
static void
update_onscreen_requirements (CobiwmWindow     *window,
                              ConstraintInfo *info)
{
  gboolean old;
  ...
  window->require_fully_onscreen =
    cobiwm_rectangle_contained_in_region (info->usable_screen_region,
                                        &info->current);
  ...
  window->require_on_single_monitor =
    cobiwm_rectangle_contained_in_region (info->usable_monitor_region,
                                        &info->current);
  ...
  if (window->frame && window->decorated)
    {
      ...
      window->require_titlebar_visible =
        cobiwm_rectangle_overlaps_with_region (info->usable_screen_region,
                                             &titlebar_rect);
...
static inline void
get_size_limits (CobiwmWindow    *window,
                 CobiwmRectangle *min_size,
                 CobiwmRectangle *max_size)
{
  /* We pack the results into CobiwmRectangle structs just for convienience; we
  ...
  cobiwm_window_client_rect_to_frame_rect (window, min_size, min_size);
  cobiwm_window_client_rect_to_frame_rect (window, max_size, max_size);
}

static gboolean
constrain_modal_dialog (CobiwmWindow         *window,
                        ConstraintInfo     *info,
                        ConstraintPriority  priority,
                        gboolean            check_only)
{
  int x, y;
  CobiwmWindow *parent = cobiwm_window_get_transient_for (window);
  CobiwmRectangle child_rect, parent_rect;
  ...
  */
  ...
  cobiwm_window_get_frame_rect (parent, &parent_rect);
...
static gboolean
constrain_maximization (CobiwmWindow         *window,
                        ConstraintInfo     *info,
                        ConstraintPriority  priority,
                        gboolean            check_only)
{
  CobiwmRectangle target_size;
  CobiwmRectangle min_size, max_size;
  ...
  if (COBIWM_WINDOW_TILED_MAXIMIZED (window))
    {
      cobiwm_window_get_current_tile_area (window, &target_size);
    }
  else if (COBIWM_WINDOW_MAXIMIZED (window))
    {
  ...
  else
    {
       ...
       */
      CobiwmDirection  direction;
      GSList        *active_workspace_struts;

      if (window->maximized_horizontally)
        direction = COBIWM_DIRECTION_HORIZONTAL;
      else
        direction = COBIWM_DIRECTION_VERTICAL;
      active_workspace_struts = window->screen->active_workspace->all_struts;

      target_size = info->current;
      cobiwm_rectangle_expand_to_avoiding_struts (&target_size,
                                                &info->entire_monitor,
                                                direction,
...
static gboolean
constrain_tiling (CobiwmWindow         *window,
                  ConstraintInfo     *info,
                  ConstraintPriority  priority,
                  gboolean            check_only)
{
...
static gboolean
constrain_fullscreen (CobiwmWindow         *window,
                      ConstraintInfo     *info,
                      ConstraintPriority  priority,
                      gboolean            check_only)
{
  CobiwmRectangle min_size, max_size, monitor;
  ...
  get_size_limits (window, &min_size, &max_size);
  too_big =   !cobiwm_rectangle_could_fit_rect (&monitor, &min_size);
  too_small = !cobiwm_rectangle_could_fit_rect (&max_size, &monitor);
  if (too_big || too_small)
    return TRUE;

  /* Determine whether constraint is already satisfied; exit if it is */
  constraint_already_satisfied =
    cobiwm_rectangle_equal (&info->current, &monitor);
  if (check_only || constraint_already_satisfied)
    return constraint_already_satisfied;
...
static gboolean
constrain_size_increments (CobiwmWindow         *window,
                           ConstraintInfo     *info,
                           ConstraintPriority  priority,
                           gboolean            check_only)
{
  ...
  /* Determine whether constraint applies; exit if it doesn't */
  if (COBIWM_WINDOW_MAXIMIZED (window) || window->fullscreen ||
      COBIWM_WINDOW_TILED_SIDE_BY_SIDE (window) ||
      info->action_type == ACTION_MOVE)
    return TRUE;

  cobiwm_window_frame_rect_to_client_rect (window, &info->current, &client_rect);
...

  {
    ...
    cobiwm_window_client_rect_to_frame_rect (window, &client_rect, &client_rect);
    new_width = client_rect.width;
    new_height = client_rect.height;
  }

  /* Figure out what original rect to pass to cobiwm_rectangle_resize_with_gravity
   * See bug 448183
   */
  ...
  cobiwm_rectangle_resize_with_gravity (start_rect,
                                      &info->current,
                                      info->resize_gravity,
                                      new_width,
                                      new_height);
...
static gboolean
constrain_size_limits (CobiwmWindow         *window,
                       ConstraintInfo     *info,
                       ConstraintPriority  priority,
                       gboolean            check_only)
{
  ...
  too_small = !cobiwm_rectangle_could_fit_rect (&info->current, &min_size);
  too_big   = !cobiwm_rectangle_could_fit_rect (&max_size, &info->current);
...
static gboolean
constrain_aspect_ratio (CobiwmWindow         *window,
                        ConstraintInfo     *info,
                        ConstraintPriority  priority,
                        gboolean            check_only)
{
  ...
  if (constraints_are_inconsistent ||
      COBIWM_WINDOW_MAXIMIZED (window) || window->fullscreen ||
      COBIWM_WINDOW_TILED_SIDE_BY_SIDE (window) ||
      info->action_type == ACTION_MOVE)
    return TRUE;
   ...
   * because of how cobiwm_rectangle_resize_with_gravity treats those as
  ...
  switch (info->resize_gravity)
    ...
    default:
       ...
       * new_height)
       */
      cobiwm_rectangle_find_linepoint_closest_to_point (alt_width, new_height,
  ...
  gboolean        check_only)
{
  ...
  CobiwmRectangle how_far_it_can_be_smushed, min_size, max_size;
  ...
  if (cobiwm_is_verbose ())
    {
      ...
      cobiwm_topic (COBIWM_DEBUG_GEOMETRY,
             ...
             cobiwm_rectangle_region_to_string (region_spanning_rectangles, ", ",
                                              spanning_region));
    }
#endif
  ...
  if (!cobiwm_rectangle_could_fit_in_region (region_spanning_rectangles,
                                           &how_far_it_can_be_smushed))
    exit_early = TRUE;

  /* Determine whether constraint is already satisfied; exit if it is */
  constraint_satisfied =
    cobiwm_rectangle_contained_in_region (region_spanning_rectangles,
                                        &info->current);
  ...
  if (info->action_type != ACTION_MOVE)
    cobiwm_rectangle_clamp_to_fit_into_region (region_spanning_rectangles,
  ...
  if (info->is_user_action && info->action_type == ACTION_RESIZE)
    ...
    cobiwm_rectangle_clip_to_region (region_spanning_rectangles,
                                   info->fixed_directions,
                                   &info->current);
  else
    /* For everything else, shove the rectangle into the relevant region */
    cobiwm_rectangle_shove_into_region (region_spanning_rectangles,
                                      info->fixed_directions,
                                      &info->current);
...
static gboolean
constrain_to_single_monitor (CobiwmWindow         *window,
                             ConstraintInfo     *info,
                             ConstraintPriority  priority,
                             gboolean            check_only)
{
...
static gboolean
constrain_fully_onscreen (CobiwmWindow         *window,
                          ConstraintInfo     *info,
                          ConstraintPriority  priority,
                          gboolean            check_only)
{
...
static gboolean
constrain_titlebar_visible (CobiwmWindow         *window,
                            ConstraintInfo     *info,
                            ConstraintPriority  priority,
                            gboolean            check_only)
{
   ...
   */
  cobiwm_rectangle_expand_region_conditionally (info->usable_screen_region,
...
static gboolean
constrain_partially_onscreen (CobiwmWindow         *window,
                              ConstraintInfo     *info,
                              ConstraintPriority  priority,
                              gboolean            check_only)
{
