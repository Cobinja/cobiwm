...
{
  CobiwmWindow *window = user_data;

  window->dialog_pid = -1;

  /* exit status of 1 means the user pressed "Force Quit" */
  if (WIFEXITED (status) && WEXITSTATUS (status) == 1)
    cobiwm_window_kill (window);
}

static void
present_existing_delete_dialog (CobiwmWindow *window,
                                guint32     timestamp)
{
      ...
      while (tmp != NULL)
        {
          ...
          if (w->transient_for == window && w->res_class &&
              g_ascii_strcasecmp (w->res_class, "cobiwm-dialog") == 0)
            {
              cobiwm_window_activate (w, timestamp);
              break;
            }
...
static void
show_delete_dialog (CobiwmWindow *window,
                    guint32     timestamp)
{
  char *window_title;
...
static void
kill_delete_dialog (CobiwmWindow *window)
{
...
void
cobiwm_window_set_alive (CobiwmWindow *window,
                       gboolean    is_alive)
{
  if (is_alive)
...
void
cobiwm_window_check_alive (CobiwmWindow *window,
                         guint32     timestamp)
{
  cobiwm_display_ping_window (window, timestamp);
}

void
cobiwm_window_delete (CobiwmWindow  *window,
                    guint32      timestamp)
{
  COBIWM_WINDOW_GET_CLASS (window)->delete (window, timestamp);

  cobiwm_window_check_alive (window, timestamp);
}

void
cobiwm_window_kill (CobiwmWindow *window)
{
  COBIWM_WINDOW_GET_CLASS (window)->kill (window);
}

void
cobiwm_window_free_delete_dialog (CobiwmWindow *window)
{
  if (window->dialog_pid >= 0)
    {
