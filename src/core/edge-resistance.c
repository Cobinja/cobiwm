...
#define WINDOW_EDGES_RELEVANT(window, display) \
  cobiwm_window_should_be_showing (window) &&    \
  window         != display->grab_window &&    \
  window->type   != COBIWM_WINDOW_DESKTOP &&     \
  window->type   != COBIWM_WINDOW_MENU    &&     \
...
struct CobiwmEdgeResistanceData
{
  GArray *left_edges;
  GArray *right_edges;
  GArray *top_edges;
...
static void compute_resistance_and_snapping_edges (CobiwmDisplay *display);
                                  ...
                                  gboolean      horizontal)
{
   ...
   */
  mid  = 0;
  edge = g_array_index (edges, CobiwmEdge*, mid);
...
find_nearest_position (const GArray        *edges,
                       int                  position,
                       int                  old_position,
                       const CobiwmRectangle *new_rect,
                       gboolean             horizontal,
                       gboolean             only_forward)
{
  ...
  edge = g_array_index (edges, CobiwmEdge*, mid);
  compare = horizontal ? edge->rect.x : edge->rect.y;
  edges_align = cobiwm_rectangle_edge_aligns (new_rect, edge);
  if (edges_align &&
  ...
  for (i = mid + 1; i < (int)edges->len; i++)
    {
      edge = g_array_index (edges, CobiwmEdge*, i);
      compare = horizontal ? edge->rect.x : edge->rect.y;

      edges_align = horizontal ?
        cobiwm_rectangle_vert_overlap (&edge->rect, new_rect) :
        cobiwm_rectangle_horiz_overlap (&edge->rect, new_rect);
...
static gboolean
movement_towards_edge (CobiwmSide side, int increment)
{
  switch (side)
    {
...
static int
apply_edge_resistance (CobiwmWindow                *window,
                       int                        old_pos,
                       int                        new_pos,
                       const CobiwmRectangle       *old_rect,
                       const CobiwmRectangle       *new_rect,
                       GArray                    *edges,
                       ...
                       gboolean                   keyboard_op)
{
  ...
  while ((increasing  && i <= end) ||
    ...
    {
      gboolean  edges_align;
      CobiwmEdge *edge = g_array_index (edges, CobiwmEdge*, i);
      int       compare = xdir ? edge->rect.x : edge->rect.y;

      /* Find out if this edge is relevant */
      edges_align = cobiwm_rectangle_edge_aligns (new_rect, edge)  ||
                    cobiwm_rectangle_edge_aligns (old_rect, edge);
              ...
              if (!resistance_data->timeout_setup &&
                  timeout_length_ms != 0)
                {
                  ...
                  g_source_set_name_by_id (resistance_data->timeout_id,
                                           "[cobiwm] edge_resistance_timeout");
 ...
 */
static gboolean
apply_edge_resistance_to_each_side (CobiwmDisplay         *display,
                                    CobiwmWindow          *window,
                                    const CobiwmRectangle *old_outer,
                                    CobiwmRectangle       *new_outer,
                                    GSourceFunc          timeout_func,
                                    gboolean             auto_snap,
                                    gboolean             keyboard_op,
                                    gboolean             is_resize)
{
  CobiwmEdgeResistanceData *edge_data;
  CobiwmRectangle           modified_rect;
  gboolean                modified;
  ...
  modified_rect = cobiwm_rect (new_left,
                             new_top,
                             new_right - new_left,
                             new_bottom - new_top);
  modified = !cobiwm_rectangle_equal (new_outer, &modified_rect);
  *new_outer = modified_rect;
  return modified;
}

void
cobiwm_display_cleanup_edges (CobiwmDisplay *display)
{
  guint i,j;
  CobiwmEdgeResistanceData *edge_data = display->grab_edge_resistance_data;
  ...
  for (i = 0; i < 4; i++)
    {
      ...
      switch (i)
        ...
        case 0:
          ...
          side = COBIWM_SIDE_LEFT;
          break;
        case 1:
          tmp = edge_data->right_edges;
          side = COBIWM_SIDE_RIGHT;
          break;
        case 2:
          tmp = edge_data->top_edges;
          side = COBIWM_SIDE_TOP;
          break;
        case 3:
          tmp = edge_data->bottom_edges;
          side = COBIWM_SIDE_BOTTOM;
          break;
        default:
          g_assert_not_reached ();
        }
      ...
      for (j = 0; j < tmp->len; j++)
        {
          CobiwmEdge *edge = g_array_index (tmp, CobiwmEdge*, j);
          if (edge->edge_type == COBIWM_EDGE_WINDOW &&
              edge->side_type == side)
            {
...
stupid_sort_requiring_extra_pointer_dereference (gconstpointer a,
                                                 gconstpointer b)
{
  const CobiwmEdge * const *a_edge = a;
  const CobiwmEdge * const *b_edge = b;
  return cobiwm_rectangle_edge_cmp_ignore_type (*a_edge, *b_edge);
}

static void
cache_edges (CobiwmDisplay *display,
             GList *window_edges,
             GList *monitor_edges,
             GList *screen_edges)
{
  ...
  if (cobiwm_is_verbose())
    {
      ...
      cobiwm_rectangle_edge_list_to_string (window_edges, ", ", big_buffer);
      cobiwm_topic (COBIWM_DEBUG_EDGE_RESISTANCE,
                  "Window edges for resistance  : %s\n", big_buffer);

      cobiwm_rectangle_edge_list_to_string (monitor_edges, ", ", big_buffer);
      cobiwm_topic (COBIWM_DEBUG_EDGE_RESISTANCE,
                  "Monitor edges for resistance: %s\n", big_buffer);

      cobiwm_rectangle_edge_list_to_string (screen_edges, ", ", big_buffer);
      cobiwm_topic (COBIWM_DEBUG_EDGE_RESISTANCE,
  ...
  display->grab_edge_resistance_data = g_new0 (CobiwmEdgeResistanceData, 1);
  edge_data = display->grab_edge_resistance_data;
...
static void
initialize_grab_edge_resistance_data (CobiwmDisplay *display)
{
  CobiwmEdgeResistanceData *edge_data = display->grab_edge_resistance_data;
...
static void
compute_resistance_and_snapping_edges (CobiwmDisplay *display)
{
   ...
   */
  stacked_windows =
    cobiwm_stack_list_windows (display->screen->stack,
  ...
  while (cur_window_iter != NULL)
    {
      CobiwmWindow *cur_window = cur_window_iter->data;
      if (WINDOW_EDGES_RELEVANT (cur_window, display))
        {
          CobiwmRectangle *new_rect;
          new_rect = g_new (CobiwmRectangle, 1);
          cobiwm_window_get_frame_rect (cur_window, new_rect);
  ...
  while (cur_window_iter != NULL)
    {
      CobiwmRectangle  cur_rect;
      CobiwmWindow    *cur_window = cur_window_iter->data;
      cobiwm_window_get_frame_rect (cur_window, &cur_rect);
       ...
       */
      if (WINDOW_EDGES_RELEVANT (cur_window, display) &&
          cur_window->type != COBIWM_WINDOW_DOCK)
        {
          GList *new_edges;
          CobiwmEdge *new_edge;
          CobiwmRectangle reduced;
           ...
           */
          cobiwm_rectangle_intersect (&cur_rect,
                                    &display->screen->rect,
                                    &reduced);
           ...
           */
          new_edge = g_new (CobiwmEdge, 1);
          new_edge->rect = reduced;
          new_edge->rect.width = 0;
          new_edge->side_type = COBIWM_SIDE_RIGHT;
          new_edge->edge_type = COBIWM_EDGE_WINDOW;
          ...
          new_edge->rect.width = 0;
          new_edge->side_type = COBIWM_SIDE_LEFT;
          new_edge->edge_type = COBIWM_EDGE_WINDOW;
          new_edges = g_list_prepend (new_edges, new_edge);
          ...
          new_edge->side_type = COBIWM_SIDE_BOTTOM;
          new_edge->edge_type = COBIWM_EDGE_WINDOW;
          new_edges = g_list_prepend (new_edges, new_edge);
          ...
          new_edge->rect.height = 0;
          new_edge->side_type = COBIWM_SIDE_TOP;
          new_edge->edge_type = COBIWM_EDGE_WINDOW;
          new_edges = g_list_prepend (new_edges, new_edge);
   ...
   */
  edges = g_list_sort (edges, cobiwm_rectangle_edge_cmp);
...
void
cobiwm_window_edge_resistance_for_move (CobiwmWindow  *window,
                                      int         *new_x,
                                      int         *new_y,
                                      GSourceFunc  timeout_func,
                                      gboolean     snap,
...
{
  CobiwmRectangle old_outer, proposed_outer, new_outer;
  gboolean is_resize;

  cobiwm_window_get_frame_rect (window, &old_outer);
...
void
cobiwm_window_edge_resistance_for_resize (CobiwmWindow  *window,
                                        int         *new_width,
                                        int         *new_height,
                                        int          gravity,
                                        GSourceFunc  timeout_func,
                                        ...
                                        gboolean     is_keyboard_op)
{
  CobiwmRectangle old_outer, new_outer;
  ...
  cobiwm_rectangle_resize_with_gravity (&old_outer,
                                      &new_outer,
                                      gravity,
