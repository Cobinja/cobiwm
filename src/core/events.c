...
#include "backends/native/cobiwm-idle-monitor-native.h"
#endif
...
static CobiwmWindow *
get_window_for_event (CobiwmDisplay        *display,
    ...
    case COBIWM_EVENT_ROUTE_NORMAL:
      {
        ...
        if (COBIWM_IS_SURFACE_ACTOR (source))
          return cobiwm_surface_actor_get_window (COBIWM_SURFACE_ACTOR (source));
        else
          return NULL;
      }
  ...
  if (COBIWM_IS_BACKEND_NATIVE (backend))
    {
      ...
      CobiwmIdleMonitor *core_monitor, *device_monitor;
      int device_id;
      ...
      core_monitor = cobiwm_idle_monitor_get_core ();
      device_monitor = cobiwm_idle_monitor_get_for_device (device_id);

      cobiwm_idle_monitor_native_reset_idletime (core_monitor);
      cobiwm_idle_monitor_native_reset_idletime (device_monitor);
...
static gboolean
sequence_is_pointer_emulated (CobiwmDisplay        *display,
                              const ClutterEvent *event)
{
  ...
  if (COBIWM_IS_BACKEND_NATIVE (backend))
    {
      ...
      tracker = cobiwm_display_get_gesture_tracker (display);

      if (event->type == CLUTTER_TOUCH_BEGIN &&
          cobiwm_gesture_tracker_get_n_current_touches (tracker) == 0)
        return TRUE;
    }
#endif /* HAVE_NATIVE_BACKEND */
...
static gboolean
cobiwm_display_handle_event (CobiwmDisplay        *display,
                           const ClutterEvent *event)
{
  CobiwmWindow *window;
...
#ifdef HAVE_WAYLAND
  CobiwmWaylandCompositor *compositor = NULL;
  if (cobiwm_is_wayland_compositor ())
    {
      compositor = cobiwm_wayland_compositor_get_default ();
      cobiwm_wayland_compositor_update (compositor, event);
    }
#endif
  ...
  if (source)
    {
      cobiwm_backend_update_last_device (cobiwm_get_backend (),
                                       clutter_input_device_get_device_id (source));
    }

  if (cobiwm_is_wayland_compositor () && event->type == CLUTTER_MOTION)
    {
      cobiwm_cursor_tracker_update_position (cobiwm_cursor_tracker_get_for_screen (NULL),
      ...
      if (CurrentTime == display->current_time)
        {
           ...
           */
          cobiwm_warning ("Event has no timestamp! You may be using a broken "
                        "program such as xse.  Please ask the authors of that "
                        "program to fix it.\n");
        }
      else
        {
          cobiwm_window_set_user_time (window, display->current_time);
          cobiwm_display_sanity_check_timestamps (display, display->current_time);
        }
    }

  tracker = cobiwm_display_get_gesture_tracker (display);

  if (cobiwm_gesture_tracker_handle_event (tracker, event))
    {
  ...
  if (display->event_route == COBIWM_EVENT_ROUTE_WINDOW_OP)
    {
      if (cobiwm_window_handle_mouse_grab_op_event (window, event))
        {
          bypass_clutter = TRUE;
          bypass_wayland = TRUE;
          goto out;
   ...
   */
  if (cobiwm_keybindings_process_event (display, window, event))
    {
      bypass_clutter = TRUE;
      bypass_wayland = TRUE;
      goto out;
  ...
  if (window)
    {
       ...
       *   directly by cobiwm_gesture_tracker_handle_event().
      ...
      cobiwm_window_handle_ungrabbed_event (window, event);
      ...
      if (display->event_route == COBIWM_EVENT_ROUTE_WINDOW_OP ||
          display->event_route == COBIWM_EVENT_ROUTE_FRAME_BUTTON)
        {
          bypass_clutter = TRUE;
          bypass_wayland = TRUE;
        }
              ...
              if (COBIWM_IS_BACKEND_X11 (backend))
                {
                  ...
                  cobiwm_verbose ("Allowing events time %u\n",
 ...
 out:
  ...
  if (display->event_route == COBIWM_EVENT_ROUTE_COMPOSITOR_GRAB)
    bypass_wayland = TRUE;

  /* If a Wayland client has a grab, don't pass that through to Clutter */
  if (display->event_route == COBIWM_EVENT_ROUTE_WAYLAND_POPUP)
    bypass_clutter = TRUE;

#ifdef HAVE_WAYLAND
  if (compositor && !bypass_wayland)
    {
      if (cobiwm_wayland_compositor_handle_event (compositor, event))
        bypass_clutter = TRUE;
    }
#endif
                ...
                gpointer            data)
{
  ...
  return cobiwm_display_handle_event (display, event);
}

void
cobiwm_display_init_events (CobiwmDisplay *display)
{
...
void
cobiwm_display_free_events (CobiwmDisplay *display)
{
