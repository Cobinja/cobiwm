...
northwestcmp (gconstpointer a, gconstpointer b)
{
  ...
  CobiwmRectangle a_frame;
  CobiwmRectangle b_frame;
  int from_origin_a;
  int from_origin_b;
  int ax, ay, bx, by;

  cobiwm_window_get_frame_rect (aw, &a_frame);
  cobiwm_window_get_frame_rect (bw, &b_frame);
  ax = a_frame.x;
  ay = a_frame.y;
  bx = b_frame.x;
  by = b_frame.y;
...
static void
find_next_cascade (CobiwmWindow *window,
...
#define CASCADE_FUZZ 15
  ...
  current = cobiwm_screen_get_current_monitor (window->screen);
  cobiwm_window_get_work_area_for_monitor (window, current, &work_area);
  ...
  while (tmp != NULL)
    {
      CobiwmWindow *w;
      CobiwmRectangle w_frame_rect;
      int wx, wy;

      w = tmp->data;

      /* we want frame position, not window position */
      cobiwm_window_get_frame_rect (w, &w_frame_rect);
      wx = w_frame_rect.x;
      wy = w_frame_rect.y;
      ...
      if (ABS (wx - cascade_x) < x_threshold &&
        ...
        {
          cobiwm_window_get_titlebar_rect (w, &titlebar_rect);
...
static void
find_most_freespace (CobiwmWindow *window,
                     /* visible windows on relevant workspaces */
                     ...
                     int        *new_y)
{
  CobiwmWindowDirection side;
  int max_area;
  int max_width, max_height, left, right, top, bottom;
  int left_space, right_space, top_space, bottom_space;
  CobiwmRectangle work_area;
  CobiwmRectangle avoid;
  CobiwmRectangle frame_rect;

  cobiwm_window_get_work_area_current_monitor (focus_window, &work_area);
  cobiwm_window_get_frame_rect (focus_window, &avoid);
  cobiwm_window_get_frame_rect (window, &frame_rect);
...
static gboolean
window_overlaps_focus_window (CobiwmWindow *window)
{
  CobiwmWindow *focus_window;
  CobiwmRectangle window_frame, focus_frame, overlap;

  focus_window = window->display->focus_window;
  if (focus_window == NULL)
    return FALSE;

  cobiwm_window_get_frame_rect (window, &window_frame);
  cobiwm_window_get_frame_rect (focus_window, &focus_frame);

  return cobiwm_rectangle_intersect (&window_frame,
                                   &focus_frame,
                                   &overlap);
}

static gboolean
window_place_centered (CobiwmWindow *window)
{
  CobiwmWindowType type;

  type = window->type;

  return (type == COBIWM_WINDOW_DIALOG ||
    type == COBIWM_WINDOW_MODAL_DIALOG ||
    type == COBIWM_WINDOW_SPLASHSCREEN ||
    (type == COBIWM_WINDOW_NORMAL && cobiwm_prefs_get_center_new_windows ()));
}

static void
avoid_being_obscured_as_second_modal_dialog (CobiwmWindow *window,
                                             int        *x,
                                             int        *y)
{
  ...
  if (window->denied_focus_and_not_transient &&
      ...
      cobiwm_window_same_application (window, focus_window) &&
...
static gboolean
rectangle_overlaps_some_window (CobiwmRectangle *rect,
                                GList         *windows)
{
  GList *tmp;
  ...
  while (tmp != NULL)
    {
      CobiwmWindow *other = tmp->data;
      CobiwmRectangle other_rect;
	...
	case COBIWM_WINDOW_OVERRIDE_OTHER:
        ...
        case COBIWM_WINDOW_MENU:
          cobiwm_window_get_frame_rect (other, &other_rect);

          if (cobiwm_rectangle_intersect (rect, &other_rect, &dest))
            return TRUE;
          break;
        }
...
static void
center_tile_rect_in_area (CobiwmRectangle *rect,
                          CobiwmRectangle *work_area)
{
  int fluff;
 ...
 */
static gboolean
find_first_fit (CobiwmWindow *window,
                /* visible windows on relevant workspaces */
                GList      *windows,
		int         monitor,
                int         x,
                ...
                int        *new_y)
{
...
#ifdef WITH_VERBOSE_MODE
  {
    ...
    cobiwm_rectangle_to_string (&window->screen->monitor_infos[monitor].rect,
  ...
  if (cobiwm_rectangle_contains_rect (&work_area, &rect) &&
  ...
  while (tmp != NULL)
    {
      ...
      cobiwm_window_get_frame_rect (w, &frame_rect);
...
void
cobiwm_window_place (CobiwmWindow        *window,
                   int                x,
                   int                y,
                   int               *new_x,
                   int               *new_y)
{
  GList *windows = NULL;
  const CobiwmMonitorInfo *xi;

  cobiwm_topic (COBIWM_DEBUG_PLACEMENT, "Placing window %s\n", window->desc);
  ...
  if (window->type == COBIWM_WINDOW_DIALOG ||
      window->type == COBIWM_WINDOW_MODAL_DIALOG)
    {
      ...
      if (parent)
        {
          CobiwmRectangle frame_rect, parent_frame_rect;

          cobiwm_window_get_frame_rect (window, &frame_rect);
          cobiwm_window_get_frame_rect (parent, &parent_frame_rect);
          ...
          cobiwm_topic (COBIWM_DEBUG_PLACEMENT, "Centered window %s over transient parent\n",
                      window->desc);
  ...
  if (window_place_centered (window))
    {
      ...
      xi = cobiwm_screen_get_current_monitor_info (window->screen);
      ...
      cobiwm_topic (COBIWM_DEBUG_PLACEMENT, "Centered window %s on screen %d monitor %d\n",
   ...
   */
  {
    ...
    all_windows = cobiwm_display_list_windows (window->display, COBIWM_LIST_DEFAULT);
    ...
    while (tmp != NULL)
      {
        ...
        if (w != window &&
            cobiwm_window_showing_on_its_workspace (w) &&
            (window->on_all_workspaces ||
             cobiwm_window_located_on_workspace (w, window->workspace)))
