 ...
 * SECTION:prefs
 * @title: Preferences
 * @short_description: Cobiwm preferences
 */
...
#define SCHEMA_COBIWM          "org.cobiwm"
#define SCHEMA_INTERFACE       "org.cinnamon.desktop.interface"
#define SCHEMA_INPUT_SOURCES   "org.cinnamon.desktop.input-sources"
#define SCHEMA_XSETTINGS       "org.cinnamon.settings-daemon.plugins.xsettings"
#define SCHEMA_MOUSE           "org.cinnamon.settings-daemon.peripherals.mouse"
...
static CobiwmVirtualModifier mouse_button_mods = Mod1Mask;
static CobiwmKeyCombo overlay_key_combo = { 0, 0, 0 };
...
static CobiwmButtonLayout button_layout;
...
static void handle_preference_update_enum (GSettings *settings,
                                           gchar     *key);
static gboolean update_binding         (CobiwmKeyPref *binding,
                                        gchar      **strokes);
...
static void queue_changed (CobiwmPreference  pref);
...
typedef struct
{
  const char *key;
  const char *schema;
  CobiwmPreference pref;
} CobiwmBasePreference;

typedef struct
{
  CobiwmBasePreference base;
  gpointer target;
} CobiwmEnumPreference;

typedef struct
{
  CobiwmBasePreference base;
  gboolean *target;
} CobiwmBoolPreference;


/**
 * CobiwmStringPreference:
 ...
 * plus in the GSettings schemas and the CobiwmPreference enum.
 */

/* FIXMEs: */
/* @@@ Don't use NULL lines at the end; glib can tell you how big it is */

static CobiwmEnumPreference preferences_enum[] =
  {
    {
      { "focus-new-windows",
        SCHEMA_GENERAL,
...
static CobiwmBoolPreference preferences_bool[] =
  {
    {
    ...
    },
    {
      { "titlebar-uses-system-font",
        SCHEMA_GENERAL,
        COBIWM_PREF_TITLEBAR_FONT, /* note! shares a pref */
      },
      &use_system_font,
    },
    {
...
static CobiwmStringPreference preferences_string[] =
  {
    {
...
static CobiwmStringArrayPreference preferences_string_array[] =
  {
    {
      { KEY_WORKSPACE_NAMES,
        SCHEMA_GENERAL,
...
static CobiwmIntPreference preferences_int[] =
  {
    {
      { KEY_NUM_WORKSPACES,
        SCHEMA_GENERAL,
 ...
 * override preferences from the "normal" cobiwm
...
{
  CobiwmEnumPreference *cursor = preferences_enum;
...
{
  CobiwmBoolPreference *cursor = preferences_bool;
...
{
  CobiwmStringPreference *cursor = preferences_string;
  ...
  while (cursor->base.key != NULL)
    {
      ...
      if (cursor->handler)
        {
          if (cursor->target)
            cobiwm_bug ("%s has both a target and a handler\n", cursor->base.key);
        ...
        {
          if (!cursor->target)
            cobiwm_bug ("%s must have handler or target\n", cursor->base.key);
...
{
  CobiwmStringArrayPreference *cursor = preferences_string_array;
...
{
  CobiwmIntPreference *cursor = preferences_int;
                               ...
                               gchar *key)
{
  ...
  *((gboolean *) cursor->target) =
  ...
  if (cursor->base.pref==COBIWM_PREF_DISABLE_WORKAROUNDS)
    maybe_give_disable_workarounds_warning ();
}
...
/**
 * cobiwm_prefs_add_listener: (skip)
 * @func: a #CobiwmPrefsChangedFunc
 * @user_data: data passed to the function
 *
 */
void
cobiwm_prefs_add_listener (CobiwmPrefsChangedFunc func,
                         gpointer             user_data)
{
  CobiwmPrefsListener *l;

  l = g_new (CobiwmPrefsListener, 1);
  l->func = func;
  l->data = user_data;
...
/**
 * cobiwm_prefs_remove_listener: (skip)
 * @func: a #CobiwmPrefsChangedFunc
 * @user_data: data passed to the function
 *
 */
void
cobiwm_prefs_remove_listener (CobiwmPrefsChangedFunc func,
                            gpointer             user_data)
{
  GList *tmp;
  ...
  while (tmp != NULL)
    {
      CobiwmPrefsListener *l = tmp->data;
  ...
  cobiwm_bug ("Did not find listener to remove\n");
}

static void
emit_changed (CobiwmPreference pref)
{
  GList *tmp;
  GList *copy;

  cobiwm_topic (COBIWM_DEBUG_PREFS, "Notifying listeners that pref %s changed\n",
              cobiwm_preference_to_string (pref));
...
changed_idle_handler (gpointer data)
{
  ...
  while (tmp != NULL)
    {
      CobiwmPreference pref = GPOINTER_TO_INT (tmp->data);
...
static void
queue_changed (CobiwmPreference pref)
{
  cobiwm_topic (COBIWM_DEBUG_PREFS, "Queueing change of pref %s\n",
              cobiwm_preference_to_string (pref));

  if (g_list_find (changes, GINT_TO_POINTER (pref)) == NULL)
    changes = g_list_prepend (changes, GINT_TO_POINTER (pref));
  else
    cobiwm_topic (COBIWM_DEBUG_PREFS, "Change of pref %s was already pending\n",
                cobiwm_preference_to_string (pref));

  if (changed_idle == 0)
    {
      changed_idle = g_idle_add_full (COBIWM_PRIORITY_PREFS_NOTIFY,
                                      changed_idle_handler, NULL, NULL);
      g_source_set_name_by_id (changed_idle, "[cobiwm] changed_idle_handler");
    }
}
...
cobiwm_prefs_init (void)
{
  ...
  schema = g_settings_schema_source_lookup(schemaSource, SCHEMA_COBIWM, TRUE);
  schemaPath = g_settings_schema_get_path(schema);
  configFilename = g_strdup_printf("%s/cobiwm/settings.conf", g_get_user_config_dir());
  settingsBackend = g_keyfile_settings_backend_new(configFilename, schemaPath, "general");
  
  settings = g_settings_new_with_backend (SCHEMA_COBIWM, settingsBackend);
  g_signal_connect (settings, "changed", G_CALLBACK (settings_changed), NULL);
  g_hash_table_insert (settings_schemas, g_strdup (SCHEMA_COBIWM), settings);
  ...
  for (tmp = overridden_keys; tmp; tmp = tmp->next)
    {
      CobiwmPrefsOverriddenKey *override = tmp->data;
             ...
             char *schema)
{
  ...
  if (!find_pref (preferences_enum, sizeof(CobiwmEnumPreference), key, &pref) &&
      !find_pref (preferences_bool, sizeof(CobiwmBoolPreference), key, &pref) &&
      !find_pref (preferences_string, sizeof(CobiwmStringPreference), key, &pref) &&
      !find_pref (preferences_int, sizeof(CobiwmIntPreference), key, &pref))
    {
      cobiwm_warning ("Can't override preference key, \"%s\" not found\n", key);
      return;
    }
...
/**
 * cobiwm_prefs_override_preference_schema:
 * @key: the preference name
 * @schema: new schema for preference @key
 *
 * Specify a schema whose keys are used to override the standard Cobiwmcity
 * keys. This might be used if a plugin expected a different value for
 * some preference than the Cobiwmcity default. While this function can be
 ...
 */
void
cobiwm_prefs_override_preference_schema (const char *key, const char *schema)
{
  CobiwmPrefsOverriddenKey *overridden;
  GSList *tmp;

  /* Merge identical overrides, this isn't an error */
  for (tmp = overridden_keys; tmp; tmp = tmp->next)
    {
      CobiwmPrefsOverriddenKey *tmp_overridden = tmp->data;
  ...
  if (overridden)
    ...
    {
      overridden = g_slice_new (CobiwmPrefsOverriddenKey);
                  ...
                  gpointer data)
{
  ...
  if (update_key_binding (key, strokes))
    queue_changed (COBIWM_PREF_KEYBINDINGS);
                              ...
                              gpointer data)
{
  ...
  if (changed)
    queue_changed (COBIWM_PREF_BUTTON_LAYOUT);
}
  ...
  if (xsettings_cursor_size != cursor_size)
    {
      ...
      queue_changed (COBIWM_PREF_CURSOR_SIZE);
    }
}
  ...
  if (first_disable && disable_workarounds)
    {
      ...
      cobiwm_warning ("Workarounds for broken applications disabled. "
                    "Some applications may not behave properly.\n");
    }
}

CobiwmVirtualModifier
cobiwm_prefs_get_mouse_button_mods  (void)
{
  return mouse_button_mods;
}

GDesktopFocusMode
cobiwm_prefs_get_focus_mode (void)
{
  return focus_mode;
}

GDesktopFocusNewWindows
cobiwm_prefs_get_focus_new_windows (void)
{
  return focus_new_windows;
}

gboolean
cobiwm_prefs_get_center_new_windows (void)
{
  return center_new_windows;
}

gboolean
cobiwm_prefs_get_attach_modal_dialogs (void)
{
  return attach_modal_dialogs;
}

gboolean
cobiwm_prefs_get_raise_on_click (void)
{
  return raise_on_click;
}

gboolean
cobiwm_prefs_get_show_fallback_app_menu (void)
{
  return show_fallback_app_menu;
}

const char*
cobiwm_prefs_get_cursor_theme (void)
{
  return cursor_theme;
}

int
cobiwm_prefs_get_cursor_size (void)
{
  return cursor_size;
}
                  ...
                  gpointer data)
{
  ...
  *result = NULL; /* ignored */
  ...
  if (desc == NULL)
    {
      cobiwm_warning ("Could not parse font description "
  ...
  if (titlebar_font &&
  ...
  else
    {
      ...
      titlebar_font = desc;
      queue_changed (COBIWM_PREF_TITLEBAR_FONT);
    }
...
mouse_button_mods_handler (GVariant *value,
                           ...
                           gpointer  data)
{
  CobiwmVirtualModifier mods;
  const gchar *string_value;

  *result = NULL; /* ignored */
  string_value = g_variant_get_string (value, NULL);

  if (!string_value || !cobiwm_parse_modifier (string_value, &mods))
    {
      cobiwm_topic (COBIWM_DEBUG_KEYBINDINGS,
                  "Failed to parse new GSettings value\n");

      cobiwm_warning ("\"%s\" found in configuration database is "
  ...
  if (mods != mouse_button_mods)
    {
      ...
      queue_changed (COBIWM_PREF_MOUSE_BUTTON_MODS);
    }
...
static gboolean
button_layout_equal (const CobiwmButtonLayout *a,
                     const CobiwmButtonLayout *b)
{
  int i;
 ...
 */
static CobiwmButtonFunction
button_function_from_string (const char *str)
{
  if (strcmp (str, "menu") == 0)
    return COBIWM_BUTTON_FUNCTION_MENU;
  else if (strcmp (str, "appmenu") == 0)
    return COBIWM_BUTTON_FUNCTION_APPMENU;
  else if (strcmp (str, "minimize") == 0)
    return COBIWM_BUTTON_FUNCTION_MINIMIZE;
  else if (strcmp (str, "maximize") == 0)
    return COBIWM_BUTTON_FUNCTION_MAXIMIZE;
  else if (strcmp (str, "close") == 0)
    return COBIWM_BUTTON_FUNCTION_CLOSE;
  else
    /* don't know; give up */
    return COBIWM_BUTTON_FUNCTION_LAST;
}
                       ...
                       gpointer  data)
{
  ...
  *result = NULL; /* ignored */
  ...
  if (sides != NULL && sides[0] != NULL)
    {
      char **buttons;
      int b;
      gboolean used[COBIWM_BUTTON_FUNCTION_LAST];

      while (i < COBIWM_BUTTON_FUNCTION_LAST)
        {
          used[i] = FALSE;
      ...
      while (buttons[b] != NULL)
        {
          CobiwmButtonFunction f = button_function_from_string (buttons[b]);
          if (i > 0 && strcmp("spacer", buttons[b]) == 0)
            ...
            {
              if (f != COBIWM_BUTTON_FUNCTION_LAST && !used[f])
                {
                ...
                {
                  cobiwm_topic (COBIWM_DEBUG_PREFS, "Ignoring unknown or already-used button name \"%s\"\n",
                              buttons[b]);
                }
            }
  ...
  for (; i < MAX_BUTTONS_PER_CORNER; i++)
    {
      new_layout.left_buttons[i] = COBIWM_BUTTON_FUNCTION_LAST;
      new_layout.left_buttons_has_spacer[i] = FALSE;
    }
  ...
  for (; i < MAX_BUTTONS_PER_CORNER; i++)
    {
      new_layout.right_buttons[i] = COBIWM_BUTTON_FUNCTION_LAST;
      new_layout.right_buttons_has_spacer[i] = FALSE;
    }
  ...
  if (cobiwm_get_locale_direction() == COBIWM_LOCALE_DIRECTION_RTL)
    {
      CobiwmButtonLayout rtl_layout;
      int j;

      for (i = 0; new_layout.left_buttons[i] != COBIWM_BUTTON_FUNCTION_LAST; i++);
      for (j = 0; j < i; j++)
        {
      ...
      for (; j < MAX_BUTTONS_PER_CORNER; j++)
        {
          rtl_layout.right_buttons[j] = COBIWM_BUTTON_FUNCTION_LAST;
          rtl_layout.right_buttons_has_spacer[j] = FALSE;
        }

      for (i = 0; new_layout.right_buttons[i] != COBIWM_BUTTON_FUNCTION_LAST; i++);
      for (j = 0; j < i; j++)
        {
      ...
      for (; j < MAX_BUTTONS_PER_CORNER; j++)
        {
          rtl_layout.left_buttons[j] = COBIWM_BUTTON_FUNCTION_LAST;
  ...
  if (!button_layout_equal (&button_layout, &new_layout))
    {
      ...
      emit_changed (COBIWM_PREF_BUTTON_LAYOUT);
    }
                     ...
                     gpointer  data)
{
  ...
  *result = NULL; /* ignored */
  ...
  if (string_value && cobiwm_parse_accelerator (string_value, &combo))
    ;
  else
    {
      cobiwm_topic (COBIWM_DEBUG_KEYBINDINGS,
...
cobiwm_prefs_get_titlebar_font (void)
{
  if (use_system_font)
    return NULL;
  else
...
int
cobiwm_prefs_get_num_workspaces (void)
{
  return num_workspaces;
}

gboolean
cobiwm_prefs_get_disable_workarounds (void)
{
...
const char*
cobiwm_preference_to_string (CobiwmPreference pref)
{
  ...
  switch (pref)
    ...
    case COBIWM_PREF_FOCUS_MODE:
      return "FOCUS_MODE";

    case COBIWM_PREF_FOCUS_NEW_WINDOWS:
      return "FOCUS_NEW_WINDOWS";

    case COBIWM_PREF_CENTER_NEW_WINDOWS:
      return "CENTER_NEW_WINDOWS";

    case COBIWM_PREF_ATTACH_MODAL_DIALOGS:
      return "ATTACH_MODAL_DIALOGS";

    case COBIWM_PREF_RAISE_ON_CLICK:
      return "RAISE_ON_CLICK";

    case COBIWM_PREF_TITLEBAR_FONT:
      return "TITLEBAR_FONT";

    case COBIWM_PREF_NUM_WORKSPACES:
      return "NUM_WORKSPACES";

    case COBIWM_PREF_KEYBINDINGS:
      return "KEYBINDINGS";

    case COBIWM_PREF_DISABLE_WORKAROUNDS:
      return "DISABLE_WORKAROUNDS";

    case COBIWM_PREF_ACTION_DOUBLE_CLICK_TITLEBAR:
      return "ACTION_DOUBLE_CLICK_TITLEBAR";

    case COBIWM_PREF_ACTION_MIDDLE_CLICK_TITLEBAR:
      return "ACTION_MIDDLE_CLICK_TITLEBAR";

    case COBIWM_PREF_ACTION_RIGHT_CLICK_TITLEBAR:
      return "ACTION_RIGHT_CLICK_TITLEBAR";

    case COBIWM_PREF_AUTO_RAISE:
      return "AUTO_RAISE";

    case COBIWM_PREF_AUTO_RAISE_DELAY:
      return "AUTO_RAISE_DELAY";

    case COBIWM_PREF_FOCUS_CHANGE_ON_POINTER_REST:
      return "FOCUS_CHANGE_ON_POINTER_REST";

    case COBIWM_PREF_BUTTON_LAYOUT:
      return "BUTTON_LAYOUT";

    case COBIWM_PREF_WORKSPACE_NAMES:
      return "WORKSPACE_NAMES";

    case COBIWM_PREF_VISUAL_BELL:
      return "VISUAL_BELL";

    case COBIWM_PREF_AUDIBLE_BELL:
      return "AUDIBLE_BELL";

    case COBIWM_PREF_VISUAL_BELL_TYPE:
      return "VISUAL_BELL_TYPE";

    case COBIWM_PREF_GNOME_ACCESSIBILITY:
      return "GNOME_ACCESSIBILTY";

    case COBIWM_PREF_GNOME_ANIMATIONS:
      return "GNOME_ANIMATIONS";

    case COBIWM_PREF_CURSOR_THEME:
      return "CURSOR_THEME";

    case COBIWM_PREF_CURSOR_SIZE:
      return "CURSOR_SIZE";

    case COBIWM_PREF_RESIZE_WITH_RIGHT_BUTTON:
      return "RESIZE_WITH_RIGHT_BUTTON";

    case COBIWM_PREF_EDGE_TILING:
      return "EDGE_TILING";

    case COBIWM_PREF_FORCE_FULLSCREEN:
      return "FORCE_FULLSCREEN";

    case COBIWM_PREF_WORKSPACES_ONLY_ON_PRIMARY:
      return "WORKSPACES_ONLY_ON_PRIMARY";

    case COBIWM_PREF_DRAGGABLE_BORDER_WIDTH:
      return "DRAGGABLE_BORDER_WIDTH";

    case COBIWM_PREF_DRAG_THRESHOLD:
      return "DRAG_TRHESHOLD";

    case COBIWM_PREF_AUTO_MAXIMIZE:
...
void
cobiwm_prefs_set_num_workspaces (int n_workspaces)
{
  CobiwmBasePreference *pref;

  if (find_pref (preferences_int, sizeof(CobiwmIntPreference),
...
static void
cobiwm_key_pref_free (CobiwmKeyPref *pref)
{
...
init_bindings (void)
{
  ...
  key_bindings = g_hash_table_new_full (g_str_hash, g_str_equal, g_free,
                                        (GDestroyNotify)cobiwm_key_pref_free);

  pref = g_new0 (CobiwmKeyPref, 1);
  pref->name = g_strdup ("overlay-key");
  pref->action = COBIWM_KEYBINDING_ACTION_OVERLAY_KEY;
...
static gboolean
update_binding (CobiwmKeyPref *binding,
                gchar      **strokes)
{
  ...
  for (i = 0; strokes && strokes[i]; i++)
    {
      ...
      combo = g_malloc0 (sizeof (CobiwmKeyCombo));

      if (!cobiwm_parse_accelerator (strokes[i], combo))
        {
          cobiwm_topic (COBIWM_DEBUG_KEYBINDINGS,
                      "Failed to parse new GSettings value\n");
          cobiwm_warning ("\"%s\" found in configuration database is not a valid value for keybinding \"%s\"\n",
  ...
  while (TRUE)
    {
      ...
      else if (memcmp (a->data, b->data, sizeof (CobiwmKeyCombo)) != 0)
        {
          changed = TRUE;
          break;
        }
...
update_key_binding (const char *key,
                    gchar     **strokes)
{
  CobiwmKeyPref *pref = g_hash_table_lookup (key_bindings, key);
...
const char*
cobiwm_prefs_get_workspace_name (int i)
{
  const char *name;
...
void
cobiwm_prefs_change_workspace_name (int         num,
                                  const char *name)
{
  ...
  if (g_strcmp0 (name, cobiwm_prefs_get_workspace_name (num)) == 0)
    {
      if (!name || !*name)
        cobiwm_topic (COBIWM_DEBUG_PREFS,
...
/**
 * cobiwm_prefs_get_button_layout:
 * @button_layout: (out):
 */
void
cobiwm_prefs_get_button_layout (CobiwmButtonLayout *button_layout_p)
{
  *button_layout_p = button_layout;
}

gboolean
cobiwm_prefs_get_visual_bell (void)
{
  return bell_is_visible;
}

gboolean
cobiwm_prefs_bell_is_audible (void)
{
  return bell_is_audible;
}

GDesktopVisualBellType
cobiwm_prefs_get_visual_bell_type (void)
{
  return visual_bell_type;
}

gboolean
cobiwm_prefs_add_keybinding (const char           *name,
                           GSettings            *settings,
                           ...
                           CobiwmKeyBindingFlags   flags)
{
  ...
  if (g_hash_table_lookup (key_bindings, name))
    {
      cobiwm_warning ("Trying to re-add keybinding \"%s\".\n", name);
      return FALSE;
    }
  ...
  pref->action = action;
  pref->combos = NULL;
  pref->builtin = (flags & COBIWM_KEY_BINDING_BUILTIN) != 0;
...
gboolean
cobiwm_prefs_remove_keybinding (const char *name)
{
  CobiwmKeyPref *pref;
  guint        id;
  ...
  if (!pref)
    {
      cobiwm_warning ("Trying to remove non-existent keybinding \"%s\".\n", name);
      return FALSE;
    }

  if (pref->builtin)
    {
      cobiwm_warning ("Trying to remove builtin keybinding \"%s\".\n", name);
      return FALSE;
    }
...
GList *
cobiwm_prefs_get_keybindings (void)
{
  return g_hash_table_get_values (key_bindings);
}

void
cobiwm_prefs_get_overlay_binding (CobiwmKeyCombo *combo)
{
  *combo = overlay_key_combo;
}

const char *
cobiwm_prefs_get_iso_next_group_option (void)
{
  return iso_next_group_option;
}

GDesktopTitlebarAction
cobiwm_prefs_get_action_double_click_titlebar (void)
{
  return action_double_click_titlebar;
}

GDesktopTitlebarAction
cobiwm_prefs_get_action_middle_click_titlebar (void)
{
  return action_middle_click_titlebar;
}

GDesktopTitlebarAction
cobiwm_prefs_get_action_right_click_titlebar (void)
{
  return action_right_click_titlebar;
}

gboolean
cobiwm_prefs_get_auto_raise (void)
{
  return auto_raise;
}

int
cobiwm_prefs_get_auto_raise_delay (void)
{
  return auto_raise_delay;
}

gboolean
cobiwm_prefs_get_focus_change_on_pointer_rest (void)
{
  return focus_change_on_pointer_rest;
}

gboolean
cobiwm_prefs_get_gnome_accessibility (void)
{
  return gnome_accessibility;
}

gboolean
cobiwm_prefs_get_gnome_animations (void)
{
  return gnome_animations;
}

gboolean
cobiwm_prefs_get_edge_tiling (void)
{
  return edge_tiling;
}

gboolean
cobiwm_prefs_get_auto_maximize (void)
{
  return auto_maximize;
}

CobiwmKeyBindingAction
cobiwm_prefs_get_keybinding_action (const char *name)
{
  CobiwmKeyPref *pref = g_hash_table_lookup (key_bindings, name);
...
gint
cobiwm_prefs_get_mouse_button_resize (void)
{
  return resize_with_right_button ? 3: 2;
}

gint
cobiwm_prefs_get_mouse_button_menu (void)
{
  return resize_with_right_button ? 2: 3;
}

gboolean
cobiwm_prefs_get_force_fullscreen (void)
{
  return force_fullscreen;
}

gboolean
cobiwm_prefs_get_workspaces_only_on_primary (void)
{
  return workspaces_only_on_primary;
}

int
cobiwm_prefs_get_draggable_border_width (void)
{
  return draggable_border_width;
}

int
cobiwm_prefs_get_drag_threshold (void)
{
  return drag_threshold;
}

void
cobiwm_prefs_set_force_fullscreen (gboolean whether)
{
  force_fullscreen = whether;
}

gboolean
cobiwm_prefs_get_ignore_request_hide_titlebar (void)
{
  return ignore_request_hide_titlebar;
}

void
cobiwm_prefs_set_ignore_request_hide_titlebar (gboolean whether)
{
