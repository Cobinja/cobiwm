 ...
 *
 * To smoothly restart Cobiwm, we want to keep the composite
 ...
 * overlay window until Cobiwm picks it back up.
 */
      ...
      char **argv)
{
  ...
  XSetSelectionOwner (display,
		      XInternAtom (display, "_COBIWM_RESTART_HELPER", False),
		      selection_window,
		      CurrentTime);

  /* Cobiwm looks for an (arbitrary) line printed to stdout to know that
   * we have started and have a reference to the COW. XSync() so that
   * everything is set on the X server before Cobiwm starts restarting.
   */
  XSync (display, False);
  ...
  while (True)
    {
      ...
      /* Cobiwm restarted and unset the selection to indicate that
