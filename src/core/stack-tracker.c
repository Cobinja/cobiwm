...
typedef union _CobiwmStackOp CobiwmStackOp;
...
/* CobiwmStackOp represents a "stacking operation" - a change to
 ...
 */
union _CobiwmStackOp
{
  struct {
    CobiwmStackOpType type;
    gulong serial;
    guint64 window;
  } any;
  struct {
...
get_window_desc (CobiwmStackTracker *tracker,
                 guint64           window)
{
  return cobiwm_display_describe_stack_id (tracker->screen->display, window);
}

static void
cobiwm_stack_op_dump (CobiwmStackTracker *tracker,
                    CobiwmStackOp      *op,
		    const char       *prefix,
		    const char       *suffix)
{
  ...
  switch (op->any.type)
    {
    case STACK_OP_ADD:
      cobiwm_topic (COBIWM_DEBUG_STACK, "%sADD(%s; %ld)%s",
		  prefix, window_desc, op->any.serial, suffix);
      break;
    case STACK_OP_REMOVE:
      cobiwm_topic (COBIWM_DEBUG_STACK, "%sREMOVE(%s; %ld)%s",
		  prefix, window_desc, op->any.serial, suffix);
      break;
    case STACK_OP_RAISE_ABOVE:
      {
        cobiwm_topic (COBIWM_DEBUG_STACK, "%sRAISE_ABOVE(%s, %s; %ld)%s",
                    prefix,
                    window_desc,
    ...
    case STACK_OP_LOWER_BELOW:
      {
        cobiwm_topic (COBIWM_DEBUG_STACK, "%sLOWER_BELOW(%s, %s; %ld)%s",
                    prefix,
                    window_desc,
...
static void
stack_dump (CobiwmStackTracker *tracker,
            GArray           *stack)
{
  guint i;
  ...
  for (i = 0; i < stack->len; i++)
    {
      ...
      cobiwm_topic (COBIWM_DEBUG_STACK, "  %s", get_window_desc (tracker, window));
    }
...
static void
cobiwm_stack_tracker_dump (CobiwmStackTracker *tracker)
{
  GList *l;

  cobiwm_topic (COBIWM_DEBUG_STACK, "CobiwmStackTracker state (screen=%d)\n", tracker->screen->number);
  cobiwm_push_no_msg_prefix ();
  cobiwm_topic (COBIWM_DEBUG_STACK, "  xserver_serial: %ld\n", tracker->xserver_serial);
  cobiwm_topic (COBIWM_DEBUG_STACK, "  verified_stack: ");
  stack_dump (tracker, tracker->verified_stack);
  cobiwm_topic (COBIWM_DEBUG_STACK, "  unverified_predictions: [");
  for (l = tracker->unverified_predictions->head; l; l = l->next)
    {
      CobiwmStackOp *op = l->data;
      cobiwm_stack_op_dump (tracker, op, "", l->next ? ", " : "");
    }
  cobiwm_topic (COBIWM_DEBUG_STACK, "]\n");
...
static void
cobiwm_stack_op_free (CobiwmStackOp *op)
{
  g_slice_free (CobiwmStackOp, op);
}
                   ...
                   ApplyFlags apply_flags)
{
  ...
  gboolean can_restack_this_window =
    (apply_flags & NO_RESTACK_X_WINDOWS) == 0  || !COBIWM_STACK_ID_IS_X11 (window);
      ...
      if ((apply_flags & IGNORE_NOOP_X_RESTACK) != 0)
        {
          ...
          for (i = old_pos + 1; i <= above_pos; i++)
            if (COBIWM_STACK_ID_IS_X11 (g_array_index (stack, guint64, i)))
              found_x_window = TRUE;
      ...
      for (i = old_pos; i < above_pos; i++)
        {
          if (!can_restack_this_window &&
              COBIWM_STACK_ID_IS_X11 (g_array_index (stack, guint64, i + 1)))
            break;
...
/* Returns TRUE if stack was changed */
static gboolean
cobiwm_stack_op_apply (CobiwmStackTracker *tracker,
                     CobiwmStackOp      *op,
		     GArray           *stack,
                     ApplyFlags        apply_flags)
{
  switch (op->any.type)
    ...
    case STACK_OP_ADD:
      {
        if (COBIWM_STACK_ID_IS_X11 (op->add.window) &&
    ...
    case STACK_OP_REMOVE:
      {
        if (COBIWM_STACK_ID_IS_X11 (op->remove.window) &&
            (apply_flags & NO_RESTACK_X_WINDOWS) != 0)
          return FALSE;
...
static void
query_xserver_stack (CobiwmStackTracker *tracker)
{
  CobiwmScreen *screen = tracker->screen;
...
CobiwmStackTracker *
cobiwm_stack_tracker_new (CobiwmScreen *screen)
{
  CobiwmStackTracker *tracker;

  tracker = g_new0 (CobiwmStackTracker, 1);
  ...
  cobiwm_stack_tracker_dump (tracker);

  return tracker;
}

void
cobiwm_stack_tracker_free (CobiwmStackTracker *tracker)
{
  if (tracker->sync_stack_later)
    cobiwm_later_remove (tracker->sync_stack_later);

  g_array_free (tracker->verified_stack, TRUE);
  if (tracker->predicted_stack)
    g_array_free (tracker->predicted_stack, TRUE);

  g_queue_foreach (tracker->unverified_predictions, (GFunc)cobiwm_stack_op_free, NULL);
...
static void
stack_tracker_apply_prediction (CobiwmStackTracker *tracker,
			        CobiwmStackOp      *op)
{
  ...
  if (op->any.serial == 0 &&
    ...
    {
      if (cobiwm_stack_op_apply (tracker, op, tracker->verified_stack, APPLY_DEFAULT))
        cobiwm_stack_tracker_queue_sync_stack (tracker);

      free_at_end = TRUE;
    }
  else
    {
      cobiwm_stack_op_dump (tracker, op, "Predicting: ", "\n");
      g_queue_push_tail (tracker->unverified_predictions, op);
    }

  if (!tracker->predicted_stack ||
      cobiwm_stack_op_apply (tracker, op, tracker->predicted_stack, APPLY_DEFAULT))
    cobiwm_stack_tracker_queue_sync_stack (tracker);

  if (free_at_end)
    cobiwm_stack_op_free (op);

  cobiwm_stack_tracker_dump (tracker);
}

void
cobiwm_stack_tracker_record_add (CobiwmStackTracker *tracker,
                               guint64           window,
			       gulong            serial)
{
  CobiwmStackOp *op = g_slice_new (CobiwmStackOp);
...
void
cobiwm_stack_tracker_record_remove (CobiwmStackTracker *tracker,
                                  guint64           window,
				  gulong            serial)
{
  CobiwmStackOp *op = g_slice_new (CobiwmStackOp);
...
static void
cobiwm_stack_tracker_record_raise_above (CobiwmStackTracker *tracker,
                                       guint64           window,
                                       guint64           sibling,
				       gulong            serial)
{
...
static void
cobiwm_stack_tracker_record_lower_below (CobiwmStackTracker *tracker,
                                       guint64           window,
                                       guint64           sibling,
				       gulong            serial)
{
...
static void
stack_tracker_event_received (CobiwmStackTracker *tracker,
			      CobiwmStackOp      *op)
{
  ...
  cobiwm_stack_op_dump (tracker, op, "Stack op event received: ", "\n");
  ...
  while (tracker->unverified_predictions->head)
    {
      CobiwmStackOp *queued_op  = tracker->unverified_predictions->head->data;

      if (queued_op->any.serial >= op->any.serial)
	break;

      cobiwm_stack_op_apply (tracker, queued_op, tracker->verified_stack,
                           NO_RESTACK_X_WINDOWS);

      g_queue_pop_head (tracker->unverified_predictions);
      cobiwm_stack_op_free (queued_op);
      need_sync = TRUE;
    }
   ...
   */
  if (cobiwm_stack_op_apply (tracker, op, tracker->verified_stack,
...
void
cobiwm_stack_tracker_create_event (CobiwmStackTracker    *tracker,
...
void
cobiwm_stack_tracker_destroy_event (CobiwmStackTracker    *tracker,
...
void
cobiwm_stack_tracker_reparent_event (CobiwmStackTracker    *tracker,
...
void
cobiwm_stack_tracker_configure_event (CobiwmStackTracker    *tracker,
...
/**
 * cobiwm_stack_tracker_get_stack:
 * @tracker: a #CobiwmStackTracker
 ...
 * hidden windows, etc. Some of these will correspond to CobiwmWindow
 ...
 */
void
cobiwm_stack_tracker_get_stack (CobiwmStackTracker *tracker,
                              guint64         **windows,
			      int              *n_windows)
{
  GArray *stack;
...
/**
 * cobiwm_stack_tracker_sync_stack:
 * @tracker: a #CobiwmStackTracker
 *
 * Informs the compositor of the current stacking order of windows,
 * based on the predicted view maintained by the #CobiwmStackTracker.
 */
void
cobiwm_stack_tracker_sync_stack (CobiwmStackTracker *tracker)
{
  guint64 *windows;
  GList *cobiwm_windows;
  int n_windows;
  int i;
  ...
  cobiwm_stack_tracker_get_stack (tracker, &windows, &n_windows);
  ...
  for (i = 0; i < n_windows; i++)
    {
      ...
      if (COBIWM_STACK_ID_IS_X11 (window))
        {
          CobiwmWindow *cobiwm_window =
            cobiwm_display_lookup_x_window (tracker->screen->display, (Window)window);

          /* When mapping back from xwindow to CobiwmWindow we have to be a bit careful;
           ...
           */
          if (cobiwm_window &&
              ((Window)window == cobiwm_window->xwindow ||
               (cobiwm_window->frame && (Window)window == cobiwm_window->frame->xwindow)))
            cobiwm_windows = g_list_prepend (cobiwm_windows, cobiwm_window);
        }
      else
        cobiwm_windows = g_list_prepend (cobiwm_windows,
                                       cobiwm_display_lookup_stamp (tracker->screen->display, window));
    }

  cobiwm_compositor_sync_stack (tracker->screen->display->compositor,
                              cobiwm_windows);
  g_list_free (cobiwm_windows);

  cobiwm_screen_restacked (tracker->screen);
}

static gboolean
stack_tracker_sync_stack_later (gpointer data)
{
  cobiwm_stack_tracker_sync_stack (data);

  return FALSE;
}

/**
 * cobiwm_stack_tracker_queue_sync_stack:
 * @tracker: a #CobiwmStackTracker
 *
 * Queue informing the compositor of the new stacking order before the
 * next redraw. (See cobiwm_stack_tracker_sync_stack()). This is called
 ...
 * the windows that have a corresponding CobiwmWindow) will change without
 * any change to the stacking order of the X windows, if we are creating
 * or destroying CobiwmWindows.
 */
void
cobiwm_stack_tracker_queue_sync_stack (CobiwmStackTracker *tracker)
{
  if (tracker->sync_stack_later == 0)
    {
      tracker->sync_stack_later = cobiwm_later_add (COBIWM_LATER_SYNC_STACK,
...
static Window
find_x11_sibling_downwards (CobiwmStackTracker *tracker,
                            guint64           sibling)
{
  guint64 *windows;
  int n_windows;
  ...
  if (COBIWM_STACK_ID_IS_X11 (sibling))
    return (Window)sibling;

  cobiwm_stack_tracker_get_stack (tracker,
                                &windows, &n_windows);
  ...
  for (; i >= 0; i--)
    {
      if (COBIWM_STACK_ID_IS_X11 (windows[i]))
...
static Window
find_x11_sibling_upwards (CobiwmStackTracker *tracker,
                          guint64           sibling)
{
  guint64 *windows;
  int n_windows;
...
static void
cobiwm_stack_tracker_lower_below (CobiwmStackTracker *tracker,
                                guint64           window,
                                guint64           sibling)
{
  gulong serial = 0;
      ...
      if (changes.sibling != find_x11_sibling_upwards (tracker, window))
        {
          ...
          cobiwm_error_trap_push (tracker->screen->display);
          ...
          cobiwm_error_trap_pop (tracker->screen->display);
        }
    }

  cobiwm_stack_tracker_record_lower_below (tracker,
                                         window, sibling,
                                         serial);
}

static void
cobiwm_stack_tracker_raise_above (CobiwmStackTracker *tracker,
                                guint64           window,
                                guint64           sibling)
{
  gulong serial = 0;
  ...
  cobiwm_stack_tracker_record_raise_above (tracker, window,
                                         sibling, serial);
}

void
cobiwm_stack_tracker_lower (CobiwmStackTracker *tracker,
                          guint64           window)
{
  cobiwm_stack_tracker_raise_above (tracker, window, None);
}

void
cobiwm_stack_tracker_restack_managed (CobiwmStackTracker *tracker,
                                    ...
                                    int               n_managed)
{
  ...
  for (old_pos = n_windows - 1; old_pos >= 0; old_pos--)
    {
      CobiwmWindow *old_window = cobiwm_display_lookup_stack_id (tracker->screen->display, windows[old_pos]);
  ...
  if (managed[new_pos] != windows[old_pos])
    {
      ...
      cobiwm_stack_tracker_raise_above (tracker, managed[new_pos], windows[old_pos]);
      cobiwm_stack_tracker_get_stack (tracker, &windows, &n_windows);
  ...
  while (old_pos >= 0 && new_pos >= 0)
    {
      ...
      cobiwm_stack_tracker_lower_below (tracker, managed[new_pos], managed[new_pos + 1]);
      cobiwm_stack_tracker_get_stack (tracker, &windows, &n_windows);
      /* Moving managed[new_pos] above windows[old_pos] moves the window at old_pos down by one,
...
void
cobiwm_stack_tracker_restack_at_bottom (CobiwmStackTracker *tracker,
  ...
  for (pos = 0; pos < n_new_order; pos++)
    {
      if (pos >= n_windows || windows[pos] != new_order[pos])
        {
          if (pos == 0)
            cobiwm_stack_tracker_lower (tracker, new_order[pos]);
          else
            cobiwm_stack_tracker_raise_above (tracker, new_order[pos], new_order[pos - 1]);
