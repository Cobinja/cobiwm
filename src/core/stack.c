...
#define WINDOW_HAS_TRANSIENT_TYPE(w)                    \
          (w->type == COBIWM_WINDOW_DIALOG ||             \
	   w->type == COBIWM_WINDOW_MODAL_DIALOG ||       \
           w->type == COBIWM_WINDOW_TOOLBAR ||            \
           w->type == COBIWM_WINDOW_MENU ||               \
           w->type == COBIWM_WINDOW_UTILITY)
...
static void stack_sync_to_xserver (CobiwmStack *stack);
static void cobiwm_window_set_stack_position_no_sync (CobiwmWindow *window,
                                                    int         position);
static void stack_do_window_deletions (CobiwmStack *stack);
static void stack_do_window_additions (CobiwmStack *stack);
static void stack_do_relayer          (CobiwmStack *stack);
static void stack_do_constrain        (CobiwmStack *stack);
static void stack_do_resort           (CobiwmStack *stack);

static void stack_ensure_sorted (CobiwmStack *stack);

CobiwmStack*
cobiwm_stack_new (CobiwmScreen *screen)
{
  CobiwmStack *stack;

  stack = g_new (CobiwmStack, 1);
...
void
cobiwm_stack_free (CobiwmStack *stack)
{
...
void
cobiwm_stack_add (CobiwmStack  *stack,
                CobiwmWindow *window)
{
  g_return_if_fail (!window->override_redirect);

  cobiwm_topic (COBIWM_DEBUG_STACK, "Adding window %s to the stack\n", window->desc);

  if (window->stack_position >= 0)
    cobiwm_bug ("Window %s had stack position already\n", window->desc);
  ...
  cobiwm_stack_update_window_tile_matches (stack, window->screen->active_workspace);
}

void
cobiwm_stack_remove (CobiwmStack  *stack,
                   CobiwmWindow *window)
{
  cobiwm_topic (COBIWM_DEBUG_STACK, "Removing window %s from the stack\n", window->desc);

  if (window->stack_position < 0)
    cobiwm_bug ("Window %s removed from stack but had no stack position\n",
              window->desc);

  /* Set window to top position, so removing it will not leave gaps
   * in the set of positions
   */
  cobiwm_window_set_stack_position_no_sync (window,
                                          stack->n_positions - 1);
...
void
cobiwm_stack_update_layer (CobiwmStack  *stack,
                         CobiwmWindow *window)
{
...
void
cobiwm_stack_update_transient (CobiwmStack  *stack,
                             CobiwmWindow *window)
{
...
void
cobiwm_stack_raise (CobiwmStack  *stack,
                  CobiwmWindow *window)
{
  GList *l;
  ...
  workspace = cobiwm_window_get_workspace (window);
  for (l = stack->sorted; l; l = l->next)
    {
      CobiwmWindow *w = (CobiwmWindow *) l->data;
      if (cobiwm_window_located_on_workspace (w, workspace) &&
  ...
  cobiwm_window_set_stack_position_no_sync (window, max_stack_position);
...
void
cobiwm_stack_lower (CobiwmStack  *stack,
                  CobiwmWindow *window)
{
  GList *l;
  ...
  cobiwm_window_set_stack_position_no_sync (window, min_stack_position);
...
void
cobiwm_stack_freeze (CobiwmStack *stack)
{
  stack->freeze_count += 1;
}

void
cobiwm_stack_thaw (CobiwmStack *stack)
{
  g_return_if_fail (stack->freeze_count > 0);

  stack->freeze_count -= 1;
  stack_sync_to_xserver (stack);
  cobiwm_stack_update_window_tile_matches (stack, NULL);
}

void
cobiwm_stack_update_window_tile_matches (CobiwmStack     *stack,
                                       CobiwmWorkspace *workspace)
{
  GList *windows, *tmp;
  ...
  windows = cobiwm_stack_list_windows (stack, workspace);
  tmp = windows;
  while (tmp)
    {
      cobiwm_window_compute_tile_match ((CobiwmWindow *) tmp->data);
      tmp = tmp->next;
    }
...
static gboolean
is_focused_foreach (CobiwmWindow *window,
                    void       *data)
{
  if (window->has_focus)
    {
...
static gboolean
windows_on_different_monitor (CobiwmWindow *a,
                              CobiwmWindow *b)
{
...
static CobiwmStackLayer
get_standalone_layer (CobiwmWindow *window)
{
  ...
  switch (window->type)
    ...
    case COBIWM_WINDOW_DESKTOP:
      layer = COBIWM_LAYER_DESKTOP;
      break;

    case COBIWM_WINDOW_DOCK:
      /* still experimenting here */
      if (window->wm_state_below)
        layer = COBIWM_LAYER_BOTTOM;
      else
        layer = COBIWM_LAYER_DOCK;
      break;
    ...
    case COBIWM_WINDOW_OVERRIDE_OTHER:
      layer = COBIWM_LAYER_OVERRIDE_REDIRECT;
      break;
    default:
      ...
      if (window->wm_state_below)
        ...
        layer = COBIWM_LAYER_FULLSCREEN;
      else if (window->wm_state_above && !COBIWM_WINDOW_MAXIMIZED (window))
        layer = COBIWM_LAYER_TOP;
      else
        layer = COBIWM_LAYER_NORMAL;
      break;
    }
 ...
 */
static CobiwmStackLayer
get_maximum_layer_in_group (CobiwmWindow *window)
{
  GSList *members;
  CobiwmGroup *group;
  GSList *tmp;
  CobiwmStackLayer max;
  CobiwmStackLayer layer;

  max = COBIWM_LAYER_DESKTOP;

  group = cobiwm_window_get_group (window);

  if (group != NULL)
    members = cobiwm_group_list_windows (group);
  else
    members = NULL;
...
static void
compute_layer (CobiwmWindow *window)
{
  CobiwmStackLayer old_layer = window->layer;
   ...
   * fullscreen.
   */
  if (window->layer != COBIWM_LAYER_DESKTOP &&
  ...
  cobiwm_topic (COBIWM_DEBUG_STACK, "Window %s on layer %u type = %u has_focus = %d\n",
              window->desc, window->layer,
              window->type, window->has_focus);

  if (window->layer != old_layer &&
      (old_layer == COBIWM_LAYER_FULLSCREEN || window->layer == COBIWM_LAYER_FULLSCREEN))
...
compare_window_position (void *a,
                         void *b)
{
  CobiwmWindow *window_a = a;
  CobiwmWindow *window_b = b;
  ...
  while (tmp != NULL)
    {
      ...
      if (!WINDOW_IN_STACK (w))
        {
          cobiwm_topic (COBIWM_DEBUG_STACK, "Window %s not in the stack, not constraining it\n",
                      w->desc);
          tmp = tmp->next;
          continue;
        }
      ...
      if (WINDOW_TRANSIENT_FOR_WHOLE_GROUP (w))
        {
          ...
          group = cobiwm_window_get_group (w);

          if (group != NULL)
            group_windows = cobiwm_group_list_windows (group);
          else
            group_windows = NULL;
          ...
          while (tmp2 != NULL)
            {
              CobiwmWindow *group_window = tmp2->data;
...
#if 0
              /* old way of doing it */
              if (!(cobiwm_window_is_ancestor_of_transient (w, group_window)) &&
...
#endif
                {
                  cobiwm_topic (COBIWM_DEBUG_STACK, "Constraining %s above %s as it's transient for its group\n",
      ...
      else if (w->transient_for != NULL)
        {
          ...
          if (parent && WINDOW_IN_STACK (parent))
            {
              cobiwm_topic (COBIWM_DEBUG_STACK, "Constraining %s above %s due to transiency\n",
                          w->desc, parent->desc);
...
static void
ensure_above (CobiwmWindow *above,
              CobiwmWindow *below)
{
  ...
  if (above->stack_position < below->stack_position)
    {
      ...
      cobiwm_window_set_stack_position_no_sync (above, below->stack_position);
      g_assert (below->stack_position + 1 == above->stack_position);
    }
  cobiwm_topic (COBIWM_DEBUG_STACK, "%s above at %d > %s below at %d\n",
 ...
 */
static void
stack_do_window_deletions (CobiwmStack *stack)
{
...
static void
stack_do_window_additions (CobiwmStack *stack)
{
  GList *tmp;
  gint i, n_added;
 ...
 */
static void
stack_do_relayer (CobiwmStack *stack)
{
  GList *tmp;
 ...
 */
static void
stack_do_constrain (CobiwmStack *stack)
{
 ...
 */
static void
stack_do_resort (CobiwmStack *stack)
{
 ...
 */
static void
stack_ensure_sorted (CobiwmStack *stack)
{
 ...
 * with CobiwmStackTracker it should be possible to do a simpler and better
 * job of computing the minimal set of stacking requests needed.
 */
static void
stack_sync_to_xserver (CobiwmStack *stack)
{
  GArray *x11_stacked;
  ...
  cobiwm_topic (COBIWM_DEBUG_STACK, "Syncing window stack to server\n");
  ...
  cobiwm_topic (COBIWM_DEBUG_STACK, "Bottom to top: ");
  ...
  for (tmp = g_list_last(stack->sorted); tmp != NULL; tmp = tmp->prev)
    {
      ...
      cobiwm_topic (COBIWM_DEBUG_STACK, "%u:%d - %s ",
	...
	top_level_window = w->xwindow;
      ...
      if (w->client_type == COBIWM_WINDOW_CLIENT_TYPE_X11)
  ...
  cobiwm_topic (COBIWM_DEBUG_STACK, "Restacking %u windows\n",
              all_root_children_stacked->len);

  cobiwm_stack_tracker_restack_managed (stack->screen->stack_tracker,
                                      (guint64 *)all_root_children_stacked->data,
                                      all_root_children_stacked->len);
  cobiwm_stack_tracker_restack_at_bottom (stack->screen->stack_tracker,
...
CobiwmWindow*
cobiwm_stack_get_top (CobiwmStack *stack)
{
  stack_ensure_sorted (stack);
...
CobiwmWindow*
cobiwm_stack_get_bottom (CobiwmStack  *stack)
{
  GList *link;
...
CobiwmWindow*
cobiwm_stack_get_above (CobiwmStack      *stack,
                      CobiwmWindow     *window,
...
CobiwmWindow*
cobiwm_stack_get_below (CobiwmStack      *stack,
                      CobiwmWindow     *window,
                      gboolean        only_within_layer)
{
  GList *link;
...
static gboolean
window_contains_point (CobiwmWindow *window,
                       int         root_x,
                       int         root_y)
{
...
static CobiwmWindow*
get_default_focus_window (CobiwmStack     *stack,
                          CobiwmWorkspace *workspace,
  ...
  for (l = stack->sorted; l != NULL; l = l->next)
    {
      ...
      if (!cobiwm_window_should_be_showing (window))
        continue;
...
CobiwmWindow*
cobiwm_stack_get_default_focus_window_at_point (CobiwmStack     *stack,
                                              CobiwmWorkspace *workspace,
                                              CobiwmWindow    *not_this_one,
                                              int            root_x,
                                              int            root_y)
...
CobiwmWindow*
cobiwm_stack_get_default_focus_window (CobiwmStack     *stack,
                                     CobiwmWorkspace *workspace,
                                     CobiwmWindow    *not_this_one)
{
  return get_default_focus_window (stack, workspace, not_this_one,
...
GList*
cobiwm_stack_list_windows (CobiwmStack     *stack,
                         CobiwmWorkspace *workspace)
{
  ...
  while (link)
    {
      CobiwmWindow *window = link->data;

      if (window &&
          (workspace == NULL || cobiwm_window_located_on_workspace (window, workspace)))
        {
...
int
cobiwm_stack_windows_cmp  (CobiwmStack  *stack,
                         CobiwmWindow *window_a,
                         CobiwmWindow *window_b)
{
...
GList*
cobiwm_stack_get_positions (CobiwmStack *stack)
{
  GList *tmp;
...
void
cobiwm_stack_set_positions (CobiwmStack *stack,
                          GList     *windows)
{
  int i;
  GList *tmp;
  ...
  if (!lists_contain_same_windows (windows, stack->sorted))
    {
      cobiwm_warning ("This list of windows has somehow changed; not resetting "
...
void
cobiwm_window_set_stack_position_no_sync (CobiwmWindow *window,
                                        int         position)
{
  int low, high, delta;
  GList *tmp;
  ...
  if (position == window->stack_position)
    {
      cobiwm_topic (COBIWM_DEBUG_STACK, "Window %s already has position %d\n",
...
void
cobiwm_window_set_stack_position (CobiwmWindow *window,
                                int         position)
{
  cobiwm_window_set_stack_position_no_sync (window, position);
