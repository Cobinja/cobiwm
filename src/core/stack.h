...
#ifndef COBIWM_STACK_H
#define COBIWM_STACK_H
 ...
 *
 * (This is only used as a field within a CobiwmScreen; we treat it as a separate
 * class for simplicity.)
 */
struct _CobiwmStack
{
  /** The CobiwmScreen containing this stack. */
  CobiwmScreen *screen;

  /**
   * A sequence of all the Windows (X handles, not CobiwmWindows) of the windows
   * we manage, sorted in order.  Suitable to be passed into _NET_CLIENT_LIST.
   */
  GArray *xwindows;

  /** The CobiwmWindows of the windows we manage, sorted in order. */
  GList *sorted;

  /**
   * CobiwmWindows waiting to be added to the "sorted" and "windows" list, after
   * being added by cobiwm_stack_add() and before being assimilated by
   * stack_ensure_sorted().
   *
  ...
  /**
   * Windows (X handles, not CobiwmWindows) waiting to be removed from the
   * "windows" list, after being removed by cobiwm_stack_remove() and before
   ...
   * again.  You may freeze the stack with cobiwm_stack_freeze() and thaw it
   * with cobiwm_stack_thaw().
   */
  int freeze_count;
...
/**
 * cobiwm_stack_new:
 * @screen: The CobiwmScreen which will be the parent of this stack.
 *
 * Creates and initialises a CobiwmStack.
 *
 * Returns: The new screen.
 */
CobiwmStack *cobiwm_stack_new       (CobiwmScreen     *screen);

/**
 * cobiwm_stack_free:
 * @stack: The stack to destroy.
 *
 * Destroys and frees a CobiwmStack.
 */
void       cobiwm_stack_free      (CobiwmStack      *stack);
 ...
 */
void       cobiwm_stack_add       (CobiwmStack      *stack,
                                 CobiwmWindow     *window);
 ...
 */
void       cobiwm_stack_remove    (CobiwmStack      *stack,
                                 CobiwmWindow     *window);
/**
 * cobiwm_stack_update_layer:
 ...
 *
 */
void       cobiwm_stack_update_layer    (CobiwmStack      *stack,
                                       CobiwmWindow     *window);

/**
 * cobiwm_stack_update_transient:
 * @stack: The stack to recalculate
 * @window: Dummy parameter
 *
 ...
 */
void       cobiwm_stack_update_transient (CobiwmStack     *stack,
                                        CobiwmWindow    *window);
 ...
 */
void       cobiwm_stack_raise     (CobiwmStack      *stack,
                                 CobiwmWindow     *window);
/**
 * cobiwm_stack_lower:
 * @stack: The stack to modify.
 ...
 */
void       cobiwm_stack_lower     (CobiwmStack      *stack,
                                 CobiwmWindow     *window);
 ...
 *
 * Prevent syncing to server until the next call of cobiwm_stack_thaw(),
 * so that we can carry out multiple operations in one go without having
 * everything halfway reflected on the X server.
 *
 * (Calls to cobiwm_stack_freeze() nest, so that multiple calls to
 * cobiwm_stack_freeze will require multiple calls to cobiwm_stack_thaw().)
 */
void       cobiwm_stack_freeze    (CobiwmStack      *stack);

/**
 * cobiwm_stack_thaw:
 * @stack: The stack to thaw.
 *
 * Undoes a cobiwm_stack_freeze(), and processes anything which has become
 * necessary during the freeze.  It is an error to call this function if
 * the stack has not been frozen.
 */
void       cobiwm_stack_thaw      (CobiwmStack      *stack);

/**
 * cobiwm_stack_get_top:
 ...
 */
CobiwmWindow* cobiwm_stack_get_top    (CobiwmStack  *stack);

/**
 * cobiwm_stack_get_bottom:
 ...
 */
CobiwmWindow* cobiwm_stack_get_bottom (CobiwmStack  *stack);

/**
 * cobiwm_stack_get_above:
 * @stack: The stack to search.
 ...
 */
CobiwmWindow* cobiwm_stack_get_above  (CobiwmStack  *stack,
                                   CobiwmWindow *window,
                                   gboolean    only_within_layer);

/**
 * cobiwm_stack_get_below:
 * @stack: The stack to search.
 ...
 */
CobiwmWindow* cobiwm_stack_get_below  (CobiwmStack  *stack,
                                   CobiwmWindow *window,
                                   gboolean    only_within_layer);

/**
 * cobiwm_stack_get_default_focus_window:
 * @stack: The stack to search.
 ...
 * Returns: The window matching all these constraints or %NULL if none does.
  */
CobiwmWindow* cobiwm_stack_get_default_focus_window          (CobiwmStack     *stack,
                                                          CobiwmWorkspace *workspace,
                                                          CobiwmWindow    *not_this_one);

/**
 * cobiwm_stack_get_default_focus_window_at_point:
 * @stack: The stack to search.
 * @workspace: %NULL to search all workspaces; otherwise only windows
 *             from that workspace will be returned.
 * @not_this_one: Window to ignore because it's being unfocussed or
 ...
 * Returns: The window matching all these constraints or %NULL if none does.
 */
CobiwmWindow* cobiwm_stack_get_default_focus_window_at_point (CobiwmStack     *stack,
                                                          CobiwmWorkspace *workspace,
                                                          CobiwmWindow    *not_this_one,
                                                          int            root_x,
                                                          int            root_y);
 ...
 */
GList*      cobiwm_stack_list_windows (CobiwmStack *stack,
                                     CobiwmWorkspace *workspace);

/**
 * cobiwm_stack_windows_cmp:
 ...
 */
int         cobiwm_stack_windows_cmp  (CobiwmStack  *stack,
                                     CobiwmWindow *window_a,
                                     CobiwmWindow *window_b);

/**
 * cobiwm_window_set_stack_position:
 ...
 */
void cobiwm_window_set_stack_position (CobiwmWindow *window,
                                     int         position);

/**
 * cobiwm_stack_get_positions:
 * @stack: The stack to examine.
 *
 ...
 *          Pass this to cobiwm_stack_set_positions() later if you want to restore
 *          the state to where it was when you called this function.
 */
GList* cobiwm_stack_get_positions (CobiwmStack *stack);

/**
 * cobiwm_stack_set_positions:
 * @stack:  The stack to roll back.
 * @windows:  The list returned from cobiwm_stack_get_positions().
 *
 * Rolls back a transaction, given the list returned from
 * cobiwm_stack_get_positions().
 *
 */
void   cobiwm_stack_set_positions (CobiwmStack *stack,
                                 GList     *windows);

void cobiwm_stack_update_window_tile_matches (CobiwmStack     *stack,
                                            CobiwmWorkspace *workspace);
#endif
