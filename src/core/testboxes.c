...
/* Cobiwm box operation testing program */
...
static void
get_random_rect (CobiwmRectangle *rect)
{
  rect->x = rand () % 1600;
  rect->y = rand () % 1200;
...
static CobiwmRectangle*
new_cobiwm_rect (int x, int y, int width, int height)
{
  CobiwmRectangle* temporary;
  temporary = g_new (CobiwmRectangle, 1);
  temporary->x = x;
  temporary->y = y;
...
static CobiwmStrut*
new_cobiwm_strut (int x, int y, int width, int height, int side)
{
  CobiwmStrut* temporary;
  temporary = g_new (CobiwmStrut, 1);
  temporary->rect = cobiwm_rect(x, y, width, height);
  temporary->side = side;
...
{
  CobiwmEdge* temporary;
  temporary = g_new (CobiwmEdge, 1);
  temporary->rect.x = x;
  temporary->rect.y = y;
  temporary->rect.width  = width;
  temporary->rect.height = height;
  temporary->side_type = side_type;
  temporary->edge_type = COBIWM_EDGE_SCREEN;
...
new_monitor_edge (int x, int y, int width, int height, int side_type)
{
  ...
  temporary->rect.height = height;
  temporary->side_type = side_type;
  temporary->edge_type = COBIWM_EDGE_MONITOR;
...
test_area (void)
{
  CobiwmRectangle temp;
  int i;
  for (i = 0; i < NUM_RANDOM_RUNS; i++)
    {
      get_random_rect (&temp);
      g_assert (cobiwm_rectangle_area (&temp) == temp.width * temp.height);
    }

  temp = cobiwm_rect (0, 0, 5, 7);
  g_assert (cobiwm_rectangle_area (&temp) == 35);
...
test_intersect (void)
{
  CobiwmRectangle a = {100, 200,  50,  40};
  CobiwmRectangle b = {  0,  50, 110, 152};
  CobiwmRectangle c = {  0,   0,  10,  10};
  CobiwmRectangle d = {100, 100,  50,  50};
  CobiwmRectangle b_intersect_d = {100, 100, 10, 50};
  CobiwmRectangle temp;
  CobiwmRectangle temp2;

  cobiwm_rectangle_intersect (&a, &b, &temp);
  temp2 = cobiwm_rect (100, 200, 10, 2);
  g_assert (cobiwm_rectangle_equal (&temp, &temp2));
  g_assert (cobiwm_rectangle_area (&temp) == 20);

  cobiwm_rectangle_intersect (&a, &c, &temp);
  g_assert (cobiwm_rectangle_area (&temp) == 0);

  cobiwm_rectangle_intersect (&a, &d, &temp);
  g_assert (cobiwm_rectangle_area (&temp) == 0);

  cobiwm_rectangle_intersect (&b, &d, &b);
  g_assert (cobiwm_rectangle_equal (&b, &b_intersect_d));
...
test_equal (void)
{
  CobiwmRectangle a = {10, 12, 4, 18};
  CobiwmRectangle b = a;
  CobiwmRectangle c = {10, 12, 4, 19};
  CobiwmRectangle d = {10, 12, 7, 18};
  CobiwmRectangle e = {10, 62, 4, 18};
  CobiwmRectangle f = {27, 12, 4, 18};

  g_assert ( cobiwm_rectangle_equal (&a, &b));
  g_assert (!cobiwm_rectangle_equal (&a, &c));
  g_assert (!cobiwm_rectangle_equal (&a, &d));
  g_assert (!cobiwm_rectangle_equal (&a, &e));
  g_assert (!cobiwm_rectangle_equal (&a, &f));
...
{
  CobiwmRectangle temp1, temp2;
  int i;
  for (i = 0; i < NUM_RANDOM_RUNS; i++)
    {
      get_random_rect (&temp1);
      get_random_rect (&temp2);
      g_assert (cobiwm_rectangle_overlap (&temp1, &temp2) ==
                (cobiwm_rectangle_horiz_overlap (&temp1, &temp2) &&
                 cobiwm_rectangle_vert_overlap (&temp1, &temp2)));
    }

  temp1 = cobiwm_rect ( 0, 0, 10, 10);
  temp2 = cobiwm_rect (20, 0, 10,  5);
  g_assert (!cobiwm_rectangle_overlap (&temp1, &temp2));
  g_assert (!cobiwm_rectangle_horiz_overlap (&temp1, &temp2));
  g_assert ( cobiwm_rectangle_vert_overlap (&temp1, &temp2));
...
{
  CobiwmRectangle temp1, temp2, temp3;
  int i;
  /* Four cases:
  ...
  for (i = 0; i < NUM_RANDOM_RUNS; i++)
    {
      get_random_rect (&temp1);
      get_random_rect (&temp2);
      g_assert (cobiwm_rectangle_contains_rect (&temp1, &temp2) == FALSE ||
                cobiwm_rectangle_could_fit_rect (&temp1, &temp2) == TRUE);
      g_assert (cobiwm_rectangle_contains_rect (&temp2, &temp1) == FALSE ||
                cobiwm_rectangle_could_fit_rect (&temp2, &temp1) == TRUE);
    }

  temp1 = cobiwm_rect ( 0, 0, 10, 10);
  temp2 = cobiwm_rect ( 5, 5,  5,  5);
  temp3 = cobiwm_rect ( 8, 2,  3,  7);
  g_assert ( cobiwm_rectangle_contains_rect (&temp1, &temp2));
  g_assert (!cobiwm_rectangle_contains_rect (&temp2, &temp1));
  g_assert (!cobiwm_rectangle_contains_rect (&temp1, &temp3));
  g_assert ( cobiwm_rectangle_could_fit_rect (&temp1, &temp3));
  g_assert (!cobiwm_rectangle_could_fit_rect (&temp3, &temp2));
...
get_strut_list (int which)
{
  ...
  switch (which)
    ...
    case 0:
      break;
    case 1:
      ans = g_slist_prepend (ans, new_cobiwm_strut (   0,    0, 1600,   20, COBIWM_SIDE_TOP));
      ans = g_slist_prepend (ans, new_cobiwm_strut ( 400, 1160, 1600,   40, COBIWM_SIDE_BOTTOM));
      break;
    case 2:
      ans = g_slist_prepend (ans, new_cobiwm_strut (   0,    0, 1600,   20, COBIWM_SIDE_TOP));
      ans = g_slist_prepend (ans, new_cobiwm_strut ( 800, 1100,  400,  100, COBIWM_SIDE_BOTTOM));
      ans = g_slist_prepend (ans, new_cobiwm_strut ( 300, 1150,  150,   50, COBIWM_SIDE_BOTTOM));
      break;
    case 3:
      ans = g_slist_prepend (ans, new_cobiwm_strut (   0,    0, 1600,   20, COBIWM_SIDE_TOP));
      ans = g_slist_prepend (ans, new_cobiwm_strut ( 800, 1100,  400,  100, COBIWM_SIDE_LEFT));
      ans = g_slist_prepend (ans, new_cobiwm_strut ( 300, 1150,   80,   50, COBIWM_SIDE_BOTTOM));
      ans = g_slist_prepend (ans, new_cobiwm_strut ( 700,  525,  200,  150, wc));
      break;
    case 4:
      ans = g_slist_prepend (ans, new_cobiwm_strut (   0,    0,  800, 1200, COBIWM_SIDE_LEFT));
      ans = g_slist_prepend (ans, new_cobiwm_strut ( 800,    0, 1600,   20, COBIWM_SIDE_TOP));
      break;
    case 5:
      ans = g_slist_prepend (ans, new_cobiwm_strut ( 800,    0, 1600,   20, COBIWM_SIDE_TOP));
      ans = g_slist_prepend (ans, new_cobiwm_strut (   0,    0,  800, 1200, COBIWM_SIDE_LEFT));
      ans = g_slist_prepend (ans, new_cobiwm_strut ( 800,   10,  800, 1200, COBIWM_SIDE_RIGHT));
      break;
    case 6:
      ans = g_slist_prepend (ans, new_cobiwm_strut (   0,    0, 1600,   40, COBIWM_SIDE_TOP));
      ans = g_slist_prepend (ans, new_cobiwm_strut (   0,    0, 1600,   20, COBIWM_SIDE_TOP));
      break;
    }
...
get_screen_region (int which)
{
  GList *ret;
  GSList *struts;
  CobiwmRectangle basic_rect;

  basic_rect = cobiwm_rect (0, 0, 1600, 1200);
  ret = NULL;

  struts = get_strut_list (which);
  ret = cobiwm_rectangle_get_minimal_spanning_set_for_region (&basic_rect, struts);
...
get_screen_edges (int which)
{
  ...
  struts = get_strut_list (which);
  ret = cobiwm_rectangle_find_onscreen_edges (&basic_rect, struts);
  free_strut_list (struts);
...
get_monitor_edges (int which_monitor_set, int which_strut_set)
{
  ...
  switch (which_monitor_set)
    {
    case 0:
      xins = g_list_prepend (xins, new_cobiwm_rect (  0,   0, 1600, 1200));
      break;
    case 1:
      xins = g_list_prepend (xins, new_cobiwm_rect (  0,   0,  800, 1200));
      xins = g_list_prepend (xins, new_cobiwm_rect (800,   0,  800, 1200));
      break;
    case 2:
      xins = g_list_prepend (xins, new_cobiwm_rect (  0,   0, 1600,  600));
      xins = g_list_prepend (xins, new_cobiwm_rect (  0, 600, 1600,  600));
      break;
    case 3:
      xins = g_list_prepend (xins, new_cobiwm_rect (  0,   0, 1600,  600));
      xins = g_list_prepend (xins, new_cobiwm_rect (  0, 600,  800,  600));
      xins = g_list_prepend (xins, new_cobiwm_rect (800, 600,  800,  600));
      break;
    }
  ...
  ret = cobiwm_rectangle_find_nonintersected_monitor_edges (xins, struts);

  free_strut_list (struts);
  cobiwm_rectangle_free_list_and_elements (xins);
...
test_merge_regions (void)
{
  ...
  cobiwm_rectangle_region_to_string (region, ", ", region_list);
  printf ("  Initial rectangles: %s\n", region_list);
#endif

  while (compare && compare->next)
    {
      CobiwmRectangle *a = compare->data;
      ...
      while (other)
        {
          ...
          printf ("    -- Comparing %s to %s --\n",
                  cobiwm_rectangle_to_string (a, rect1),
                  cobiwm_rectangle_to_string (b, rect2));
#endif

          /* If a contains b, just remove b */
          if (cobiwm_rectangle_contains_rect (a, b))
            {
              delete_me = other;
              num_contains++;
              num_merged++;
          ...
          else if (cobiwm_rectangle_contains_rect (a, b))
            {
              delete_me = compare;
              num_contains++;
              num_merged++;
          ...
          /* If a and b might be mergeable horizontally */
            ...
            {
              /* If a and b overlap */
              if (cobiwm_rectangle_overlap (a, b))
                {
          ...
          if (delete_me != NULL)
            {
#ifdef PRINT_DEBUG
              CobiwmRectangle *bla = delete_me->data;
              printf ("    Deleting rect %s\n",
                      cobiwm_rectangle_to_string (bla, rect1));
#endif
  ...
  cobiwm_rectangle_region_to_string (region, ", ", region_list2);
  printf ("  Final rectangles: %s\n", region_list2);
#endif

  cobiwm_rectangle_free_spanning_set (region);
  region = NULL;
...
verify_lists_are_equal (GList *code, GList *answer)
{
  ...
  while (code && answer)
    {
      CobiwmRectangle *a = code->data;
      CobiwmRectangle *b = answer->data;
  ...
  if (answer)
    {
      CobiwmRectangle *tmp = answer->data;
...
test_regions_okay (void)
{
  ...
  tmp = NULL;
  tmp = g_list_prepend (tmp, new_cobiwm_rect (0, 0, 1600, 1200));
  verify_lists_are_equal (region, tmp);
  cobiwm_rectangle_free_list_and_elements (tmp);
  ...
  tmp = NULL;
  tmp = g_list_prepend (tmp, new_cobiwm_rect (0, 20,  400, 1180));
  tmp = g_list_prepend (tmp, new_cobiwm_rect (0, 20, 1600, 1140));
  verify_lists_are_equal (region, tmp);
  cobiwm_rectangle_free_list_and_elements (tmp);
  cobiwm_rectangle_free_list_and_elements (region);
  ...
  tmp = NULL;
  tmp = g_list_prepend (tmp, new_cobiwm_rect (   0,   20,  300, 1180));
  tmp = g_list_prepend (tmp, new_cobiwm_rect ( 450,   20,  350, 1180));
  tmp = g_list_prepend (tmp, new_cobiwm_rect (1200,   20,  400, 1180));
  tmp = g_list_prepend (tmp, new_cobiwm_rect (   0,   20,  800, 1130));
  tmp = g_list_prepend (tmp, new_cobiwm_rect (   0,   20, 1600, 1080));
  verify_lists_are_equal (region, tmp);
  cobiwm_rectangle_free_list_and_elements (tmp);
  cobiwm_rectangle_free_list_and_elements (region);
  ...
  tmp = NULL;
  tmp = g_list_prepend (tmp, new_cobiwm_rect (   0,   20,  300, 1180)); /* 354000 */
  tmp = g_list_prepend (tmp, new_cobiwm_rect ( 380,   20,  1220, 1180)); /* 377600 */
  tmp = g_list_prepend (tmp, new_cobiwm_rect (   0,   20,  1600, 1130)); /* 791000 */
#if 0
  printf ("Got to here...\n");
  char region_list[(RECT_LENGTH+2) * g_list_length (region)];
  char tmp_list[   (RECT_LENGTH+2) * g_list_length (tmp)];
  cobiwm_rectangle_region_to_string (region, ", ", region_list);
  cobiwm_rectangle_region_to_string (region, ", ", tmp_list);
  ...
  tmp = NULL;
  tmp = g_list_prepend (tmp, new_cobiwm_rect ( 800,   20,  800, 1180));
  verify_lists_are_equal (region, tmp);
  cobiwm_rectangle_free_list_and_elements (tmp);
  cobiwm_rectangle_free_list_and_elements (region);
...
test_region_fitting (void)
{
  ...
  for (i = 0; i < NUM_RANDOM_RUNS; i++)
    {
      ...
      g_assert (cobiwm_rectangle_contained_in_region (region, &rect) == FALSE ||
                cobiwm_rectangle_could_fit_in_region (region, &rect) == TRUE);
    }
  cobiwm_rectangle_free_list_and_elements (region);
  ...
  rect = cobiwm_rect (50, 50, 400, 400);
  g_assert (cobiwm_rectangle_could_fit_in_region (region, &rect));
  g_assert (cobiwm_rectangle_contained_in_region (region, &rect));

  rect = cobiwm_rect (250, 0, 500, 1150);
  g_assert (!cobiwm_rectangle_could_fit_in_region (region, &rect));
  g_assert (!cobiwm_rectangle_contained_in_region (region, &rect));

  rect = cobiwm_rect (250, 0, 400, 400);
  g_assert (cobiwm_rectangle_could_fit_in_region (region, &rect));
  g_assert (!cobiwm_rectangle_contained_in_region (region, &rect));
  ...
  region = get_screen_region (2);
  rect = cobiwm_rect (1000, 50, 600, 1100);
  g_assert (cobiwm_rectangle_could_fit_in_region (region, &rect));
  g_assert (!cobiwm_rectangle_contained_in_region (region, &rect));
...
test_clamping_to_region (void)
{
  ...
  for (i = 0; i < NUM_RANDOM_RUNS; i++)
    {
      ...
      get_random_rect (&rect);
      temp = rect;
      cobiwm_rectangle_clamp_to_fit_into_region (region,
                                               fixed_directions,
                                               &rect,
                                               &min_size);
      g_assert (cobiwm_rectangle_could_fit_in_region (region, &rect) == TRUE);
  ...
  rect = cobiwm_rect (50, 50, 10000, 10000);
  cobiwm_rectangle_clamp_to_fit_into_region (region,
                                           fixed_directions,
                                           &rect,
                                           &min_size);
  ...
  rect = cobiwm_rect (275, -50, 410, 10000);
  cobiwm_rectangle_clamp_to_fit_into_region (region,
                                           fixed_directions,
                                           &rect,
                                           &min_size);
  ...
  rect = cobiwm_rect (350, 50, 100, 1100);
  ...
  rect = cobiwm_rect (300, 70, 500, 1100);
  min_size.width = 1;  min_size.height = 1;
  ...
  printf ("The next test intentionally causes a warning, "
          "but it can be ignored.\n");
  rect = cobiwm_rect (300, 70, 999999, 999999);
                      ...
                      const CobiwmRectangle *rect)
{
  ...
  while (!overlaps && temp != NULL)
    {
      overlaps = overlaps || cobiwm_rectangle_overlap (temp->data, rect);
      temp = temp->next;
    }
...
test_clipping_to_region (void)
{
  ...
  for (i = 0; i < NUM_RANDOM_RUNS; i++)
    {
      ...
      if (rect_overlaps_region (region, &rect))
        {
          cobiwm_rectangle_clip_to_region (region, 0, &rect);
          g_assert (cobiwm_rectangle_contained_in_region (region, &rect) == TRUE);
        }
    }
  cobiwm_rectangle_free_list_and_elements (region);
  ...
  rect = cobiwm_rect (-50, -10, 10000, 10000);
  cobiwm_rectangle_clip_to_region (region,
                                 fixed_directions,
                                 &rect);
  g_assert (cobiwm_rectangle_equal (region->data, &rect));

  rect = cobiwm_rect (300, 1000, 400, 200);
  temp = cobiwm_rect (300, 1000, 400, 150);
  cobiwm_rectangle_clip_to_region (region,
                                 fixed_directions,
                                 &rect);
  g_assert (cobiwm_rectangle_equal (&rect, &temp));

  rect = cobiwm_rect (400, 1000, 300, 200);
  temp = cobiwm_rect (450, 1000, 250, 200);
  cobiwm_rectangle_clip_to_region (region,
                                 fixed_directions,
                                 &rect);
  g_assert (cobiwm_rectangle_equal (&rect, &temp));
  ...
  rect = cobiwm_rect (400, 1000, 300, 200);
  temp = cobiwm_rect (400, 1000, 300, 150);
  cobiwm_rectangle_clip_to_region (region,
                                 FIXED_DIRECTION_X,
                                 &rect);
  g_assert (cobiwm_rectangle_equal (&rect, &temp));
...
test_shoving_into_region (void)
{
  ...
  for (i = 0; i < NUM_RANDOM_RUNS; i++)
    {
      get_random_rect (&rect);
      if (cobiwm_rectangle_could_fit_in_region (region, &rect))
        {
          cobiwm_rectangle_shove_into_region (region, 0, &rect);
          g_assert (cobiwm_rectangle_contained_in_region (region, &rect));
        }
    }
  cobiwm_rectangle_free_list_and_elements (region);
  ...
  rect = cobiwm_rect (300, 1000, 400, 200);
  temp = cobiwm_rect (300,  950, 400, 200);
  cobiwm_rectangle_shove_into_region (region,
                                    fixed_directions,
                                    &rect);
  g_assert (cobiwm_rectangle_equal (&rect, &temp));

  rect = cobiwm_rect (425, 1000, 300, 200);
  temp = cobiwm_rect (450, 1000, 300, 200);
  cobiwm_rectangle_shove_into_region (region,
                                    fixed_directions,
                                    &rect);
  g_assert (cobiwm_rectangle_equal (&rect, &temp));
  ...
  rect = cobiwm_rect (425, 1000, 300, 200);
  temp = cobiwm_rect (425,  950, 300, 200);
  cobiwm_rectangle_shove_into_region (region,
                                    FIXED_DIRECTION_X,
                                    &rect);
  g_assert (cobiwm_rectangle_equal (&rect, &temp));
  ...
  rect = cobiwm_rect ( 300, 1000, 400, 200);
  temp = cobiwm_rect (1200, 1000, 400, 200);
  cobiwm_rectangle_shove_into_region (region,
                                    FIXED_DIRECTION_Y,
                                    &rect);
  g_assert (cobiwm_rectangle_equal (&rect, &temp));

  rect = cobiwm_rect ( 800, 1150, 400,  50);  /* Completely "offscreen" :) */
  temp = cobiwm_rect ( 800, 1050, 400,  50);
  cobiwm_rectangle_shove_into_region (region,
                                    0,
                                    &rect);
  g_assert (cobiwm_rectangle_equal (&rect, &temp));

  rect = cobiwm_rect (-1000,  0, 400, 150);  /* Offscreen in 2 directions */
  temp = cobiwm_rect (    0, 20, 400, 150);
  cobiwm_rectangle_shove_into_region (region,
                                    0,
                                    &rect);
  g_assert (cobiwm_rectangle_equal (&rect, &temp));
...
verify_edge_lists_are_equal (GList *code, GList *answer)
{
  ...
  while (code && answer)
    {
      CobiwmEdge *a = code->data;
      CobiwmEdge *b = answer->data;

      if (!cobiwm_rectangle_equal (&a->rect, &b->rect) ||
...
test_find_onscreen_edges (void)
{
  ...
  int left   = COBIWM_DIRECTION_LEFT;
  int right  = COBIWM_DIRECTION_RIGHT;
  int top    = COBIWM_DIRECTION_TOP;
  int bottom = COBIWM_DIRECTION_BOTTOM;
  ...
  cobiwm_rectangle_edge_list_to_string (edges, "\n ", big_buffer1);
  cobiwm_rectangle_edge_list_to_string (tmp,   "\n ", big_buffer2);
...
{
  CobiwmRectangle oldrect, rect, temp;
  ...
  rect.x = -500;  /* Some random amount not equal to oldrect.x to ensure that
                   ...
                   */
  oldrect = cobiwm_rect ( 50,  300, 250, 400);
  temp    = cobiwm_rect ( 50,  300,  20,   5);
  cobiwm_rectangle_resize_with_gravity (&oldrect,
                                      &rect,
                                      NorthWestGravity,
                                      20,
                                      5);
  ...
  rect = cobiwm_rect ( 50,  300, 250, 400);
  temp = cobiwm_rect (165,  300,  20,   5);
  cobiwm_rectangle_resize_with_gravity (&rect,
                                      &rect,
                                      NorthGravity,
                                      20,
                                      5);
  ...
  rect = cobiwm_rect (167,  738, 237, 843);
  temp = cobiwm_rect (167, 1113, 832,  93);
  cobiwm_rectangle_resize_with_gravity (&rect,
                                      &rect,
                                      WestGravity,
                                      832,
  ...
  rect = cobiwm_rect ( 167,  738, 237, 843);
  temp = cobiwm_rect (-131, 1113, 833,  93);
  cobiwm_rectangle_resize_with_gravity (&rect,
                                      &rect,
                                      CenterGravity,
                                      832,
  ...
  rect = cobiwm_rect (300, 1000, 400, 200);
  temp = cobiwm_rect (270,  994, 430, 212);
  cobiwm_rectangle_resize_with_gravity (&rect,
                                      &rect,
                                      EastGravity,
                                      430,
  ...
  rect = cobiwm_rect (300, 1000, 400, 200);
  temp = cobiwm_rect (300, 1000, 430, 211);
  cobiwm_rectangle_resize_with_gravity (&rect,
                                      &rect,
                                      StaticGravity,
                                      430,
...
test_find_closest_point_to_line (void)
{
  ...
  cobiwm_rectangle_find_linepoint_closest_to_point (x1,  y1,
                                                  x2,  y2,
                                                  px,  py,
                                                  &rx, &ry);
