 ...
 * SECTION:window
 * @title: CobiwmWindow
 * @short_description: Cobiwm X managed windows
 */
...
static void     invalidate_work_areas     (CobiwmWindow     *window);
static void     set_wm_state              (CobiwmWindow     *window);
static void     set_net_wm_state          (CobiwmWindow     *window);
static void     cobiwm_window_set_above     (CobiwmWindow     *window,
                                           gboolean        new_value);

static void     cobiwm_window_force_placement (CobiwmWindow     *window);

static void     cobiwm_window_show          (CobiwmWindow     *window);
static void     cobiwm_window_hide          (CobiwmWindow     *window);

static void     cobiwm_window_save_rect         (CobiwmWindow    *window);

static void     ensure_mru_position_after (CobiwmWindow *window,
                                           CobiwmWindow *after_this_one);

static void cobiwm_window_move_resize_now (CobiwmWindow  *window);

static void cobiwm_window_unqueue (CobiwmWindow *window, guint queuebits);

static void     update_move           (CobiwmWindow   *window,
                                       gboolean      snap,
                                       int           x,
                                       int           y);
static gboolean update_move_timeout   (gpointer data);
static void     update_resize         (CobiwmWindow   *window,
                                       gboolean      snap,
                                       int           x,
                                       int           y,
                                       gboolean      force);
static gboolean update_resize_timeout (gpointer data);
static gboolean should_be_on_all_workspaces (CobiwmWindow *window);

static void cobiwm_window_flush_calc_showing   (CobiwmWindow *window);

static gboolean queue_calc_showing_func (CobiwmWindow *window,
                                         void       *data);

static void cobiwm_window_move_between_rects (CobiwmWindow          *window,
                                            const CobiwmRectangle *old_area,
                                            const CobiwmRectangle *new_area);

static void unmaximize_window_before_freeing (CobiwmWindow        *window);
static void unminimize_window_and_all_transient_parents (CobiwmWindow *window);

static void cobiwm_window_propagate_focus_appearance (CobiwmWindow *window,
                                                    gboolean    focused);
static void cobiwm_window_update_icon_now (CobiwmWindow *window,
                                         gboolean    force);

static void set_workspace_state (CobiwmWindow    *window,
                                 gboolean       on_all_workspaces,
                                 CobiwmWorkspace *workspace);

/* Idle handlers for the three queues (run with cobiwm_later_add()). The
...
G_DEFINE_ABSTRACT_TYPE (CobiwmWindow, cobiwm_window, G_TYPE_OBJECT);
                        ...
                        gpointer       data)
{
  ...
  if (pref == COBIWM_PREF_WORKSPACES_ONLY_ON_PRIMARY)
    {
      cobiwm_window_on_all_workspaces_changed (window);
    }
  else if (pref == COBIWM_PREF_ATTACH_MODAL_DIALOGS &&
           window->type == COBIWM_WINDOW_MODAL_DIALOG)
    {
      window->attached = cobiwm_window_should_attach_to_parent (window);
...
static void
cobiwm_window_real_grab_op_began (CobiwmWindow *window,
                                CobiwmGrabOp  op)
{
}

static void
cobiwm_window_real_grab_op_ended (CobiwmWindow *window,
                                CobiwmGrabOp  op)
{
...
static void
cobiwm_window_real_current_workspace_changed (CobiwmWindow *window)
{
}

static gboolean
cobiwm_window_real_update_struts (CobiwmWindow *window)
{
  return FALSE;
}

static void
cobiwm_window_real_get_default_skip_hints (CobiwmWindow *window,
...
static gboolean
cobiwm_window_real_update_icon (CobiwmWindow       *window,
...
static void
cobiwm_window_finalize (GObject *object)
{
  ...
  G_OBJECT_CLASS (cobiwm_window_parent_class)->finalize (object);
}

static void
cobiwm_window_get_property(GObject         *object,
                         guint            prop_id,
                         GValue          *value,
                         GParamSpec      *pspec)
{
  CobiwmWindow *win = COBIWM_WINDOW (object);
  ...
  switch (prop_id)
    ...
    case PROP_SKIP_TASKBAR:
      ...
      break;
    case PROP_COBIWM_HINTS:
      g_value_set_string (value, win->cobiwm_hints);
      break;
    case PROP_APPEARS_FOCUSED:
      g_value_set_boolean (value, cobiwm_window_appears_focused (win));
      break;
    case PROP_WM_CLASS:
...
static void
cobiwm_window_set_property(GObject         *object,
                         guint            prop_id,
                         const GValue    *value,
                         GParamSpec      *pspec)
{
...
static void
cobiwm_window_class_init (CobiwmWindowClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->finalize = cobiwm_window_finalize;

  object_class->get_property = cobiwm_window_get_property;
  object_class->set_property = cobiwm_window_set_property;

  klass->grab_op_began = cobiwm_window_real_grab_op_began;
  klass->grab_op_ended = cobiwm_window_real_grab_op_ended;
  klass->current_workspace_changed = cobiwm_window_real_current_workspace_changed;
  klass->update_struts = cobiwm_window_real_update_struts;
  klass->get_default_skip_hints = cobiwm_window_real_get_default_skip_hints;
  klass->update_icon = cobiwm_window_real_update_icon;
  ...
  obj_props[PROP_SKIP_TASKBAR] =
    g_param_spec_boolean ("skip-taskbar",
                          ...
                          FALSE,
                          G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);
  obj_props[PROP_COBIWM_HINTS] =
    g_param_spec_string ("cobiwm-hints",
                         "_COBIWM_HINTS",
                         "Contents of the _COBIWM_HINTS property of this window",
                         NULL,
                         G_PARAM_READABLE | G_PARAM_STATIC_STRINGS);
...
static void
cobiwm_window_init (CobiwmWindow *self)
{
  self->stamp = next_window_stamp++;
  cobiwm_prefs_add_listener (prefs_changed_callback, self);
}

static gboolean
is_desktop_or_dock_foreach (CobiwmWindow *window,
                            void       *data)
{
 ...
 */
static void
maybe_leave_show_desktop_mode (CobiwmWindow *window)
{
  ...
  cobiwm_window_foreach_ancestor (window, is_desktop_or_dock_foreach,
                                &is_desktop_or_dock);

  if (!is_desktop_or_dock)
    {
      cobiwm_screen_minimize_all_on_active_workspace_except (window->screen,
                                                           window);
      cobiwm_screen_unshow_desktop (window->screen);
    }
}

gboolean
cobiwm_window_should_attach_to_parent (CobiwmWindow *window)
{
  CobiwmWindow *parent;

  if (!cobiwm_prefs_get_attach_modal_dialogs () ||
      window->type != COBIWM_WINDOW_MODAL_DIALOG)
    return FALSE;

  parent = cobiwm_window_get_transient_for (window);
  if (!parent)
    return FALSE;
...
static gboolean
client_window_should_be_mapped (CobiwmWindow *window)
{
#ifdef HAVE_WAYLAND
  if (window->client_type == COBIWM_WINDOW_CLIENT_TYPE_WAYLAND &&
      !cobiwm_wayland_surface_get_buffer (window->surface))
    return FALSE;
#endif
...
static void
sync_client_window_mapped (CobiwmWindow *window)
{
...
static void
cobiwm_window_update_desc (CobiwmWindow *window)
{
  ...
  if (window->client_type == COBIWM_WINDOW_CLIENT_TYPE_X11)
    {
      if (title)
...
static void
cobiwm_window_main_monitor_changed (CobiwmWindow *window,
                                  const CobiwmMonitorInfo *old)
{
  COBIWM_WINDOW_GET_CLASS (window)->main_monitor_changed (window, old);
...
CobiwmWindow *
_cobiwm_window_shared_new (CobiwmDisplay         *display,
                         CobiwmScreen          *screen,
                         ...
                         XWindowAttributes   *attrs)
{
  ...
  cobiwm_verbose ("attrs->map_state = %d (%s)\n",
                attrs->map_state,
  ...
  if (client_type == COBIWM_WINDOW_CLIENT_TYPE_X11)
    window = g_object_new (COBIWM_TYPE_WINDOW_X11, NULL);
#ifdef HAVE_WAYLAND
  else if (client_type == COBIWM_WINDOW_CLIENT_TYPE_WAYLAND)
    window = g_object_new (COBIWM_TYPE_WINDOW_WAYLAND, NULL);
#endif
  else
  ...
  cobiwm_display_register_stamp (window->display, &window->stamp, window);
  ...
  cobiwm_window_update_desc (window);
  ...
  cobiwm_set_normal_hints (window, NULL);
  ...
  window->tile_mode = COBIWM_TILE_NONE;
  ...
  window->type = COBIWM_WINDOW_NORMAL;

  window->struts = NULL;

  window->layer = COBIWM_LAYER_LAST; /* invalid value */
  ...
  /* Assign this #CobiwmWindow a sequence number which can be used
   * for sorting.
   */
  ...
  COBIWM_WINDOW_GET_CLASS (window)->manage (window);

  if (!window->override_redirect)
    cobiwm_window_update_icon_now (window, TRUE);

  if (window->initially_iconic)
    {
      /* WM_HINTS said minimized */
      window->minimized = TRUE;
      cobiwm_verbose ("Window %s asked to start out minimized\n", window->desc);
    }
  ...
  if (existing_wm_state == IconicState)
    {
      ...
      window->minimized = TRUE;
      cobiwm_verbose ("Window %s had preexisting WM_STATE = IconicState, minimizing\n",
                    window->desc);
   ...
   */
  cobiwm_screen_apply_startup_properties (window->screen, window);
  ...
  if (!window->override_redirect && !window->net_wm_user_time_set) {
    ...
    if (window->initial_timestamp_set)
      ...
      cobiwm_window_set_user_time (window, window->transient_for->net_wm_user_time);
    else
      /* NOTE: Do NOT toggle net_wm_user_time_set to true; this is just
       * being recorded as a fallback for potential transients
       */
  ...
  if (window->type == COBIWM_WINDOW_DESKTOP ||
      window->type == COBIWM_WINDOW_DOCK)
    {
  ...
  if (window->initial_workspace_set)
    {
      ...
      CobiwmWorkspace *workspace = NULL;

      if (window->initial_workspace == (int) 0xFFFFFFFF)
        {
          cobiwm_topic (COBIWM_DEBUG_PLACEMENT,
	   ...
	   */
      ...
      else if (!on_all_workspaces)
        {
          ...
          workspace = cobiwm_screen_get_workspace_by_index (window->screen,
  ...
  cobiwm_window_main_monitor_changed (window, NULL);
   ...
   */
  if (!window->override_redirect)
    cobiwm_stack_add (window->screen->stack,
                    window);
  else
    window->layer = COBIWM_LAYER_OVERRIDE_REDIRECT; /* otherwise set by CobiwmStack */
  ...
  cobiwm_compositor_add_window (screen->display->compositor, window);
  window->known_to_compositor = TRUE;

  /* Sync stack changes */
  cobiwm_stack_thaw (window->screen->stack);
   ...
   */
  cobiwm_stack_tracker_queue_sync_stack (window->screen->stack_tracker);
   ...
   */
  cobiwm_window_foreach_transient (window,
  ...
  cobiwm_display_notify_window_created (display, window);
...
static gboolean
detach_foreach_func (CobiwmWindow *window,
                     void       *data)
{
  GList **children = data;
  CobiwmWindow *parent;
...
void
cobiwm_window_unmanage (CobiwmWindow  *window,
                      guint32      timestamp)
{
  GList *tmp;

  cobiwm_verbose ("Unmanaging %s\n", window->desc);
  window->unmanaging = TRUE;

#ifdef HAVE_WAYLAND
  /* This needs to happen for both Wayland and XWayland clients,
   * so it can't be in CobiwmWindowWayland. */
  if (window->surface)
    {
      cobiwm_wayland_surface_set_window (window->surface, NULL);
      window->surface = NULL;
    }
#endif
  ...
  if (window->visible_to_compositor)
    {
      window->visible_to_compositor = FALSE;
      cobiwm_compositor_hide_window (window->display->compositor, window,
                                   COBIWM_COMP_EFFECT_DESTROY);
    }

  cobiwm_compositor_remove_window (window->display->compositor, window);
  window->known_to_compositor = FALSE;

  if (destroying_windows_disallowed > 0)
    cobiwm_bug ("Tried to destroy window %s while destruction was not allowed\n",
              window->desc);

  cobiwm_display_unregister_stamp (window->display, window->stamp);
  ...
  if (cobiwm_prefs_get_attach_modal_dialogs ())
    {
      ...
      for (iter = attached_children; iter; iter = iter->next)
        cobiwm_window_unmanage (iter->data, timestamp);
   ...
   */
  if (window->screen->closing && cobiwm_prefs_get_workspaces_only_on_primary ())
    cobiwm_window_on_all_workspaces_changed (window);
  ...
  if (window->fullscreen)
    {
      ...
      cobiwm_stack_freeze (window->screen->stack);
      group = cobiwm_window_get_group (window);
      if (group)
        cobiwm_group_update_layers (group);
      cobiwm_stack_thaw (window->screen->stack);
    }

  cobiwm_display_remove_pending_pings_for_window (window->display, window);

  /* safe to do this early as group.c won't re-add to the
   * group if window->unmanaging */
  cobiwm_window_shutdown_group (window);
  ...
  if (cobiwm_window_appears_focused (window))
    cobiwm_window_propagate_focus_appearance (window, FALSE);
  if (window->has_focus)
    {
      cobiwm_topic (COBIWM_DEBUG_FOCUS,
                  "Focusing default window since we're unmanaging %s\n",
                  window->desc);
      cobiwm_workspace_focus_default_window (window->screen->active_workspace, NULL, timestamp);
    }
  else
    {
      cobiwm_topic (COBIWM_DEBUG_FOCUS,
  ...
  if (window->struts)
    {
      cobiwm_free_gslist_and_elements (window->struts);
      window->struts = NULL;
  ...
  cobiwm_window_unqueue (window, COBIWM_QUEUE_CALC_SHOWING |
                               COBIWM_QUEUE_MOVE_RESIZE |
                               COBIWM_QUEUE_UPDATE_ICON);
  cobiwm_window_free_delete_dialog (window);
  ...
  while (tmp != NULL)
    {
      CobiwmWorkspace *workspace = tmp->data;
  ...
  if (window->monitor)
    {
      const CobiwmMonitorInfo *old = window->monitor;

      window->monitor = NULL;
      cobiwm_window_main_monitor_changed (window, old);
    }

  if (!window->override_redirect)
    cobiwm_stack_remove (window->screen->stack, window);
  ...
  if (window->display->autoraise_window == window)
    cobiwm_display_remove_autoraise_callback (window->display);

  COBIWM_WINDOW_GET_CLASS (window)->unmanage (window);

  cobiwm_prefs_remove_listener (prefs_changed_callback, window);
...
static void
set_wm_state (CobiwmWindow *window)
{
  if (window->client_type == COBIWM_WINDOW_CLIENT_TYPE_X11)
...
static void
set_net_wm_state (CobiwmWindow *window)
{
  if (window->client_type == COBIWM_WINDOW_CLIENT_TYPE_X11)
...
static void
set_allowed_actions_hint (CobiwmWindow *window)
{
  if (window->client_type == COBIWM_WINDOW_CLIENT_TYPE_X11)
    cobiwm_window_x11_set_allowed_actions_hint (window);
}

/**
 * cobiwm_window_located_on_workspace:
 * @window: a #CobiwmWindow
 ...
 */
gboolean
cobiwm_window_located_on_workspace (CobiwmWindow    *window,
                                  CobiwmWorkspace *workspace)
{
...
static gboolean
is_minimized_foreach (CobiwmWindow *window,
                      void       *data)
{
  gboolean *result = data;
...
static gboolean
ancestor_is_minimized (CobiwmWindow *window)
{
  gboolean is_minimized;

  is_minimized = FALSE;

  cobiwm_window_foreach_ancestor (window, is_minimized_foreach, &is_minimized);

  return is_minimized;
}

/**
 * cobiwm_window_showing_on_its_workspace:
 * @window: A #CobiwmWindow
 *
 * Returns: %TRUE if window would be visible, if its workspace was current
 */
gboolean
cobiwm_window_showing_on_its_workspace (CobiwmWindow *window)
{
  gboolean showing;
  gboolean is_desktop_or_dock;
  CobiwmWorkspace* workspace_of_window;
  ...
  if (showing &&
      ...
      !is_desktop_or_dock)
    {
      cobiwm_verbose ("We're showing the desktop on the workspace(s) that window %s is on\n",
                    window->desc);
      showing = FALSE;
    }
...
gboolean
cobiwm_window_should_be_showing (CobiwmWindow  *window)
{
#ifdef HAVE_WAYLAND
  if (window->client_type == COBIWM_WINDOW_CLIENT_TYPE_WAYLAND &&
      !cobiwm_wayland_surface_get_buffer (window->surface))
  ...
  return (cobiwm_window_located_on_workspace (window, window->screen->active_workspace) &&
          cobiwm_window_showing_on_its_workspace (window));
}

static void
implement_showing (CobiwmWindow *window,
                   gboolean    showing)
{
  /* Actually show/hide the window */
  cobiwm_verbose ("Implement showing = %d for window %s\n",
                showing, window->desc);
  ...
  if (!showing)
    {
      ...
      if (!window->placed && client_window_should_be_mapped (window))
        cobiwm_window_force_placement (window);

      cobiwm_window_hide (window);
    }
  else
    cobiwm_window_show (window);
...
static void
cobiwm_window_calc_showing (CobiwmWindow  *window)
{
  implement_showing (window, cobiwm_window_should_be_showing (window));
}
...
{
  CobiwmWindow *aw = (gpointer) a;
  CobiwmWindow *bw = (gpointer) b;

  if (aw->screen != bw->screen)
    return 0; /* don't care how they sort with respect to each other */
  else
    return cobiwm_stack_windows_cmp (aw->screen->stack,
                                   aw, bw);
}
  ...
  while (tmp != NULL)
    {
      ...
      else if (cobiwm_window_should_be_showing (window))
  ...
  while (tmp != NULL)
    {
      ...
      cobiwm_window_calc_showing (window);
  ...
  while (tmp != NULL)
    {
       ...
       */
      window->is_in_queues &= ~COBIWM_QUEUE_CALC_SHOWING;
      ...
      while (tmp != NULL)
        {
          ...
          if (!window->display->mouse_mode)
            cobiwm_display_increment_focus_sentinel (window->display);
...
static const gchar* cobiwm_window_queue_names[NUMBER_OF_QUEUES] =
  {"calc_showing", "move_resize", "update_icon"};
#endif

static void
cobiwm_window_unqueue (CobiwmWindow *window, guint queuebits)
{
  gint queuenum;
      ...
      if ((queuebits & 1<<queuenum) /* they have asked to unqueue */
          ...
          (window->is_in_queues & 1<<queuenum)) /* it's in the queue */
        {
          ...
          cobiwm_topic (COBIWM_DEBUG_WINDOW_STATE,
              ...
              window->desc,
              cobiwm_window_queue_names[queuenum]);
          ...
          if (queue_pending[queuenum] == NULL && queue_later[queuenum] != 0)
            {
              cobiwm_later_remove (queue_later[queuenum]);
...
static void
cobiwm_window_flush_calc_showing (CobiwmWindow *window)
{
  if (window->is_in_queues & COBIWM_QUEUE_CALC_SHOWING)
    {
      cobiwm_window_unqueue (window, COBIWM_QUEUE_CALC_SHOWING);
      cobiwm_window_calc_showing (window);
    }
...
void
cobiwm_window_queue (CobiwmWindow *window, guint queuebits)
{
  guint queuenum;

  /* Easier to debug by checking here rather than in the idle */
  g_return_if_fail (!window->override_redirect || (queuebits & COBIWM_QUEUE_MOVE_RESIZE) == 0);
      ...
      if (queuebits & 1<<queuenum)
        {
          ...
          const CobiwmLaterType window_queue_later_when[NUMBER_OF_QUEUES] =
            {
              COBIWM_LATER_CALC_SHOWING, /* CALC_SHOWING */
              COBIWM_LATER_RESIZE,        /* MOVE_RESIZE */
              COBIWM_LATER_BEFORE_REDRAW  /* UPDATE_ICON */
            };
          ...
          if (queue_later[queuenum] == 0)
            queue_later[queuenum] = cobiwm_later_add
              (
...
static gboolean
intervening_user_event_occurred (CobiwmWindow *window)
{
  guint32 compare;
 ...
 */
static gboolean
window_is_terminal (CobiwmWindow *window)
{
 ...
 */
static void
window_state_on_map (CobiwmWindow *window,
                     ...
                     gboolean *places_on_top)
{
  ...
  *places_on_top = *takes_focus;
   ...
   */
  if (*takes_focus &&
      cobiwm_prefs_get_focus_new_windows () == G_DESKTOP_FOCUS_NEW_WINDOWS_STRICT &&
      !window->display->allow_terminal_deactivation &&
      window_is_terminal (window->display->focus_window) &&
      !cobiwm_window_is_ancestor_of_transient (window->display->focus_window,
                                             window))
    {
      cobiwm_topic (COBIWM_DEBUG_FOCUS,
...
static gboolean
windows_overlap (const CobiwmWindow *w1, const CobiwmWindow *w2)
{
  CobiwmRectangle w1rect, w2rect;
  cobiwm_window_get_frame_rect (w1, &w1rect);
  cobiwm_window_get_frame_rect (w2, &w2rect);
  return cobiwm_rectangle_overlap (&w1rect, &w2rect);
}
 ...
 */
static gboolean
window_would_be_covered (const CobiwmWindow *newbie)
{
  CobiwmWorkspace *workspace = cobiwm_window_get_workspace ((CobiwmWindow *)newbie);
  GList *tmp, *windows;
...
static void
cobiwm_window_force_placement (CobiwmWindow *window)
{
  if (window->placed)
    return;
  ...
  cobiwm_window_move_resize_now (window);
  window->calc_placement = FALSE;
...
static void
cobiwm_window_show (CobiwmWindow *window)
{
  gboolean did_show;
  ...
  if ( focus_window != NULL && window->showing_for_first_time &&
      ...
      if (cobiwm_window_is_ancestor_of_transient (focus_window, window))
        {
          guint32     timestamp;
  ...
  if (!window->placed)
    {
      if (cobiwm_prefs_get_auto_maximize() && window->showing_for_first_time && window->has_maximize_func)
        {
          CobiwmRectangle work_area;
          cobiwm_window_get_work_area_for_monitor (window, window->monitor->number, &work_area);
  ...
  if (needs_stacking_adjustment)
    {
       ...
       */
      if (overlap ||
          (cobiwm_prefs_get_focus_mode () == G_DESKTOP_FOCUS_MODE_CLICK &&
           cobiwm_prefs_get_raise_on_click ()))
        cobiwm_window_stack_just_below (window, focus_window);
       ...
       * cobiwm_window_set_demands_attention() because that would cause
  ...
  if (!window->visible_to_compositor)
    {
      CobiwmCompEffect effect = COBIWM_COMP_EFFECT_NONE;
      ...
      cobiwm_compositor_show_window (window->display->compositor,
                                   window, effect);
      window->pending_compositor_effect = COBIWM_COMP_EFFECT_NONE;
    }
...
#ifdef HAVE_WAYLAND
  if (did_show && window->client_type == COBIWM_WINDOW_CLIENT_TYPE_WAYLAND)
    cobiwm_wayland_compositor_repick (cobiwm_wayland_compositor_get_default ());
#endif
...
static void
cobiwm_window_hide (CobiwmWindow *window)
{
  gboolean did_hide;
  ...
  if (window->visible_to_compositor)
    {
      ...
      cobiwm_compositor_hide_window (window->display->compositor, window, effect);
      window->pending_compositor_effect = COBIWM_COMP_EFFECT_NONE;
    }
  ...
  if (window->has_focus)
    {
      CobiwmWindow *not_this_one = NULL;
      CobiwmWorkspace *my_workspace = cobiwm_window_get_workspace (window);
      guint32 timestamp = cobiwm_display_get_current_time_roundtrip (window->display);
...
static gboolean
queue_calc_showing_func (CobiwmWindow *window,
                         void       *data)
{
  cobiwm_window_queue(window, COBIWM_QUEUE_CALC_SHOWING);
  return TRUE;
...
void
cobiwm_window_minimize (CobiwmWindow  *window)
{
  g_return_if_fail (!window->override_redirect);
  ...
  if (!window->minimized)
    {
      window->minimized = TRUE;
      window->pending_compositor_effect = COBIWM_COMP_EFFECT_MINIMIZE;
      cobiwm_window_queue(window, COBIWM_QUEUE_CALC_SHOWING);
...
void
cobiwm_window_unminimize (CobiwmWindow  *window)
{
  g_return_if_fail (!window->override_redirect);
  ...
  if (window->minimized)
    {
      window->minimized = FALSE;
      window->pending_compositor_effect = COBIWM_COMP_EFFECT_UNMINIMIZE;
      cobiwm_window_queue(window, COBIWM_QUEUE_CALC_SHOWING);
...
static void
ensure_size_hints_satisfied (CobiwmRectangle    *rect,
...
static void
cobiwm_window_save_rect (CobiwmWindow *window)
{
  if (!(COBIWM_WINDOW_MAXIMIZED (window) || COBIWM_WINDOW_TILED_SIDE_BY_SIDE (window) || window->fullscreen))
    {
...
void
cobiwm_window_maximize_internal (CobiwmWindow        *window,
                               CobiwmMaximizeFlags  directions,
                               CobiwmRectangle     *saved_rect)
{
  /* At least one of the two directions ought to be set */
  gboolean maximize_horizontally, maximize_vertically;
  maximize_horizontally = directions & COBIWM_MAXIMIZE_HORIZONTAL;
  maximize_vertically   = directions & COBIWM_MAXIMIZE_VERTICAL;
  ...
  if (saved_rect != NULL)
    ...
    cobiwm_window_save_rect (window);
...
void
cobiwm_window_maximize (CobiwmWindow        *window,
                      CobiwmMaximizeFlags  directions)
{
  CobiwmRectangle *saved_rect = NULL;
  gboolean maximize_horizontally, maximize_vertically;
  ...
  if ((maximize_horizontally && !window->maximized_horizontally) ||
      ...
      if (!window->placed)
	{
      ...
      cobiwm_window_maximize_internal (window,
                                     directions,
                                     saved_rect);

      CobiwmRectangle old_frame_rect, old_buffer_rect;

      cobiwm_window_get_frame_rect (window, &old_frame_rect);
      cobiwm_window_get_buffer_rect (window, &old_buffer_rect);

      cobiwm_window_move_resize_internal (window,
                                        (COBIWM_MOVE_RESIZE_MOVE_ACTION |
                                         COBIWM_MOVE_RESIZE_RESIZE_ACTION |
                                         COBIWM_MOVE_RESIZE_STATE_CHANGED |
                                         COBIWM_MOVE_RESIZE_DONT_SYNC_COMPOSITOR),
                                        NorthWestGravity,
...
/**
 * cobiwm_window_get_maximized:
 * @window: a #CobiwmWindow
 *
 * Gets the current maximization state of the window, as combination
 * of the %COBIWM_MAXIMIZE_HORIZONTAL and %COBIWM_MAXIMIZE_VERTICAL flags;
 *
 * Return value: current maximization state
 */
CobiwmMaximizeFlags
cobiwm_window_get_maximized (CobiwmWindow *window)
{
  return ((window->maximized_horizontally ? COBIWM_MAXIMIZE_HORIZONTAL : 0) |
          (window->maximized_vertically ? COBIWM_MAXIMIZE_VERTICAL : 0));
}

/**
 * cobiwm_window_is_fullscreen:
 * @window: a #CobiwmWindow
 *
 * Return value: %TRUE if the window is currently fullscreen
 */
gboolean
cobiwm_window_is_fullscreen (CobiwmWindow *window)
{
  return window->fullscreen;
}

/**
 * cobiwm_window_get_all_monitors:
 * @window: The #CobiwmWindow
 ...
 */
gint *
cobiwm_window_get_all_monitors (CobiwmWindow *window, gsize *length)
{
  GArray *monitors;
  ...
  for (i = 0; i < window->screen->n_monitor_infos; i++)
    {
      CobiwmRectangle *monitor_rect = &window->screen->monitor_infos[i].rect;

      if (cobiwm_rectangle_overlap (&window_rect, monitor_rect))
 ...
 */
gboolean
cobiwm_window_is_screen_sized (CobiwmWindow *window)
{
  CobiwmRectangle window_rect;
  int screen_width, screen_height;

  cobiwm_screen_get_size (window->screen, &screen_width, &screen_height);
  cobiwm_window_get_frame_rect (window, &window_rect);
...
/**
 * cobiwm_window_is_monitor_sized:
 * @window: a #CobiwmWindow
 *
 ...
 */
gboolean
cobiwm_window_is_monitor_sized (CobiwmWindow *window)
{
  if (window->fullscreen)
    return TRUE;

  if (cobiwm_window_is_screen_sized (window))
    return TRUE;

  if (window->override_redirect)
    {
      CobiwmRectangle window_rect, monitor_rect;

      cobiwm_window_get_frame_rect (window, &window_rect);
      cobiwm_screen_get_monitor_geometry (window->screen, window->monitor->number, &monitor_rect);

      if (cobiwm_rectangle_equal (&window_rect, &monitor_rect))
        return TRUE;
    }
...
/**
 * cobiwm_window_is_on_primary_monitor:
 * @window: a #CobiwmWindow
 *
 * Return value: %TRUE if the window is on the primary monitor
 */
gboolean
cobiwm_window_is_on_primary_monitor (CobiwmWindow *window)
{
  return window->monitor->is_primary;
}

/**
 * cobiwm_window_requested_bypass_compositor:
 * @window: a #CobiwmWindow
 *
 * Return value: %TRUE if the window requested to bypass the compositor
 */
gboolean
cobiwm_window_requested_bypass_compositor (CobiwmWindow *window)
{
  return window->bypass_compositor == _NET_WM_BYPASS_COMPOSITOR_HINT_ON;
}

/**
 * cobiwm_window_requested_dont_bypass_compositor:
 * @window: a #CobiwmWindow
 *
 * Return value: %TRUE if the window requested to opt out of unredirecting
 */
gboolean
cobiwm_window_requested_dont_bypass_compositor (CobiwmWindow *window)
{
  return window->bypass_compositor == _NET_WM_BYPASS_COMPOSITOR_HINT_OFF;
}

void
cobiwm_window_tile (CobiwmWindow *window)
{
  CobiwmMaximizeFlags directions;
  ...
  if (window->tile_mode == COBIWM_TILE_MAXIMIZED)
    directions = COBIWM_MAXIMIZE_BOTH;
  else
    directions = COBIWM_MAXIMIZE_VERTICAL;

  cobiwm_window_maximize_internal (window, directions, NULL);
  cobiwm_screen_update_tile_preview (window->screen, FALSE);
...
static gboolean
cobiwm_window_can_tile_maximized (CobiwmWindow *window)
{
  return window->has_maximize_func;
}

gboolean
cobiwm_window_can_tile_side_by_side (CobiwmWindow *window)
{
  int monitor;
  CobiwmRectangle tile_area;
  CobiwmRectangle client_rect;

  if (!cobiwm_window_can_tile_maximized (window))
    return FALSE;

  monitor = cobiwm_screen_get_current_monitor (window->screen);
  cobiwm_window_get_work_area_for_monitor (window, monitor, &tile_area);
  ...
  cobiwm_window_frame_rect_to_client_rect (window, &tile_area, &client_rect);
...
static void
unmaximize_window_before_freeing (CobiwmWindow        *window)
{
  cobiwm_topic (COBIWM_DEBUG_WINDOW_OPS,
  ...
  if (window->withdrawn)                /* See bug #137185 */
  ...
  else if (window->screen->closing)     /* See bug #358042 */
    {
       ...
       * before closing it. */
      cobiwm_window_move_resize_frame (window,
                                     FALSE,
                                     window->saved_rect.x,
                                     window->saved_rect.y,
...
void
cobiwm_window_unmaximize (CobiwmWindow        *window,
                        CobiwmMaximizeFlags  directions)
{
  ...
  /* At least one of the two directions ought to be set */
  unmaximize_horizontally = directions & COBIWM_MAXIMIZE_HORIZONTAL;
  unmaximize_vertically   = directions & COBIWM_MAXIMIZE_VERTICAL;
  ...
  if ((unmaximize_horizontally && window->maximized_horizontally) ||
    ...
    {
      CobiwmRectangle *desired_rect;
      CobiwmRectangle target_rect;
      CobiwmRectangle work_area;
      CobiwmRectangle old_frame_rect, old_buffer_rect;
       ...
       */
      cobiwm_window_frame_rect_to_client_rect (window, &target_rect, &target_rect);
      ensure_size_hints_satisfied (&target_rect, &window->size_hints);
      cobiwm_window_client_rect_to_frame_rect (window, &target_rect, &target_rect);
       ...
       * by constraints.
       */
      if (cobiwm_grab_op_is_moving (window->display->grab_op) &&
...
void
cobiwm_window_make_above (CobiwmWindow  *window)
{
  g_return_if_fail (!window->override_redirect);

  cobiwm_window_set_above (window, TRUE);
  cobiwm_window_raise (window);
}

void
cobiwm_window_unmake_above (CobiwmWindow  *window)
{
  g_return_if_fail (!window->override_redirect);

  cobiwm_window_set_above (window, FALSE);
  cobiwm_window_raise (window);
}

static void
cobiwm_window_set_above (CobiwmWindow *window,
                       gboolean    new_value)
{
...
void
cobiwm_window_make_fullscreen_internal (CobiwmWindow  *window)
{
  if (!window->fullscreen)
    {
      cobiwm_topic (COBIWM_DEBUG_WINDOW_OPS,
...
void
cobiwm_window_make_fullscreen (CobiwmWindow  *window)
{
  g_return_if_fail (!window->override_redirect);
...
void
cobiwm_window_unmake_fullscreen (CobiwmWindow  *window)
{
  g_return_if_fail (!window->override_redirect);

  if (window->fullscreen)
    {
      CobiwmRectangle old_frame_rect, old_buffer_rect, target_rect;
...
void
cobiwm_window_update_fullscreen_monitors (CobiwmWindow    *window,
                                        unsigned long  top,
...
void
cobiwm_window_shade (CobiwmWindow  *window,
                   guint32      timestamp)
{
  g_return_if_fail (!window->override_redirect);
  ...
  if (!window->shaded)
    {
      ...
      cobiwm_window_queue(window, COBIWM_QUEUE_MOVE_RESIZE | COBIWM_QUEUE_CALC_SHOWING);
      cobiwm_window_frame_size_changed (window);
...
void
cobiwm_window_unshade (CobiwmWindow  *window,
                     guint32      timestamp)
{
  g_return_if_fail (!window->override_redirect);
...
static gboolean
unminimize_func (CobiwmWindow *window,
                 void       *data)
{
  cobiwm_window_unminimize (window);
  return TRUE;
}

static void
unminimize_window_and_all_transient_parents (CobiwmWindow *window)
{
  cobiwm_window_unminimize (window);
  cobiwm_window_foreach_ancestor (window, unminimize_func, NULL);
}

void
cobiwm_window_activate_full (CobiwmWindow     *window,
                           guint32         timestamp,
                           ...
                           CobiwmWorkspace  *workspace)
{
  ...
  if (cobiwm_prefs_get_raise_on_click () ||
      source_indication == COBIWM_CLIENT_TYPE_PAGER)
    cobiwm_window_raise (window);
  ...
  if (cobiwm_window_located_on_workspace (window, workspace))
    ...
    cobiwm_workspace_activate_with_focus (window->workspace, window, timestamp);

  cobiwm_window_check_alive (window, timestamp);
}

/* This function exists since most of the functionality in window_activate
 * is useful for Cobiwm, but Cobiwm shouldn't need to specify a client
 * type for itself.  ;-)
 */
void
cobiwm_window_activate (CobiwmWindow     *window,
                      guint32         timestamp)
{
  g_return_if_fail (!window->override_redirect);
   ...
   */
  cobiwm_window_activate_full (window, timestamp, COBIWM_CLIENT_TYPE_PAGER, NULL);
}

void
cobiwm_window_activate_with_workspace (CobiwmWindow     *window,
                                     guint32         timestamp,
                                     CobiwmWorkspace  *workspace)
{
  g_return_if_fail (!window->override_redirect);

  cobiwm_window_activate_full (window, timestamp, COBIWM_CLIENT_TYPE_APPLICATION, workspace);
}

/**
 * cobiwm_window_updates_are_frozen:
 * @window: a #CobiwmWindow
 *
 ...
 */
gboolean
cobiwm_window_updates_are_frozen (CobiwmWindow *window)
{
...
static gboolean
maybe_move_attached_dialog (CobiwmWindow *window,
                            void       *data)
{
  if (cobiwm_window_is_attached_dialog (window))
    /* It ignores x,y for such a dialog  */
    cobiwm_window_move_frame (window, FALSE, 0, 0);

  return FALSE;
}

/**
 * cobiwm_window_get_monitor:
 * @window: a #CobiwmWindow
 *
 ...
 */
int
cobiwm_window_get_monitor (CobiwmWindow *window)
{
  return window->monitor->number;
}

static CobiwmMonitorInfo *
find_monitor_by_winsys_id (CobiwmWindow *window,
                           guint       winsys_id)
{
  int i;
  ...
  for (i = 0; i < window->screen->n_monitor_infos; i++)
    {
      CobiwmMonitorInfo *info = &window->screen->monitor_infos[i];
...
void
cobiwm_window_update_for_monitors_changed (CobiwmWindow *window)
{
  const CobiwmMonitorInfo *old, *new;

  if (window->override_redirect || window->type == COBIWM_WINDOW_DESKTOP)
    {
  ...
  /* This will eventually reach cobiwm_window_update_monitor that
  ...
  cobiwm_window_move_between_rects (window,
                                  &old->rect,
                                  &new->rect);
}

void
cobiwm_window_update_monitor (CobiwmWindow *window,
                            gboolean    user_op)
{
  const CobiwmMonitorInfo *old;

  old = window->monitor;
  COBIWM_WINDOW_GET_CLASS (window)->update_main_monitor (window);
  if (old != window->monitor)
    {
       ...
       * workspace.
       */
      if (cobiwm_prefs_get_workspaces_only_on_primary () && user_op &&
          cobiwm_window_is_on_primary_monitor (window)  &&
          window->screen->active_workspace != window->workspace)
        cobiwm_window_change_workspace (window, window->screen->active_workspace);
...
void
cobiwm_window_move_resize_internal (CobiwmWindow          *window,
                                  ...
                                  CobiwmRectangle        frame_rect)
{
  ...
  CobiwmMoveResizeResultFlags result = 0;
   ...
   */
  g_assert (flags & (COBIWM_MOVE_RESIZE_MOVE_ACTION | COBIWM_MOVE_RESIZE_RESIZE_ACTION | COBIWM_MOVE_RESIZE_WAYLAND_RESIZE));

  did_placement = !window->placed && window->calc_placement;

  /* We don't need it in the idle queue anymore. */
  cobiwm_window_unqueue (window, COBIWM_QUEUE_MOVE_RESIZE);

  if ((flags & COBIWM_MOVE_RESIZE_RESIZE_ACTION) && (flags & COBIWM_MOVE_RESIZE_MOVE_ACTION))
    {
      /* We're both moving and resizing. Just use the passed in rect. */
      unconstrained_rect = frame_rect;
    }
  else if ((flags & COBIWM_MOVE_RESIZE_RESIZE_ACTION))
    {
      ...
      cobiwm_rectangle_resize_with_gravity (&window->rect,
                                          &unconstrained_rect,
                                          gravity,
                                          frame_rect.width,
                                          frame_rect.height);
    }
  else if ((flags & COBIWM_MOVE_RESIZE_MOVE_ACTION))
    {
  ...
  else if ((flags & COBIWM_MOVE_RESIZE_WAYLAND_RESIZE))
    {
  ...
  if (flags & (COBIWM_MOVE_RESIZE_MOVE_ACTION | COBIWM_MOVE_RESIZE_RESIZE_ACTION))
    {
      CobiwmRectangle old_rect;
      cobiwm_window_get_frame_rect (window, &old_rect);

      cobiwm_window_constrain (window,
                             flags,
                             gravity,
                             &old_rect,
                             &constrained_rect);
   ...
   * was placed at to make sure that cobiwm_window_move_resize_now places the
   * window correctly.
   */
  if (did_placement)
    {
  ...
  COBIWM_WINDOW_GET_CLASS (window)->move_resize_internal (window, gravity, unconstrained_rect, constrained_rect, flags, &result);

  if (result & COBIWM_MOVE_RESIZE_RESULT_MOVED)
    g_signal_emit (window, window_signals[POSITION_CHANGED], 0);

  if (result & COBIWM_MOVE_RESIZE_RESULT_RESIZED)
    g_signal_emit (window, window_signals[SIZE_CHANGED], 0);

  if ((result & (COBIWM_MOVE_RESIZE_RESULT_MOVED | COBIWM_MOVE_RESIZE_RESULT_RESIZED)) != 0 || did_placement)
    {
      window->unconstrained_rect = unconstrained_rect;

      if (window->known_to_compositor && !(flags & COBIWM_MOVE_RESIZE_DONT_SYNC_COMPOSITOR))
  ...
  cobiwm_window_update_monitor (window, flags & COBIWM_MOVE_RESIZE_USER_ACTION);

  if (old_output_winsys_id != window->monitor->winsys_id &&
      flags & COBIWM_MOVE_RESIZE_MOVE_ACTION && flags & COBIWM_MOVE_RESIZE_USER_ACTION)
    window->preferred_output_winsys_id = window->monitor->winsys_id;

  if ((result & COBIWM_MOVE_RESIZE_RESULT_FRAME_SHAPE_CHANGED) && window->frame_bounds)
    {
      cairo_region_destroy (window->frame_bounds);
      window->frame_bounds = NULL;
    }

  cobiwm_window_foreach_transient (window, maybe_move_attached_dialog, NULL);

  cobiwm_stack_update_window_tile_matches (window->screen->stack,
                                         window->screen->active_workspace);
}

/**
 * cobiwm_window_move_frame:
 * @window: a #CobiwmWindow
 ...
 * Otherwise, acts identically to cobiwm_window_move().
 */
void
cobiwm_window_move_frame (CobiwmWindow *window,
                        gboolean    user_op,
                        int         root_x_nw,
                        int         root_y_nw)
{
  CobiwmMoveResizeFlags flags;
  CobiwmRectangle rect = { root_x_nw, root_y_nw, 0, 0 };

  g_return_if_fail (!window->override_redirect);

  flags = (user_op ? COBIWM_MOVE_RESIZE_USER_ACTION : 0) | COBIWM_MOVE_RESIZE_MOVE_ACTION;
  cobiwm_window_move_resize_internal (window, flags, NorthWestGravity, rect);
}

static void
cobiwm_window_move_between_rects (CobiwmWindow  *window,
                                const CobiwmRectangle *old_area,
                                const CobiwmRectangle *new_area)
{
  int rel_x, rel_y;
...
/**
 * cobiwm_window_move_resize_frame:
 * @window: a #CobiwmWindow
 * @user_op: bool to indicate whether or not this is a user operation
 * @root_x_nw: new x
 * @root_y_nw: new y
 ...
 */
void
cobiwm_window_move_resize_frame (CobiwmWindow  *window,
                               gboolean     user_op,
                               int          root_x_nw,
                               int          root_y_nw,
                               int          w,
...
{
  CobiwmMoveResizeFlags flags;
  CobiwmRectangle rect = { root_x_nw, root_y_nw, w, h };

  g_return_if_fail (!window->override_redirect);

  flags = (user_op ? COBIWM_MOVE_RESIZE_USER_ACTION : 0) | COBIWM_MOVE_RESIZE_MOVE_ACTION | COBIWM_MOVE_RESIZE_RESIZE_ACTION;

  cobiwm_window_move_resize_internal (window, flags, NorthWestGravity, rect);
}

/**
 * cobiwm_window_move_to_monitor:
 * @window: a #CobiwmWindow
 ...
 */
void
cobiwm_window_move_to_monitor (CobiwmWindow  *window,
                             int          monitor)
{
  CobiwmRectangle old_area, new_area;

  if (monitor == window->monitor->number)
    return;

  cobiwm_window_get_work_area_for_monitor (window,
  ...
  cobiwm_window_move_between_rects (window, &old_area, &new_area);
  window->preferred_output_winsys_id = window->monitor->winsys_id;
...
void
cobiwm_window_resize_frame_with_gravity (CobiwmWindow *window,
                                       gboolean     user_op,
                                       int          w,
                                       int          h,
                                       int          gravity)
{
  ...
  flags = (user_op ? COBIWM_MOVE_RESIZE_USER_ACTION : 0) | COBIWM_MOVE_RESIZE_RESIZE_ACTION;
  cobiwm_window_move_resize_internal (window, flags, gravity, rect);
}

static void
cobiwm_window_move_resize_now (CobiwmWindow  *window)
{
  cobiwm_window_move_resize_frame (window, FALSE,
...
idle_move_resize (gpointer data)
{
  ...
  cobiwm_topic (COBIWM_DEBUG_GEOMETRY, "Clearing the move_resize queue\n");
...
void
cobiwm_window_get_gravity_position (CobiwmWindow  *window,
                                  int          gravity,
                                  int         *root_x,
                                  int         *root_y)
{
  CobiwmRectangle frame_extents;
  int w, h;
  int x, y;
...
void
cobiwm_window_get_session_geometry (CobiwmWindow  *window,
                                  int         *x,
                                  int         *y,
                                  int         *width,
                                  int         *height)
...
/**
 * cobiwm_window_get_buffer_rect:
 * @window: a #CobiwmWindow
 * @rect: (out): pointer to an allocated #CobiwmRectangle
 *
 ...
 */
void
cobiwm_window_get_buffer_rect (const CobiwmWindow *window,
                             CobiwmRectangle    *rect)
{
...
/**
 * cobiwm_window_client_rect_to_frame_rect:
 * @window: a #CobiwmWindow
 ...
 */
void
cobiwm_window_client_rect_to_frame_rect (CobiwmWindow    *window,
                                       CobiwmRectangle *client_rect,
                                       CobiwmRectangle *frame_rect)
{
  if (!frame_rect)
...
/**
 * cobiwm_window_frame_rect_to_client_rect:
 * @window: a #CobiwmWindow
 ...
 */
void
cobiwm_window_frame_rect_to_client_rect (CobiwmWindow    *window,
                                       CobiwmRectangle *frame_rect,
                                       CobiwmRectangle *client_rect)
{
  if (!client_rect)
 ...
 */
void
cobiwm_window_get_frame_rect (const CobiwmWindow *window,
                            CobiwmRectangle    *rect)
{
  *rect = window->rect;
}
...
/**
 * cobiwm_window_get_client_area_rect:
 * @window: a #CobiwmWindow
 ...
 */
void
cobiwm_window_get_client_area_rect (const CobiwmWindow      *window,
...
void
cobiwm_window_get_titlebar_rect (CobiwmWindow    *window,
                               CobiwmRectangle *rect)
{
...
const char*
cobiwm_window_get_startup_id (CobiwmWindow *window)
{
  if (window->startup_id == NULL)
    {
      ...
      if (group != NULL)
        return cobiwm_group_get_startup_id (group);
    }
...
static CobiwmWindow*
get_modal_transient (CobiwmWindow *window)
{
  GSList *windows;
  GSList *tmp;
  CobiwmWindow *modal_transient;
  ...
  windows = cobiwm_display_list_windows (window->display, COBIWM_LIST_DEFAULT);
  tmp = windows;
  while (tmp != NULL)
    {
      CobiwmWindow *transient = tmp->data;

      if (transient->transient_for == modal_transient &&
          transient->type == COBIWM_WINDOW_MODAL_DIALOG)
        {
...
/* XXX COBIWM_EFFECT_FOCUS */
void
cobiwm_window_focus (CobiwmWindow  *window,
                   guint32      timestamp)
{
  CobiwmWindow *modal_transient;
  ...
  if (modal_transient != NULL &&
      !modal_transient->unmanaging)
    {
      ...
      if (!cobiwm_window_located_on_workspace (modal_transient, window->screen->active_workspace))
        cobiwm_window_change_workspace (modal_transient, window->screen->active_workspace);
      window = modal_transient;
    }

  cobiwm_window_flush_calc_showing (window);
  ...
  COBIWM_WINDOW_GET_CLASS (window)->focus (window, timestamp);

  if (window->wm_state_demands_attention)
    cobiwm_window_unset_demands_attention(window);

/*  cobiwm_effect_run_focus(window, NULL, NULL); */
}
...
static void
set_workspace_state (CobiwmWindow    *window,
                     gboolean       on_all_workspaces,
                     CobiwmWorkspace *workspace)
{
  ...
  if (window->workspace)
    cobiwm_workspace_remove_window (window->workspace, window);
  else if (window->on_all_workspaces)
    {
      GList *l;
      for (l = window->screen->workspaces; l != NULL; l = l->next)
        {
          CobiwmWorkspace *ws = l->data;
          cobiwm_workspace_remove_window (ws, window);
        }
    }
  ...
  if (window->workspace)
    cobiwm_workspace_add_window (window->workspace, window);
  else if (window->on_all_workspaces)
    {
      GList *l;
      for (l = window->screen->workspaces; l != NULL; l = l->next)
        {
          CobiwmWorkspace *ws = l->data;
          cobiwm_workspace_add_window (ws, window);
        }
    }
  ...
  if (!window->override_redirect)
    cobiwm_window_queue (window, COBIWM_QUEUE_MOVE_RESIZE);
  cobiwm_window_queue (window, COBIWM_QUEUE_CALC_SHOWING);
  cobiwm_window_current_workspace_changed (window);
...
static gboolean
should_be_on_all_workspaces (CobiwmWindow *window)
{
  return
    window->always_sticky ||
    window->on_all_workspaces_requested ||
    window->override_redirect ||
    (cobiwm_prefs_get_workspaces_only_on_primary () &&
     !window->unmanaging &&
     !cobiwm_window_is_on_primary_monitor (window));
}

void
cobiwm_window_on_all_workspaces_changed (CobiwmWindow *window)
{
...
static void
cobiwm_window_change_workspace_without_transients (CobiwmWindow    *window,
                                                 CobiwmWorkspace *workspace)
{
...
static gboolean
change_workspace_foreach (CobiwmWindow *window,
                          void       *data)
{
  cobiwm_window_change_workspace_without_transients (window, data);
  return TRUE;
}

void
cobiwm_window_change_workspace (CobiwmWindow    *window,
                              CobiwmWorkspace *workspace)
{
  g_return_if_fail (!window->override_redirect);

  cobiwm_window_change_workspace_without_transients (window, workspace);

  cobiwm_window_foreach_transient (window, change_workspace_foreach,
                                 workspace);
  cobiwm_window_foreach_ancestor (window, change_workspace_foreach,
                                workspace);
}

static void
window_stick_impl (CobiwmWindow  *window)
{
  cobiwm_verbose ("Sticking window %s current on_all_workspaces = %d\n",
...
static void
window_unstick_impl (CobiwmWindow  *window)
{
  if (!window->on_all_workspaces_requested)
    return;
...
static gboolean
stick_foreach_func (CobiwmWindow *window,
                    void       *data)
{
  gboolean stick;
...
void
cobiwm_window_stick (CobiwmWindow  *window)
{
  gboolean stick = TRUE;
...
void
cobiwm_window_unstick (CobiwmWindow  *window)
{
...
void
cobiwm_window_current_workspace_changed (CobiwmWindow *window)
{
  COBIWM_WINDOW_GET_CLASS (window)->current_workspace_changed (window);
}

static gboolean
find_root_ancestor (CobiwmWindow *window,
                    void       *data)
{
  CobiwmWindow **ancestor = data;

  /* Overwrite the previously "most-root" ancestor with the new one found */
  *ancestor = window;

  /* We want this to continue until cobiwm_window_foreach_ancestor quits because
...
/**
 * cobiwm_window_find_root_ancestor:
 * @window: a #CobiwmWindow
 *
 ...
 */
CobiwmWindow *
cobiwm_window_find_root_ancestor (CobiwmWindow *window)
{
  CobiwmWindow *ancestor;
  ancestor = window;
  cobiwm_window_foreach_ancestor (window, find_root_ancestor, &ancestor);
  return ancestor;
}

void
cobiwm_window_raise (CobiwmWindow  *window)
{
  CobiwmWindow *ancestor;
  ...
  if (window->screen->stack == ancestor->screen->stack)
    cobiwm_stack_raise (window->screen->stack, ancestor);
  else
    {
      cobiwm_warning (
   ...
   */
  if (window != ancestor)
    cobiwm_stack_raise (window->screen->stack, window);

  g_signal_emit (window, window_signals[RAISED], 0);
}

void
cobiwm_window_lower (CobiwmWindow  *window)
{
  g_return_if_fail (!window->override_redirect);
  ...
  cobiwm_stack_lower (window->screen->stack, window);
}
 ...
 */
void
cobiwm_window_change_workspace_by_index (CobiwmWindow *window,
                                       gint        space_index,
                                       gboolean    append)
{
  CobiwmWorkspace *workspace;
  ...
  workspace =
    cobiwm_screen_get_workspace_by_index (screen, space_index);

  if (!workspace && append)
    workspace = cobiwm_screen_append_new_workspace (screen, FALSE, CurrentTime);
...
static void
cobiwm_window_appears_focused_changed (CobiwmWindow *window)
{
  set_net_wm_state (window);
  cobiwm_window_frame_size_changed (window);
...
static gboolean
should_propagate_focus_appearance (CobiwmWindow *window)
{
...
/**
 * cobiwm_window_propagate_focus_appearance:
 ...
 *
 * Adjusts the value of #CobiwmWindow:appears-focused on @window's
 ...
 */
static void
cobiwm_window_propagate_focus_appearance (CobiwmWindow *window,
                                        gboolean    focused)
{
  CobiwmWindow *child, *parent, *focus_window;

  focus_window = window->display->focus_window;

  child = window;
  parent = cobiwm_window_get_transient_for (child);
  while (parent && (!focused || should_propagate_focus_appearance (child)))
    {
      ...
      if (child_focus_state_changed && !parent->has_focus)
        {
          cobiwm_window_appears_focused_changed (parent);
        }
...
void
cobiwm_window_set_focused_internal (CobiwmWindow *window,
                                  gboolean    focused)
{
  if (focused)
    {
      ...
      if (!window->attached_focus_window)
        cobiwm_window_appears_focused_changed (window);

      cobiwm_window_propagate_focus_appearance (window, TRUE);
    }
  else
    {
      window->has_focus = FALSE;
...
/**
 * cobiwm_window_get_icon_geometry:
 * @window: a #CobiwmWindow
 ...
 */
gboolean
cobiwm_window_get_icon_geometry (CobiwmWindow    *window,
                               CobiwmRectangle *rect)
{
...
/**
 * cobiwm_window_set_icon_geometry:
 * @window: a #CobiwmWindow
 ...
 */
void
cobiwm_window_set_icon_geometry (CobiwmWindow    *window,
                               CobiwmRectangle *rect)
{
  if (rect)
    {
...
static void
redraw_icon (CobiwmWindow *window)
{
...
load_default_window_icon (int size)
{
  ...
  if (gtk_icon_theme_has_icon (theme, COBIWM_DEFAULT_ICON_NAME))
    icon_name = COBIWM_DEFAULT_ICON_NAME;
  else
...
get_default_window_icon (void)
{
  ...
  if (default_icon == NULL)
    {
      default_icon = load_default_window_icon (COBIWM_ICON_WIDTH);
...
get_default_mini_icon (void)
{
  ...
  if (default_icon == NULL)
    {
      default_icon = load_default_window_icon (COBIWM_MINI_ICON_WIDTH);
      g_assert (default_icon);
    }
...
static void
cobiwm_window_update_icon_now (CobiwmWindow *window,
                             gboolean    force)
{
  gboolean changed;
  ...
  changed = COBIWM_WINDOW_GET_CLASS (window)->update_icon (window, &icon, &mini_icon);
...
idle_update_icon (gpointer data)
{
  ...
  cobiwm_topic (COBIWM_DEBUG_GEOMETRY, "Clearing the update_icon queue\n");
  ...
  while (tmp != NULL)
    {
      ...
      cobiwm_window_update_icon_now (window, FALSE);
      window->is_in_queues &= ~COBIWM_QUEUE_UPDATE_ICON;
...
GList*
cobiwm_window_get_workspaces (CobiwmWindow *window)
{
  if (window->on_all_workspaces)
...
static void
invalidate_work_areas (CobiwmWindow *window)
{
  GList *tmp;

  tmp = cobiwm_window_get_workspaces (window);

  while (tmp != NULL)
    {
      cobiwm_workspace_invalidate_work_area (tmp->data);
      tmp = tmp->next;
    }
}
...
{
  if (COBIWM_WINDOW_GET_CLASS (window)->update_struts (window))
    invalidate_work_areas (window);
}

static void
cobiwm_window_type_changed (CobiwmWindow *window)
{
...
void
cobiwm_window_set_type (CobiwmWindow     *window,
                      CobiwmWindowType  type)
{
  ...
  window->type = type;
  cobiwm_window_type_changed (window);
}

void
cobiwm_window_frame_size_changed (CobiwmWindow *window)
{
  if (window->frame)
    cobiwm_frame_clear_cached_borders (window->frame);
}

static void
cobiwm_window_get_default_skip_hints (CobiwmWindow *window,
                                    gboolean   *skip_taskbar_out,
                                    gboolean   *skip_pager_out)
{
  COBIWM_WINDOW_GET_CLASS (window)->get_default_skip_hints (window, skip_taskbar_out, skip_pager_out);
}

static void
cobiwm_window_recalc_skip_features (CobiwmWindow *window)
{
  switch (window->type)
    {
    ...
    case COBIWM_WINDOW_NORMAL:
      {
        ...
        cobiwm_window_get_default_skip_hints (window, &skip_taskbar_hint, &skip_pager_hint);
...
void
cobiwm_window_recalc_features (CobiwmWindow *window)
{
  ...
  if (window->size_hints.min_width == window->size_hints.max_width &&
  ...
  else if (!window->mwm_has_resize_func)
    {
      ...
      cobiwm_warning ("Window %s sets an MWM hint indicating it isn't resizable, but sets min size %d x %d and max size %d x %d; this doesn't make much sense.\n",
                    window->desc,
  ...
  if (window->type == COBIWM_WINDOW_TOOLBAR)
    window->decorated = FALSE;
  ...
  if (window->override_redirect ||
      cobiwm_window_get_frame_type (window) == COBIWM_FRAME_TYPE_LAST)
    {
      window->decorated = FALSE;
      window->has_close_func = FALSE;
      window->has_shade_func = FALSE;
  ...
  if (window->type != COBIWM_WINDOW_NORMAL)
    {
  ...
  if (window->has_maximize_func)
    {
      CobiwmRectangle work_area, client_rect;

      cobiwm_window_get_work_area_current_monitor (window, &work_area);
      cobiwm_window_frame_rect_to_client_rect (window, &work_area, &client_rect);
  ...
  cobiwm_window_recalc_skip_features (window);
...
void
cobiwm_window_show_menu (CobiwmWindow         *window,
                       CobiwmWindowMenuType  menu,
                       int                 x,
                       int                 y)
{
  g_return_if_fail (!window->override_redirect);
  cobiwm_compositor_show_window_menu (window->display->compositor, window, menu, x, y);
}

void
cobiwm_window_show_menu_for_rect (CobiwmWindow         *window,
                                CobiwmWindowMenuType  menu,
                                CobiwmRectangle      *rect)
{
  g_return_if_fail (!window->override_redirect);
  cobiwm_compositor_show_window_menu_for_rect (window->display->compositor, window, menu, rect);
}

void
cobiwm_window_shove_titlebar_onscreen (CobiwmWindow *window)
{
  CobiwmRectangle  frame_rect;
  ...
  cobiwm_rectangle_expand_region (onscreen_region,
                                horiz_amount,
                                horiz_amount,
                                0,
                                vert_amount);
  cobiwm_rectangle_shove_into_region(onscreen_region,
                                   FIXED_DIRECTION_X,
                                   &frame_rect);
  cobiwm_rectangle_expand_region (onscreen_region,
                                -horiz_amount,
  ...
  cobiwm_window_move_frame (window, FALSE, frame_rect.x, frame_rect.y);
}

gboolean
cobiwm_window_titlebar_is_onscreen (CobiwmWindow *window)
{
  CobiwmRectangle  titlebar_rect, frame_rect;
  GList         *onscreen_region;
  ...
  cobiwm_window_get_titlebar_rect (window, &titlebar_rect);
  ...
  while (onscreen_region)
    {
      CobiwmRectangle *spanning_rect = onscreen_region->data;
      CobiwmRectangle overlap;

      cobiwm_rectangle_intersect (&titlebar_rect, spanning_rect, &overlap);
...
static gboolean
check_moveresize_frequency (CobiwmWindow *window,
			    gdouble    *remaining)
{
...
static void
update_move (CobiwmWindow  *window,
             gboolean     snap,
             int          x,
             int          y)
{
  ...
  cobiwm_verbose ("x,y = %d,%d anchor ptr %d,%d anchor pos %d,%d dx,dy %d,%d\n",
                x, y,
...
#define DRAG_THRESHOLD_TO_SHAKE_THRESHOLD_FACTOR 6
  shake_threshold = cobiwm_prefs_get_drag_threshold () *
  ...
  else if (cobiwm_prefs_get_edge_tiling () &&
           !COBIWM_WINDOW_MAXIMIZED (window) &&
           !COBIWM_WINDOW_TILED_SIDE_BY_SIDE (window))
    {
       ...
       *
       * The "current" idea of cobiwm_window_get_work_area_current_monitor() and
       * cobiwm_screen_get_current_monitor() is slightly different: the former
       * refers to the monitor which contains the largest part of the window,
       * the latter to the one where the pointer is located.
       */
      monitor = cobiwm_screen_get_current_monitor_info_for_pos (window->screen, x, y);
      cobiwm_window_get_work_area_for_monitor (window,
                                             monitor->number,
                                             &work_area);
       ...
       */
      if (cobiwm_window_can_tile_side_by_side (window) &&
          x >= monitor->rect.x && x < (work_area.x + shake_threshold))
        window->tile_mode = COBIWM_TILE_LEFT;
      else if (cobiwm_window_can_tile_side_by_side (window) &&
               x >= work_area.x + work_area.width - shake_threshold &&
               x < (monitor->rect.x + monitor->rect.width))
        window->tile_mode = COBIWM_TILE_RIGHT;
      else if (cobiwm_window_can_tile_maximized (window) &&
               y >= monitor->rect.y && y <= work_area.y)
        window->tile_mode = COBIWM_TILE_MAXIMIZED;
      else
        window->tile_mode = COBIWM_TILE_NONE;
  ...
  if ((COBIWM_WINDOW_MAXIMIZED (window) && ABS (dy) >= shake_threshold) ||
      (COBIWM_WINDOW_TILED_SIDE_BY_SIDE (window) && (MAX (ABS (dx), ABS (dy)) >= shake_threshold)))
    {
      double prop;
       ...
       */
      window->shaken_loose = !cobiwm_prefs_get_edge_tiling ();
      window->tile_mode = COBIWM_TILE_NONE;
   ...
   */
  else if ((window->shaken_loose || COBIWM_WINDOW_MAXIMIZED (window)) &&
           window->tile_mode != COBIWM_TILE_LEFT && window->tile_mode != COBIWM_TILE_RIGHT)
    {
      const CobiwmMonitorInfo *wmonitor;
      CobiwmRectangle work_area;
      int monitor;
      ...
      for (monitor = 0; monitor < window->screen->n_monitor_infos; monitor++)
        {
          cobiwm_window_get_work_area_for_monitor (window, monitor, &work_area);
   ...
   */
  cobiwm_screen_update_tile_preview (window->screen,
                                   window->tile_mode != COBIWM_TILE_NONE);

  cobiwm_window_get_frame_rect (window, &old);

  /* Don't allow movement in the maximized directions or while tiled */
  if (window->maximized_horizontally || COBIWM_WINDOW_TILED_SIDE_BY_SIDE (window))
    new_x = old.x;
  if (window->maximized_vertically)
    new_y = old.y;
  ...
  cobiwm_window_move_frame (window, TRUE, new_x, new_y);
}
...
static void
update_resize (CobiwmWindow *window,
               gboolean    snap,
               int x, int y,
               gboolean force)
{
  ...
  if (window->display->grab_op == COBIWM_GRAB_OP_KEYBOARD_RESIZING_UNKNOWN)
    {
      CobiwmGrabOp op = COBIWM_GRAB_OP_WINDOW_BASE | COBIWM_GRAB_OP_WINDOW_FLAG_KEYBOARD;
  ...
  if (window->display->grab_op & COBIWM_GRAB_OP_WINDOW_DIR_EAST)
    new_w += dx;
  else if (window->display->grab_op & COBIWM_GRAB_OP_WINDOW_DIR_WEST)
    new_w -= dx;

  if (window->display->grab_op & COBIWM_GRAB_OP_WINDOW_DIR_SOUTH)
    new_h += dy;
  else if (window->display->grab_op & COBIWM_GRAB_OP_WINDOW_DIR_NORTH)
    new_h -= dy;
  ...
  if ((window->display->grab_op & (COBIWM_GRAB_OP_WINDOW_DIR_WEST | COBIWM_GRAB_OP_WINDOW_DIR_EAST)) == 0)
    new_w = old.width;

  if ((window->display->grab_op & (COBIWM_GRAB_OP_WINDOW_DIR_NORTH | COBIWM_GRAB_OP_WINDOW_DIR_SOUTH)) == 0)
    new_h = old.height;
  ...
  cobiwm_window_resize_frame_with_gravity (window, TRUE, new_w, new_h, gravity);
...
static void
update_tile_mode (CobiwmWindow *window)
{
...
void
cobiwm_window_update_resize (CobiwmWindow *window,
                           gboolean    snap,
                           int x, int y,
                           gboolean force)
{
...
static void
end_grab_op (CobiwmWindow *window,
             const ClutterEvent *event)
{
  ...
  cobiwm_display_check_threshold_reached (window->display, x, y);
  ...
  if (!window->display->grab_last_user_action_was_snap)
    {
      ...
      else if (cobiwm_grab_op_is_resizing (window->display->grab_op))
        {
          update_resize (window,
                         modifiers & CLUTTER_SHIFT_MASK,
                         x, y,
  ...
  cobiwm_display_end_grab_op (window->display, clutter_event_get_time (event));
}

gboolean
cobiwm_window_handle_mouse_grab_op_event  (CobiwmWindow         *window,
                                         const ClutterEvent *event)
{
  ...
  switch (event->type)
    ...
    case CLUTTER_TOUCH_END:
      if (cobiwm_display_is_pointer_emulating_sequence (window->display, sequence))
        end_grab_op (window, event);
    ...
    case CLUTTER_BUTTON_RELEASE:
      if (event->button.button == 1 ||
          event->button.button == (unsigned int) cobiwm_prefs_get_mouse_button_resize ())
        end_grab_op (window, event);
...
static void
get_work_area_monitor (CobiwmWindow    *window,
                       ...
                       int            which_monitor)
{
  ...
  *area = window->screen->monitor_infos[which_monitor].rect;
  ...
  while (tmp != NULL)
    {
      CobiwmRectangle workspace_work_area;
      cobiwm_workspace_get_work_area_for_monitor (tmp->data,
                                                which_monitor,
                                                &workspace_work_area);
      cobiwm_rectangle_intersect (area,
                                &workspace_work_area,
                                area);
      tmp = tmp->next;
    }
...
/**
 * cobiwm_window_get_work_area_current_monitor:
 * @window: a #CobiwmWindow
 ...
 */
void
cobiwm_window_get_work_area_current_monitor (CobiwmWindow    *window,
                                           CobiwmRectangle *area)
{
  cobiwm_window_get_work_area_for_monitor (window,
                                         window->monitor->number,
...
/**
 * cobiwm_window_get_work_area_for_monitor:
 * @window: a #CobiwmWindow
 ...
 */
void
cobiwm_window_get_work_area_for_monitor (CobiwmWindow    *window,
                                       int            which_monitor,
                                       CobiwmRectangle *area)
{
...
/**
 * cobiwm_window_get_work_area_all_monitors:
 * @window: a #CobiwmWindow
 * @area: (out): a location to store the work area
 *
 ...
 */
void
cobiwm_window_get_work_area_all_monitors (CobiwmWindow    *window,
                                        CobiwmRectangle *area)
{
  GList *tmp;
  ...
  *area = window->screen->rect;
  ...
  while (tmp != NULL)
    {
      CobiwmRectangle workspace_work_area;
      cobiwm_workspace_get_work_area_all_monitors (tmp->data,
                                                 &workspace_work_area);
      cobiwm_rectangle_intersect (area,
                                &workspace_work_area,
                                area);
...
int
cobiwm_window_get_current_tile_monitor_number (CobiwmWindow *window)
{
  int tile_monitor_number = window->tile_monitor_number;

  if (tile_monitor_number < 0)
    {
      cobiwm_warning ("%s called with an invalid monitor number; using 0 instead\n", G_STRFUNC);
...
void
cobiwm_window_get_current_tile_area (CobiwmWindow    *window,
                                   CobiwmRectangle *tile_area)
{
  int tile_monitor_number;

  g_return_if_fail (window->tile_mode != COBIWM_TILE_NONE);

  tile_monitor_number = cobiwm_window_get_current_tile_monitor_number (window);

  cobiwm_window_get_work_area_for_monitor (window, tile_monitor_number, tile_area);

  if (window->tile_mode == COBIWM_TILE_LEFT  ||
      window->tile_mode == COBIWM_TILE_RIGHT)
    tile_area->width /= 2;

  if (window->tile_mode == COBIWM_TILE_RIGHT)
    tile_area->x += tile_area->width;
}

gboolean
cobiwm_window_same_application (CobiwmWindow *window,
                              CobiwmWindow *other_window)
{
  CobiwmGroup *group       = cobiwm_window_get_group (window);
  CobiwmGroup *other_group = cobiwm_window_get_group (other_window);
...
/**
 * cobiwm_window_is_client_decorated:
 *
 * Check if if the window has decorations drawn by the client.
 * (window->decorated refers only to whether we should add decorations)
 */
gboolean
cobiwm_window_is_client_decorated (CobiwmWindow *window)
{
  if (window->client_type == COBIWM_WINDOW_CLIENT_TYPE_WAYLAND)
    {
...
/**
 * cobiwm_window_foreach_transient:
 * @window: a #CobiwmWindow
 ...
 */
void
cobiwm_window_foreach_transient (CobiwmWindow            *window,
  ...
  while (tmp != NULL)
    {
      ...
      if (cobiwm_window_is_ancestor_of_transient (window, transient))
        {
...
/**
 * cobiwm_window_foreach_ancestor:
 * @window: a #CobiwmWindow
 ...
 */
void
cobiwm_window_foreach_ancestor (CobiwmWindow            *window,
                              CobiwmWindowForeachFunc  func,
                              void                  *user_data)
{
  CobiwmWindow *w;
...
static gboolean
find_ancestor_func (CobiwmWindow *window,
                    void       *data)
{
...
/**
 * cobiwm_window_is_ancestor_of_transient:
 * @window: a #CobiwmWindow
 * @transient: a #CobiwmWindow
 *
 ...
 */
gboolean
cobiwm_window_is_ancestor_of_transient (CobiwmWindow *window,
                                      CobiwmWindow *transient)
{
  ...
  cobiwm_window_foreach_ancestor (transient, find_ancestor_func, &d);
 ...
 */
static gboolean
warp_grab_pointer (CobiwmWindow          *window,
                   ...
                   int                 *y)
{
  ...
  if (grab_op & COBIWM_GRAB_OP_WINDOW_DIR_WEST)
    *x = 0;
  else if (grab_op & COBIWM_GRAB_OP_WINDOW_DIR_EAST)
    *x = rect.width - 1;
  else
    *x = rect.width / 2;

  if (grab_op & COBIWM_GRAB_OP_WINDOW_DIR_NORTH)
    *y = 0;
  else if (grab_op & COBIWM_GRAB_OP_WINDOW_DIR_SOUTH)
    *y = rect.height - 1;
  else
    *y = rect.height / 2;
  ...
  *y = CLAMP (*y, 0, window->screen->rect.height-1);
  ...
  display->grab_latest_motion_x = *x;
  display->grab_latest_motion_y = *y;
  cobiwm_window_get_frame_rect (window,
                              &display->grab_anchor_window_pos);

  {
    CobiwmBackend *backend = cobiwm_get_backend ();
    cobiwm_backend_warp_pointer (backend, *x, *y);
  }

  if (cobiwm_error_trap_pop_with_return (display) != Success)
    {
      cobiwm_verbose ("Failed to warp pointer for window %s\n",
                    window->desc);
      return FALSE;
    }
...
void
cobiwm_window_begin_grab_op (CobiwmWindow *window,
                           CobiwmGrabOp  op,
...
void
cobiwm_window_update_keyboard_resize (CobiwmWindow *window,
                                    gboolean    update_cursor)
{
  ...
  if (update_cursor)
    cobiwm_display_update_cursor (window->display);
}

void
cobiwm_window_update_keyboard_move (CobiwmWindow *window)
{
  int x, y;
...
void
cobiwm_window_update_layer (CobiwmWindow *window)
{
  CobiwmGroup *group;
  ...
  if (group)
    ...
    cobiwm_stack_update_layer (window->screen->stack, window);
  cobiwm_stack_thaw (window->screen->stack);
}
 ...
 */
static void
ensure_mru_position_after (CobiwmWindow *window,
                           CobiwmWindow *after_this_one)
{
...
void
cobiwm_window_stack_just_below (CobiwmWindow *window,
  ...
  if (window->stack_position > below_this_one->stack_position)
    {
      cobiwm_topic (COBIWM_DEBUG_STACK,
                  ...
                  below_this_one->desc);
      cobiwm_window_set_stack_position (window, below_this_one->stack_position);
    }
  else
    {
      cobiwm_topic (COBIWM_DEBUG_STACK,
 ...
 *
 * The property is set by Cobiwm initially upon window creation,
 * and updated thereafter on input events (key and button presses) seen by Cobiwm,
 ...
 */
guint32
cobiwm_window_get_user_time (CobiwmWindow *window)
{
  return window->net_wm_user_time;
}

void
cobiwm_window_set_user_time (CobiwmWindow *window,
                           guint32     timestamp)
{
  ...
  if (window->net_wm_user_time_set &&
  ...
  else
    {
       ...
       */
      if (cobiwm_prefs_get_focus_new_windows () == G_DESKTOP_FOCUS_NEW_WINDOWS_STRICT &&
...
/**
 * cobiwm_window_get_stable_sequence:
 * @window: A #CobiwmWindow
 *
 * The stable sequence number is a monotonicially increasing
 * unique integer assigned to each #CobiwmWindow upon creation.
 *
 ...
 */
guint32
cobiwm_window_get_stable_sequence (CobiwmWindow *window)
{
  g_return_val_if_fail (COBIWM_IS_WINDOW (window), 0);
 ...
 */
void
cobiwm_window_set_demands_attention (CobiwmWindow *window)
{
  CobiwmRectangle candidate_rect, other_rect;
  GList *stack = window->screen->stack->sorted;
  CobiwmWindow *other_window;
  gboolean obscured = FALSE;

  CobiwmWorkspace *workspace = window->screen->active_workspace;
  ...
  if (!cobiwm_window_located_on_workspace (window, workspace))
    ...
    {
      cobiwm_window_get_frame_rect (window, &candidate_rect);
      ...
      while (stack != NULL && stack->data != window)
        {
          ...
          if (cobiwm_window_located_on_workspace (other_window, workspace))
            {
              cobiwm_window_get_frame_rect (other_window, &other_rect);

              if (cobiwm_rectangle_overlap (&candidate_rect, &other_rect))
                {
                  obscured = TRUE;
                  break;
                }
...
void
cobiwm_window_unset_demands_attention (CobiwmWindow *window)
{
  cobiwm_topic (COBIWM_DEBUG_WINDOW_OPS,
...
/**
 * cobiwm_window_get_frame: (skip)
 * @window: a #CobiwmWindow
 *
 */
CobiwmFrame *
cobiwm_window_get_frame (CobiwmWindow *window)
{
  return window->frame;
}

/**
 * cobiwm_window_appears_focused:
 * @window: a #CobiwmWindow
 *
 ...
 */
gboolean
cobiwm_window_appears_focused (CobiwmWindow *window)
{
  return window->has_focus || (window->attached_focus_window != NULL);
}

gboolean
cobiwm_window_has_focus (CobiwmWindow *window)
{
  return window->has_focus;
}

gboolean
cobiwm_window_is_shaded (CobiwmWindow *window)
{
  return window->shaded;
}

/**
 * cobiwm_window_is_override_redirect:
 * @window: A #CobiwmWindow
 *
 * Returns: %TRUE if this window isn't managed by cobiwm; it will
 * control its own positioning and cobiwm won't draw decorations
 * among other things.  In X terminology this is "override redirect".
 */
gboolean
cobiwm_window_is_override_redirect (CobiwmWindow *window)
{
  return window->override_redirect;
}

/**
 * cobiwm_window_is_skip_taskbar:
 * @window: A #CobiwmWindow
 *
 ...
 */
gboolean
cobiwm_window_is_skip_taskbar (CobiwmWindow *window)
{
  g_return_val_if_fail (COBIWM_IS_WINDOW (window), FALSE);

  return window->skip_taskbar;
}

/**
 * cobiwm_window_get_screen:
 * @window: a #CobiwmWindow
 *
 * Gets the #CobiwmScreen that the window is on.
 *
 * Return value: (transfer none): the #CobiwmScreen for the window
 */
CobiwmScreen *
cobiwm_window_get_screen (CobiwmWindow *window)
{
  return window->screen;
}

/**
 * cobiwm_window_get_display:
 * @window: A #CobiwmWindow
 *
 * Returns: (transfer none): The display for @window
 */
CobiwmDisplay *
cobiwm_window_get_display (CobiwmWindow *window)
{
  return window->display;
}

/**
 * cobiwm_window_get_xwindow: (skip)
 * @window: a #CobiwmWindow
 *
 */
Window
cobiwm_window_get_xwindow (CobiwmWindow *window)
{
  return window->xwindow;
}

CobiwmWindowType
cobiwm_window_get_window_type (CobiwmWindow *window)
{
  return window->type;
}

/**
 * cobiwm_window_get_workspace:
 * @window: a #CobiwmWindow
 *
 * Gets the #CobiwmWorkspace that the window is currently displayed on.
 * If the window is on all workspaces, returns the currently active
 * workspace.
 *
 * Return value: (transfer none): the #CobiwmWorkspace for the window
 */
CobiwmWorkspace *
cobiwm_window_get_workspace (CobiwmWindow *window)
{
  if (window->on_all_workspaces)
...
gboolean
cobiwm_window_is_on_all_workspaces (CobiwmWindow *window)
{
  return window->on_all_workspaces;
}

gboolean
cobiwm_window_is_hidden (CobiwmWindow *window)
{
  return window->hidden;
}

const char *
cobiwm_window_get_description (CobiwmWindow *window)
{
  if (!window)
    return NULL;
...
/**
 * cobiwm_window_get_wm_class:
 * @window: a #CobiwmWindow
 *
 * Return the current value of the name part of WM_CLASS X property.
 */
const char *
cobiwm_window_get_wm_class (CobiwmWindow *window)
{
  if (!window)
    return NULL;
...
/**
 * cobiwm_window_get_wm_class_instance:
 * @window: a #CobiwmWindow
 *
 * Return the current value of the instance part of WM_CLASS X property.
 */
const char *
cobiwm_window_get_wm_class_instance (CobiwmWindow *window)
{
  if (!window)
    return NULL;
...
/**
 * cobiwm_window_get_gtk_theme_variant:
 * @window: a #CobiwmWindow
 *
 * Return value: (transfer none): the theme variant or %NULL
 **/
const char *
cobiwm_window_get_gtk_theme_variant (CobiwmWindow *window)
{
  return window->gtk_theme_variant;
}

/**
 * cobiwm_window_get_gtk_application_id:
 * @window: a #CobiwmWindow
 *
 * Return value: (transfer none): the application ID
 **/
const char *
cobiwm_window_get_gtk_application_id (CobiwmWindow *window)
{
  return window->gtk_application_id;
}

/**
 * cobiwm_window_get_gtk_unique_bus_name:
 * @window: a #CobiwmWindow
 *
 * Return value: (transfer none): the unique name
 **/
const char *
cobiwm_window_get_gtk_unique_bus_name (CobiwmWindow *window)
{
  return window->gtk_unique_bus_name;
}

/**
 * cobiwm_window_get_gtk_application_object_path:
 * @window: a #CobiwmWindow
 *
 * Return value: (transfer none): the object path
 **/
const char *
cobiwm_window_get_gtk_application_object_path (CobiwmWindow *window)
{
  return window->gtk_application_object_path;
}

/**
 * cobiwm_window_get_gtk_window_object_path:
 * @window: a #CobiwmWindow
 *
 * Return value: (transfer none): the object path
 **/
const char *
cobiwm_window_get_gtk_window_object_path (CobiwmWindow *window)
{
  return window->gtk_window_object_path;
}

/**
 * cobiwm_window_get_gtk_app_menu_object_path:
 * @window: a #CobiwmWindow
 *
 * Return value: (transfer none): the object path
 **/
const char *
cobiwm_window_get_gtk_app_menu_object_path (CobiwmWindow *window)
{
  return window->gtk_app_menu_object_path;
}

/**
 * cobiwm_window_get_gtk_menubar_object_path:
 * @window: a #CobiwmWindow
 *
 * Return value: (transfer none): the object path
 **/
const char *
cobiwm_window_get_gtk_menubar_object_path (CobiwmWindow *window)
{
  return window->gtk_menubar_object_path;
}

/**
 * cobiwm_window_get_compositor_private:
 * @window: a #CobiwmWindow
 *
 ...
 **/
GObject *
cobiwm_window_get_compositor_private (CobiwmWindow *window)
{
  if (!window)
    return NULL;
...
void
cobiwm_window_set_compositor_private (CobiwmWindow *window, GObject *priv)
{
  if (!window)
    return;
...
const char *
cobiwm_window_get_role (CobiwmWindow *window)
{
  if (!window)
    return NULL;
...
/**
 * cobiwm_window_get_title:
 * @window: a #CobiwmWindow
 *
 * Returns: the current title of the window.
 */
const char *
cobiwm_window_get_title (CobiwmWindow *window)
{
  g_return_val_if_fail (COBIWM_IS_WINDOW (window), NULL);

  return window->title;
}

CobiwmStackLayer
cobiwm_window_get_layer (CobiwmWindow *window)
{
  return window->layer;
}

/**
 * cobiwm_window_get_transient_for:
 * @window: a #CobiwmWindow
 *
 * Returns the #CobiwmWindow for the window that is pointed to by the
 * WM_TRANSIENT_FOR hint on this window (see XGetTransientForHint()
 * or XSetTransientForHint()). Cobiwmcity keeps transient windows above their
 ...
 * window that Cobiwmcity knows about.
 */
CobiwmWindow *
cobiwm_window_get_transient_for (CobiwmWindow *window)
{
  g_return_val_if_fail (COBIWM_IS_WINDOW (window), NULL);
  ...
  if (window->transient_for)
    ...
    return cobiwm_display_lookup_x_window (window->display,
 ...
 */
int
cobiwm_window_get_pid (CobiwmWindow *window)
{
  g_return_val_if_fail (COBIWM_IS_WINDOW (window), -1);

  return window->net_wm_pid;
}

/**
 * cobiwm_window_get_client_machine:
 * @window: a #CobiwmWindow
 *
 ...
 */
const char *
cobiwm_window_get_client_machine (CobiwmWindow *window)
{
  g_return_val_if_fail (COBIWM_IS_WINDOW (window), NULL);
...
/**
 * cobiwm_window_is_remote:
 * @window: a #CobiwmWindow
 *
 * Returns: %TRUE if this window originates from a host
 * different from the one running cobiwm.
 */
gboolean
cobiwm_window_is_remote (CobiwmWindow *window)
{
  return window->is_remote;
}

/**
 * cobiwm_window_get_cobiwm_hints:
 * @window: a #CobiwmWindow
 *
 * Gets the current value of the _COBIWM_HINTS property.
 *
 ...
 * use; 'cobiwm-' key prefix is reserved for internal use, and must not be used
 * by plugins.
 *
 * Return value: (transfer none): the _COBIWM_HINTS string, or %NULL if no hints
 * are set.
 */
const char *
cobiwm_window_get_cobiwm_hints (CobiwmWindow *window)
{
  g_return_val_if_fail (COBIWM_IS_WINDOW (window), NULL);

  return window->cobiwm_hints;
}

/**
 * cobiwm_window_get_frame_type:
 * @window: a #CobiwmWindow
 *
 ...
 */
CobiwmFrameType
cobiwm_window_get_frame_type (CobiwmWindow *window)
{
  CobiwmFrameType base_type = COBIWM_FRAME_TYPE_LAST;

  switch (window->type)
    {
    case COBIWM_WINDOW_NORMAL:
      base_type = COBIWM_FRAME_TYPE_NORMAL;
      break;

    case COBIWM_WINDOW_DIALOG:
      base_type = COBIWM_FRAME_TYPE_DIALOG;
      break;

    case COBIWM_WINDOW_MODAL_DIALOG:
      if (cobiwm_window_is_attached_dialog (window))
        base_type = COBIWM_FRAME_TYPE_ATTACHED;
      else
        base_type = COBIWM_FRAME_TYPE_MODAL_DIALOG;
      break;

    case COBIWM_WINDOW_MENU:
      base_type = COBIWM_FRAME_TYPE_MENU;
      break;

    case COBIWM_WINDOW_UTILITY:
      base_type = COBIWM_FRAME_TYPE_UTILITY;
      break;
    ...
    case COBIWM_WINDOW_OVERRIDE_OTHER:
      /* No frame */
      base_type = COBIWM_FRAME_TYPE_LAST;
      break;
    }

  if (base_type == COBIWM_FRAME_TYPE_LAST)
    {
      /* can't add border if undecorated */
      return COBIWM_FRAME_TYPE_LAST;
    }
  else if (window->border_only ||
           (window->hide_titlebar_when_maximized && COBIWM_WINDOW_MAXIMIZED (window)) ||
           (window->hide_titlebar_when_maximized && COBIWM_WINDOW_TILED_SIDE_BY_SIDE (window)))
    {
      /* override base frame type */
      return COBIWM_FRAME_TYPE_BORDER;
    }
  else
    {
      return base_type;
...
/**
 * cobiwm_window_get_frame_bounds:
 * @window: a #CobiwmWindow
 *
 ...
 */
cairo_region_t *
cobiwm_window_get_frame_bounds (CobiwmWindow *window)
{
  if (!window->frame_bounds)
    {
      if (window->frame)
        window->frame_bounds = cobiwm_frame_get_frame_bounds (window->frame);
    }
...
/**
 * cobiwm_window_is_attached_dialog:
 * @window: a #CobiwmWindow
 *
 ...
 */
gboolean
cobiwm_window_is_attached_dialog (CobiwmWindow *window)
{
  return window->attached;
}

/**
 * cobiwm_window_get_tile_match:
 * @window: a #CobiwmWindow
 *
 ...
 */
CobiwmWindow *
cobiwm_window_get_tile_match (CobiwmWindow *window)
{
  return window->tile_match;
}

void
cobiwm_window_compute_tile_match (CobiwmWindow *window)
{
  CobiwmWindow *match;
  CobiwmStack *stack;
  CobiwmTileMode match_tile_mode = COBIWM_TILE_NONE;
  ...
  if (COBIWM_WINDOW_TILED_LEFT (window))
    match_tile_mode = COBIWM_TILE_RIGHT;
  else if (COBIWM_WINDOW_TILED_RIGHT (window))
    match_tile_mode = COBIWM_TILE_LEFT;
  else
    return;

  stack = window->screen->stack;

  for (match = cobiwm_stack_get_top (stack);
       match;
       match = cobiwm_stack_get_below (stack, match, FALSE))
    {
      if (!match->shaded &&
          !match->minimized &&
          match->tile_mode == match_tile_mode &&
          match->monitor == window->monitor &&
          cobiwm_window_get_workspace (match) == cobiwm_window_get_workspace (window))
        break;
    }

  if (match)
    {
      CobiwmWindow *above, *bottommost, *topmost;
      CobiwmRectangle above_rect, bottommost_rect, topmost_rect;

      if (cobiwm_stack_windows_cmp (window->screen->stack, match, window) > 0)
        {
          topmost = match;
          bottommost = window;
        }
      ...
      cobiwm_window_get_frame_rect (bottommost, &bottommost_rect);
      cobiwm_window_get_frame_rect (topmost, &topmost_rect);
      /*
       ...
       */
      for (above = cobiwm_stack_get_above (stack, bottommost, FALSE);
           above && above != topmost;
           above = cobiwm_stack_get_above (stack, above, FALSE))
        {
          if (above->minimized ||
              above->monitor != window->monitor ||
              cobiwm_window_get_workspace (above) != cobiwm_window_get_workspace (window))
            continue;

          cobiwm_window_get_frame_rect (above, &above_rect);

          if (cobiwm_rectangle_overlap (&above_rect, &bottommost_rect) &&
              cobiwm_rectangle_overlap (&above_rect, &topmost_rect))
            return;
        }
...
void
cobiwm_window_set_title (CobiwmWindow *window,
                       const char *title)
{
  g_free (window->title);
  ...
  if (window->frame)
    cobiwm_frame_update_title (window->frame);
...
void
cobiwm_window_set_wm_class (CobiwmWindow *window,
                          const char *wm_class,
...
void
cobiwm_window_set_gtk_dbus_properties (CobiwmWindow *window,
...
static gboolean
check_transient_for_loop (CobiwmWindow *window,
                          CobiwmWindow *parent)
{
  while (parent)
    {
...
void
cobiwm_window_set_transient_for (CobiwmWindow *window,
                               CobiwmWindow *parent)
{
  if (check_transient_for_loop (window, parent))
    {
      cobiwm_warning ("Setting %s transient for %s would create a loop.\n",
                    window->desc, parent->desc);
      return;
    }

  if (cobiwm_window_appears_focused (window) && window->transient_for != NULL)
    cobiwm_window_propagate_focus_appearance (window, FALSE);
  ...
  if (!window->constructing)
    {
       ...
       * parents, we need to destroy the CobiwmWindow and let a new one
       * be created (which happens as a side effect of
       * cobiwm_window_unmanage()). The condition below is correct
       * because we know window->transient_for has changed.
       */
      if (window->attached || cobiwm_window_should_attach_to_parent (window))
        {
          guint32 timestamp;
  ...
  /* update stacking constraints */
  if (!window->override_redirect)
    cobiwm_stack_update_transient (window->screen->stack, window);
  ...
  if (window->xtransient_for != None &&
    ...
    cobiwm_window_group_leader_changed (window);
...
void
cobiwm_window_set_opacity (CobiwmWindow *window,
                         guint8      opacity)
{
  window->opacity = opacity;

  cobiwm_compositor_window_opacity_changed (window->display->compositor, window);
}

static void
reset_ignored_crossing_serials (CobiwmDisplay *display)
{
  int i;
...
static void
mouse_mode_focus (CobiwmWindow  *window,
                  guint32      timestamp)
{
  CobiwmDisplay *display = window->display;
  ...
  if (window->type != COBIWM_WINDOW_DESKTOP)
    {
      ...
      if (cobiwm_prefs_get_auto_raise ())
        cobiwm_display_queue_autoraise_callback (display, window);
      else
        cobiwm_topic (COBIWM_DEBUG_FOCUS, "Auto raise is disabled\n");
    }
  else
    {
       ...
       */
      if (cobiwm_prefs_get_focus_mode() == G_DESKTOP_FOCUS_MODE_MOUSE &&
...
static gboolean
window_has_pointer_wayland (CobiwmWindow *window)
{
...
static gboolean
window_has_pointer_x11 (CobiwmWindow *window)
{
  CobiwmDisplay *display = window->display;
  CobiwmScreen *screen = window->screen;
  Window root, child;
  ...
  return cobiwm_display_lookup_x_window (display, child) == window;
}

gboolean
cobiwm_window_has_pointer (CobiwmWindow *window)
{
...
{
  CobiwmFocusData *focus_data = data;
  CobiwmWindow *window = focus_data->window;
  CobiwmDisplay *display = window->display;
  CobiwmScreen *screen = window->screen;
  ...
  cobiwm_cursor_tracker_get_pointer (tracker, &root_x, &root_y, NULL);
...
static void
queue_focus_callback (CobiwmDisplay *display,
                      CobiwmWindow  *window,
                      int          pointer_x,
                      int          pointer_y)
{
  CobiwmFocusData *focus_data;

  focus_data = g_new (CobiwmFocusData, 1);
  ...
  g_source_set_name_by_id (display->focus_timeout_id,
                           "[cobiwm] window_focus_on_pointer_rest_callback");
}

void
cobiwm_window_handle_enter (CobiwmWindow  *window,
                          guint32      timestamp,
                          guint        root_x,
                          guint        root_y)
{
  ...
  switch (cobiwm_prefs_get_focus_mode ())
    {
    ...
    case G_DESKTOP_FOCUS_MODE_MOUSE:
      ...
      if (window->type != COBIWM_WINDOW_DOCK)
        {
          if (cobiwm_prefs_get_focus_change_on_pointer_rest())
...
void
cobiwm_window_handle_leave (CobiwmWindow *window)
{
  if (window->type == COBIWM_WINDOW_DOCK && !window->has_focus)
    cobiwm_window_lower (window);
}

void
cobiwm_window_handle_ungrabbed_event (CobiwmWindow         *window,
                                    const ClutterEvent *event)
{
  CobiwmDisplay *display = window->display;
  gboolean unmodified;
  ...
  if (window->frame && cobiwm_ui_frame_handle_event (window->frame->ui_frame, event))
    return;
  ...
  if (window->type != COBIWM_WINDOW_DOCK)
    {
      cobiwm_topic (COBIWM_DEBUG_FOCUS,
                  ...
                  window->desc, button);
      cobiwm_window_focus (window, event->any.time);
    }
  else
  ...
  grab_mods = cobiwm_display_get_window_grab_modifiers (display);
  ...
  else if (is_window_grab && (int) button == cobiwm_prefs_get_mouse_button_resize ())
    {
      if (window->has_resize_func)
        {
          ...
          if (op != COBIWM_GRAB_OP_WINDOW_BASE)
            cobiwm_display_begin_grab_op (display,
                                        window->screen,
                                        window,
                                        op,
                                        TRUE,
  ...
  else if (is_window_grab && (int) button == cobiwm_prefs_get_mouse_button_menu ())
    {
      if (cobiwm_prefs_get_raise_on_click ())
        cobiwm_window_raise (window);
      cobiwm_window_show_menu (window,
...
gboolean
cobiwm_window_can_maximize (CobiwmWindow *window)
{
  return window->has_maximize_func;
}

gboolean
cobiwm_window_can_minimize (CobiwmWindow *window)
{
  return window->has_minimize_func;
}

gboolean
cobiwm_window_can_shade (CobiwmWindow *window)
{
  return window->has_shade_func;
}

gboolean
cobiwm_window_can_close (CobiwmWindow *window)
{
  return window->has_close_func;
}

gboolean
cobiwm_window_is_always_on_all_workspaces (CobiwmWindow *window)
{
  return window->always_sticky;
}

gboolean
cobiwm_window_is_above (CobiwmWindow *window)
{
  return window->wm_state_above;
}

gboolean
cobiwm_window_allows_move (CobiwmWindow *window)
{
  return COBIWM_WINDOW_ALLOWS_MOVE (window);
}

gboolean
cobiwm_window_allows_resize (CobiwmWindow *window)
{
  return COBIWM_WINDOW_ALLOWS_RESIZE (window);
}

void
cobiwm_window_set_urgent (CobiwmWindow *window,
                        gboolean    urgent)
{
...
void
cobiwm_window_grab_op_began (CobiwmWindow *window,
                           CobiwmGrabOp  op)
{
  COBIWM_WINDOW_GET_CLASS (window)->grab_op_began (window, op);
}

void
cobiwm_window_grab_op_ended (CobiwmWindow *window,
                           CobiwmGrabOp  op)
{
  COBIWM_WINDOW_GET_CLASS (window)->grab_op_ended (window, op);
}

void
cobiwm_window_emit_size_changed (CobiwmWindow *window)
{
  g_signal_emit (window, window_signals[SIZE_CHANGED], 0);
}
