...
#include "wayland/cobiwm-pointer-confinement-wayland.h"
...
#include "wayland/cobiwm-wayland-seat.h"
#include "wayland/cobiwm-wayland-pointer.h"
#include "wayland/cobiwm-wayland-pointer-constraints.h"
...
struct _CobiwmPointerConfinementWayland
{
...
G_DEFINE_TYPE (CobiwmPointerConfinementWayland, cobiwm_pointer_confinement_wayland,
            ...
            CobiwmBorderMotionDirection blocking_directions)
{
  ...
  return &g_array_index (borders, CobiwmBorder, borders->len - 1);
}

static gint
compare_lines_x (gconstpointer a, gconstpointer b)
{
  const CobiwmBorder *border_a = a;
  const CobiwmBorder *border_b = b;
...
static void
add_non_overlapping_edges (CobiwmBox     *boxes,
                           ...
                           GArray      *borders)
{
  ...
  GArray *band_merge;
  CobiwmBorder *border;
  CobiwmBorder *prev_border;
  CobiwmBorder *new_border;
  ...
  for (i = band_below_start; i < band_below_end; i++)
    {
      ...
      add_border (band_merge, box->x1, box->y1, box->x2, box->y1,
                  COBIWM_BORDER_MOTION_DIRECTION_NEGATIVE_Y);
    }
  ...
  for (i = 0; i < band_merge->len; i++)
    {
      border = &g_array_index (band_merge, CobiwmBorder, i);
      ...
      if (prev_border &&
      ...
      else
        {
           ...
           */
          g_array_append_val (borders, *border);
          prev_border = &g_array_index (borders, CobiwmBorder, borders->len - 1);
        }
    }
...
static void
add_band_bottom_edges (CobiwmBox *boxes,
                       int      band_start,
                       int      band_end,
                       GArray  *borders)
{
                   ...
                   GArray         *borders)
{
  ...
  boxes = g_new (CobiwmBox, num_boxes);
  ...
  for (i = 0; i < num_boxes; i++)
    {
      ...
      add_border (borders,
                  boxes[i].x1, boxes[i].y1,
                  boxes[i].x1, boxes[i].y2,
                  COBIWM_BORDER_MOTION_DIRECTION_NEGATIVE_X);
                    ...
                    uint32_t   directions)
{
  ...
  CobiwmVector2 delta;
  float distance_2;
  CobiwmBorder *closest_border = NULL;
  float closest_distance_2 = DBL_MAX;
  unsigned int i;

  for (i = 0; i < borders->len; i++)
    {
      border = &g_array_index (borders, CobiwmBorder, i);

      if (!cobiwm_border_is_blocking_directions (border, directions))
        continue;

      if (!cobiwm_line2_intersects_with (&border->line, motion, &intersection))
        continue;

      delta = cobiwm_vector2_subtract (intersection, motion->a);
...
static void
clamp_to_border (CobiwmBorder *border,
                 CobiwmLine2  *motion,
                 uint32_t   *motion_dir)
{
  /*
   ...
   */
  if (cobiwm_border_is_horizontal (border))
    {
      if (*motion_dir & COBIWM_BORDER_MOTION_DIRECTION_POSITIVE_Y)
        motion->b.y = border->line.a.y - wl_fixed_to_double (1);
      else
        motion->b.y = border->line.a.y + wl_fixed_to_double (1) / 10;
      *motion_dir &= ~(COBIWM_BORDER_MOTION_DIRECTION_POSITIVE_Y |
                       COBIWM_BORDER_MOTION_DIRECTION_NEGATIVE_Y);
    }
  else
    {
      if (*motion_dir & COBIWM_BORDER_MOTION_DIRECTION_POSITIVE_X)
        motion->b.x = border->line.a.x - wl_fixed_to_double (1);
      else
        motion->b.x = border->line.a.x + wl_fixed_to_double (1) / 10;
      *motion_dir &= ~(COBIWM_BORDER_MOTION_DIRECTION_POSITIVE_X |
                       COBIWM_BORDER_MOTION_DIRECTION_NEGATIVE_X);
    }
}

static uint32_t
get_motion_directions (CobiwmLine2 *motion)
{
  uint32_t directions = 0;

  if (motion->a.x < motion->b.x)
    directions |= COBIWM_BORDER_MOTION_DIRECTION_POSITIVE_X;
  else if (motion->a.x > motion->b.x)
    directions |= COBIWM_BORDER_MOTION_DIRECTION_NEGATIVE_X;
  if (motion->a.y < motion->b.y)
    directions |= COBIWM_BORDER_MOTION_DIRECTION_POSITIVE_Y;
  else if (motion->a.y > motion->b.y)
    directions |= COBIWM_BORDER_MOTION_DIRECTION_NEGATIVE_Y;

  return directions;
}

static void
cobiwm_pointer_confinement_wayland_constrain (CobiwmPointerConstraint *constraint,
                                            ...
                                            float                  *y)
{
  CobiwmPointerConfinementWayland *self =
    COBIWM_POINTER_CONFINEMENT_WAYLAND (constraint);
  ...
  GArray *borders;
  CobiwmLine2 motion;
  CobiwmBorder *closest_border;
  uint32_t directions;

  surface = cobiwm_wayland_pointer_constraint_get_surface (self->constraint);

  cobiwm_wayland_surface_get_relative_coordinates (surface, *x, *y, &sx, &sy);
  cobiwm_wayland_surface_get_relative_coordinates (surface, prev_x, prev_y,
                                                 &prev_sx, &prev_sy);
  ...
  borders = g_array_new (FALSE, FALSE, sizeof (CobiwmBorder));
   ...
   */
  region =
    cobiwm_wayland_pointer_constraint_calculate_effective_region (self->constraint);
  ...
  cobiwm_wayland_surface_get_absolute_coordinates (surface,
...
static float
point_to_border_distance_2 (CobiwmBorder *border,
                            float       x,
                            float       y)
{
  float orig_x, orig_y;
...
static void
warp_to_behind_border (CobiwmBorder *border,
                       float      *sx,
                       float      *sy)
{
  switch (border->blocking_directions)
    {
    case COBIWM_BORDER_MOTION_DIRECTION_POSITIVE_X:
    case COBIWM_BORDER_MOTION_DIRECTION_NEGATIVE_X:
      if (border->blocking_directions == COBIWM_BORDER_MOTION_DIRECTION_POSITIVE_X)
      ...
      break;
    case COBIWM_BORDER_MOTION_DIRECTION_POSITIVE_Y:
    case COBIWM_BORDER_MOTION_DIRECTION_NEGATIVE_Y:
      if (border->blocking_directions == COBIWM_BORDER_MOTION_DIRECTION_POSITIVE_Y)
        *sy = border->line.a.y - wl_fixed_to_double (1);
      else
        *sy = border->line.a.y + wl_fixed_to_double (1);
...
static void
cobiwm_pointer_confinement_wayland_maybe_warp (CobiwmPointerConfinementWayland *self)
{
  ...
  seat = cobiwm_wayland_pointer_constraint_get_seat (self->constraint);
  surface = cobiwm_wayland_pointer_constraint_get_surface (self->constraint);

  clutter_input_device_get_coords (seat->pointer.device, NULL, &point);
  cobiwm_wayland_surface_get_relative_coordinates (surface,
                                                 point.x, point.y,
                                                 &sx, &sy);
  ...
  if (!cairo_region_contains_point (region, (int)sx, (int)sy))
    {
      ...
      for (i = 0; i < borders->len; i++)
        {
          CobiwmBorder *border = &g_array_index (borders, CobiwmBorder, i);
          float distance_2;
      ...
      cobiwm_wayland_surface_get_absolute_coordinates (surface, sx, sy, &x, &y);
      cobiwm_backend_warp_pointer (cobiwm_get_backend (), (int)x, (int)y);
    }
...
surface_actor_painting (CobiwmSurfaceActorWayland       *surface_actor,
                        CobiwmPointerConfinementWayland *self)
{
  cobiwm_pointer_confinement_wayland_maybe_warp (self);
}

CobiwmPointerConstraint *
cobiwm_pointer_confinement_wayland_new (CobiwmWaylandPointerConstraint *constraint)
{
  GObject *object;
  CobiwmPointerConfinementWayland *confinement;
  CobiwmWaylandSurface *surface;

  object = g_object_new (COBIWM_TYPE_POINTER_CONFINEMENT_WAYLAND, NULL);
  confinement = COBIWM_POINTER_CONFINEMENT_WAYLAND (object);

  confinement->constraint = constraint;

  surface = cobiwm_wayland_pointer_constraint_get_surface (constraint);
  ...
  return COBIWM_POINTER_CONSTRAINT (confinement);
}

static void
cobiwm_pointer_confinement_wayland_init (CobiwmPointerConfinementWayland *confinement_wayland)
{
}

static void
cobiwm_pointer_confinement_wayland_class_init (CobiwmPointerConfinementWaylandClass *klass)
{
  CobiwmPointerConstraintClass *pointer_constraint_class =
    COBIWM_POINTER_CONSTRAINT_CLASS (klass);

  pointer_constraint_class->constrain = cobiwm_pointer_confinement_wayland_constrain;
}
