...
G_DEFINE_TYPE (CobiwmWaylandOutput, cobiwm_wayland_output, G_TYPE_OBJECT)
...
wl_output_transform_from_cobiwm_monitor_transform (CobiwmMonitorTransform transform)
{
                    ...
                    gboolean            need_all_events)
{
  ...
  CobiwmOutput *output = monitor_info->outputs[0];
  enum wl_output_transform transform = wl_output_transform_from_cobiwm_monitor_transform (output->crtc->transform);
  guint mode_flags = WL_OUTPUT_MODE_CURRENT;

  CobiwmMonitorInfo *old_monitor_info = wayland_output->monitor_info;
...
bind_output (struct wl_client *client,
             ...
             guint32 id)
{
  CobiwmWaylandOutput *wayland_output = data;
  CobiwmMonitorInfo *monitor_info = wayland_output->monitor_info;
  ...
  cobiwm_verbose ("Binding monitor %p/%s (%u, %u, %u, %u) x %f\n",
...
static void
wayland_output_set_monitor_info (CobiwmWaylandOutput *wayland_output,
                                 CobiwmMonitorInfo   *monitor_info)
{
  CobiwmOutput *output = monitor_info->outputs[0];
  enum wl_output_transform transform = wl_output_transform_from_cobiwm_monitor_transform (output->crtc->transform);
...
static void
wayland_output_update_for_output (CobiwmWaylandOutput *wayland_output,
                                  CobiwmMonitorInfo *monitor_info)
{
  GList *iter;
  ...
  /* It's very important that we change the output pointer here, as
     the old structure is about to be freed by CobiwmMonitorManager */
  wayland_output_set_monitor_info (wayland_output, monitor_info);
}

static CobiwmWaylandOutput *
cobiwm_wayland_output_new (CobiwmWaylandCompositor *compositor,
                         CobiwmMonitorInfo *monitor_info)
{
  CobiwmWaylandOutput *wayland_output;

  wayland_output = g_object_new (COBIWM_TYPE_WAYLAND_OUTPUT, NULL);
  wayland_output->global = wl_global_create (compositor->wayland_display,
                                             &wl_output_interface,
                                             COBIWM_WL_OUTPUT_VERSION,
...
static GHashTable *
cobiwm_wayland_compositor_update_outputs (CobiwmWaylandCompositor *compositor,
                                        CobiwmMonitorManager    *monitors)
{
  ...
  monitor_infos = cobiwm_monitor_manager_get_monitor_infos (monitors, &n_monitor_infos);
  new_table = g_hash_table_new_full (NULL, NULL, NULL, wayland_output_destroy_notify);

  for (i = 0; i < n_monitor_infos; i++)
    {
      CobiwmMonitorInfo *info = &monitor_infos[i];
      CobiwmWaylandOutput *wayland_output;
      ...
      if (wayland_output)
        ...
        wayland_output = cobiwm_wayland_output_new (compositor, info);
...
{
  compositor->outputs = cobiwm_wayland_compositor_update_outputs (compositor, monitors);
}

static void
cobiwm_wayland_output_init (CobiwmWaylandOutput *wayland_output)
{
}

static void
cobiwm_wayland_output_finalize (GObject *object)
{
  CobiwmWaylandOutput *wayland_output = COBIWM_WAYLAND_OUTPUT (object);
  GList *l;

  wl_global_destroy (wayland_output->global);

  /* Make sure the wl_output destructor doesn't try to access CobiwmWaylandOutput
  ...
  G_OBJECT_CLASS (cobiwm_wayland_output_parent_class)->finalize (object);
}

static void
cobiwm_wayland_output_class_init (CobiwmWaylandOutputClass *klass)
{
  GObjectClass *object_class = G_OBJECT_CLASS (klass);

  object_class->finalize = cobiwm_wayland_output_finalize;
...
void
cobiwm_wayland_outputs_init (CobiwmWaylandCompositor *compositor)
{
  CobiwmMonitorManager *monitors;
