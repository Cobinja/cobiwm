...
struct _CobiwmWaylandTouchSurface
{
...
struct _CobiwmWaylandTouchInfo
{
  CobiwmWaylandTouchSurface *touch_surface;
  guint32 slot_serial;
  gint32 slot;
  gfloat start_x;
  gfloat start_y;
...
{
  CobiwmWaylandTouchSurface *touch_surface = data;
  CobiwmWaylandTouch *touch = touch_surface->touch;
...
static CobiwmWaylandTouchSurface *
touch_surface_increment_touch (CobiwmWaylandTouchSurface *surface)
{
  surface->touch_count++;
  return surface;
}
...
static void
touch_surface_decrement_touch (CobiwmWaylandTouchSurface *touch_surface)
{
  ...
  if (touch_surface->touch_count == 0)
    {
       ...
       * CobiwmWaylandTouchSurface, the memory is actually owned by
...
{
  CobiwmWaylandTouchSurface *touch_surface = wl_container_of (listener, touch_surface, surface_destroy_listener);
  CobiwmWaylandSurface *surface = touch_surface->surface;
  CobiwmWaylandTouch *touch = touch_surface->touch;
  CobiwmWaylandTouchInfo *touch_info;
  GHashTableIter iter;
...
static CobiwmWaylandTouchSurface *
touch_surface_get (CobiwmWaylandTouch   *touch,
                   CobiwmWaylandSurface *surface)
{
  CobiwmWaylandTouchSurface *touch_surface;
  ...
  touch_surface = g_new0 (CobiwmWaylandTouchSurface, 1);
...
static CobiwmWaylandTouchInfo *
touch_get_info (CobiwmWaylandTouch     *touch,
                ...
                gboolean              create)
{
  ...
  if (!touch_info && create)
    {
      touch_info = g_new0 (CobiwmWaylandTouchInfo, 1);
...
static void
touch_get_relative_coordinates (CobiwmWaylandTouch   *touch,
                                CobiwmWaylandSurface *surface,
...
void
cobiwm_wayland_touch_update (CobiwmWaylandTouch   *touch,
                           const ClutterEvent *event)
{
  CobiwmWaylandTouchInfo *touch_info;
  ClutterEventSequence *sequence;
  ...
  if (event->type == CLUTTER_TOUCH_BEGIN)
    {
      CobiwmWaylandSurface *surface = NULL;
      ClutterActor *actor;

      actor = clutter_event_get_source (event);

      if (COBIWM_IS_SURFACE_ACTOR_WAYLAND (actor))
        surface = cobiwm_surface_actor_wayland_get_surface (COBIWM_SURFACE_ACTOR_WAYLAND (actor));
  ...
  if (event->type == CLUTTER_TOUCH_BEGIN ||
    ...
    {
      CobiwmWaylandSurface *surface = touch_info->touch_surface->surface;
...
static void
handle_touch_begin (CobiwmWaylandTouch   *touch,
                    const ClutterEvent *event)
{
  CobiwmWaylandTouchInfo *touch_info;
  ClutterEventSequence *sequence;
...
static void
handle_touch_update (CobiwmWaylandTouch   *touch,
                     const ClutterEvent *event)
{
  CobiwmWaylandTouchInfo *touch_info;
  ClutterEventSequence *sequence;
...
static void
handle_touch_end (CobiwmWaylandTouch   *touch,
                  const ClutterEvent *event)
{
  CobiwmWaylandTouchInfo *touch_info;
  ClutterEventSequence *sequence;
...
static GList *
touch_get_surfaces (CobiwmWaylandTouch *touch,
...
static void
touch_send_frame_event (CobiwmWaylandTouch *touch)
{
  GList *surfaces, *s;
  ...
  for (s = surfaces; s; s = s->next)
    {
      CobiwmWaylandTouchSurface *touch_surface = s->data;
      struct wl_resource *resource;
      struct wl_list *l;
...
static void
check_send_frame_event (CobiwmWaylandTouch   *touch,
                        const ClutterEvent *event)
{
  ClutterEventSequence *sequence;
  gint32 slot;
...
gboolean
cobiwm_wayland_touch_handle_event (CobiwmWaylandTouch   *touch,
                                 const ClutterEvent *event)
{
  switch (event->type)
    {
...
static void
touch_info_free (CobiwmWaylandTouchInfo *touch_info)
{
...
void
cobiwm_wayland_touch_cancel (CobiwmWaylandTouch *touch)
{
  GList *surfaces, *s;
                   ...
                   gpointer               data)
{
  ...
  switch (libinput_event_get_type (event))
    ...
    case LIBINPUT_EVENT_TOUCH_CANCEL:
       ...
       * requires.
       */
      cobiwm_wayland_touch_cancel (touch);
      break;
    default:
      break;
    }
...
void
cobiwm_wayland_touch_init (CobiwmWaylandTouch  *touch,
...
void
cobiwm_wayland_touch_release (CobiwmWaylandTouch *touch)
{
#ifdef HAVE_NATIVE_BACKEND
  CobiwmBackend *backend = cobiwm_get_backend ();
  if (COBIWM_IS_BACKEND_NATIVE (backend))
...
void
cobiwm_wayland_touch_create_new_resource (CobiwmWaylandTouch   *touch,
                                        struct wl_client   *client,
...
gboolean
cobiwm_wayland_touch_can_popup (CobiwmWaylandTouch *touch,
                              uint32_t          serial)
{
  CobiwmWaylandTouchInfo *touch_info;
  GHashTableIter iter;
...
cobiwm_wayland_touch_find_grab_sequence (CobiwmWaylandTouch   *touch,
                                       CobiwmWaylandSurface *surface,
                                       uint32_t            serial)
{
  CobiwmWaylandTouchInfo *touch_info;
...
gboolean
cobiwm_wayland_touch_get_press_coords (CobiwmWaylandTouch     *touch,
                                     ClutterEventSequence *sequence,
                                     gfloat               *x,
                                     gfloat               *y)
{
