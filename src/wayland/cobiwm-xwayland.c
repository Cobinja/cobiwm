...
#define COBIWM_TYPE_WAYLAND_SURFACE_ROLE_XWAYLAND (cobiwm_wayland_surface_role_xwayland_get_type ())
G_DECLARE_FINAL_TYPE (CobiwmWaylandSurfaceRoleXWayland,
                      cobiwm_wayland_surface_role_xwayland,
                      COBIWM, WAYLAND_SURFACE_ROLE_XWAYLAND,
                      CobiwmWaylandSurfaceRole);

struct _CobiwmWaylandSurfaceRoleXWayland
{
  CobiwmWaylandSurfaceRole parent;
};

G_DEFINE_TYPE (CobiwmWaylandSurfaceRoleXWayland,
               cobiwm_wayland_surface_role_xwayland,
               COBIWM_TYPE_WAYLAND_SURFACE_ROLE);

static void
associate_window_with_surface (CobiwmWindow         *window,
                               CobiwmWaylandSurface *surface)
{
  CobiwmDisplay *display = window->display;

  if (!cobiwm_wayland_surface_assign_role (surface,
                                         COBIWM_TYPE_WAYLAND_SURFACE_ROLE_XWAYLAND))
    {
  ...
  cobiwm_compositor_window_surface_changed (display->compositor, window);
...
static gboolean
associate_window_with_surface_id (CobiwmXWaylandManager *manager,
                                  CobiwmWindow          *window,
                                  guint32              surface_id)
{
...
typedef struct {
  CobiwmXWaylandManager *manager;
  CobiwmWindow *window;
  guint32 surface_id;
  guint later_id;
} AssociateWindowWithSurfaceOp;

static void associate_window_with_surface_window_unmanaged (CobiwmWindow                   *window,
                                                            AssociateWindowWithSurfaceOp *op);
static void
associate_window_with_surface_op_free (AssociateWindowWithSurfaceOp *op)
{
  if (op->later_id != 0)
    cobiwm_later_remove (op->later_id);
...
static void
associate_window_with_surface_window_unmanaged (CobiwmWindow                   *window,
                                                AssociateWindowWithSurfaceOp *op)
{
...
void
cobiwm_xwayland_handle_wl_surface_id (CobiwmWindow *window,
                                    guint32     surface_id)
{
  ...
  if (!associate_window_with_surface_id (manager, window, surface_id))
    {
      ...
      op->later_id = cobiwm_later_add (COBIWM_LATER_BEFORE_REDRAW,
...
gboolean
cobiwm_xwayland_is_xwayland_surface (CobiwmWaylandSurface *surface)
{
  CobiwmWaylandCompositor *compositor = cobiwm_wayland_compositor_get_default ();
  CobiwmXWaylandManager *manager = &compositor->xwayland_manager;
...
static gboolean
choose_xdisplay (CobiwmXWaylandManager *manager)
{
  int display = 0;
  char *lock_file = NULL;
...
on_displayfd_ready (int          fd,
                    ...
                    gpointer     user_data)
{
  CobiwmXWaylandManager *manager = user_data;
...
cobiwm_xwayland_complete_init (void)
{
  ...
  */
  ...
  cobiwm_xwayland_init_selection ();
}
...
cobiwm_xwayland_stop (CobiwmXWaylandManager *manager)
{
  ...
  cobiwm_xwayland_shutdown_selection ();
...
static void
xwayland_surface_assigned (CobiwmWaylandSurfaceRole *surface_role)
{
  CobiwmWaylandSurface *surface =
...
static void
xwayland_surface_commit (CobiwmWaylandSurfaceRole  *surface_role,
...
static void
cobiwm_wayland_surface_role_xwayland_init (CobiwmWaylandSurfaceRoleXWayland *role)
{
}

static void
cobiwm_wayland_surface_role_xwayland_class_init (CobiwmWaylandSurfaceRoleXWaylandClass *klass)
{
