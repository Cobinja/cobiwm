      ...
      event->xcookie.extension == display->xinput_opcode)
    {
      ...
      switch (input_event->evtype)
        ...
        case XI_ButtonRelease:
          if (((XIDeviceEvent *) input_event)->deviceid == COBIWM_VIRTUAL_CORE_POINTER_ID)
            return input_event;
          break;
        case XI_KeyPress:
        case XI_KeyRelease:
          if (((XIDeviceEvent *) input_event)->deviceid == COBIWM_VIRTUAL_CORE_KEYBOARD_ID)
            return input_event;
          break;
        case XI_FocusIn:
        case XI_FocusOut:
          if (((XIEnterEvent *) input_event)->deviceid == COBIWM_VIRTUAL_CORE_KEYBOARD_ID)
            return input_event;
          break;
        case XI_Enter:
        case XI_Leave:
          if (((XIEnterEvent *) input_event)->deviceid == COBIWM_VIRTUAL_CORE_POINTER_ID)
            return input_event;
          break;
#ifdef HAVE_XI23
        case XI_BarrierHit:
        case XI_BarrierLeave:
          if (((XIBarrierEvent *) input_event)->deviceid == COBIWM_VIRTUAL_CORE_POINTER_ID)
            return input_event;
          break;
#endif /* HAVE_XI23 */
        default:
...
static Window
xievent_get_modified_window (CobiwmDisplay *display,
                             XIEvent *input_event)
{
  switch (input_event->evtype)
    {
 ...
 */
static Window
event_get_modified_window (CobiwmDisplay *display,
                           XEvent *event)
{
...
static guint32
event_get_time (CobiwmDisplay *display,
                XEvent      *event)
{
  XIEvent *input_event = get_input_event (display, event);
...
const char*
cobiwm_event_detail_to_string (int d)
{
  const char *detail = "???";
  switch (d)
    {
...
const char*
cobiwm_event_mode_to_string (int m)
{
  const char *mode = "???";
  switch (m)
    {
...
static void
cobiwm_spew_xi2_event (CobiwmDisplay *display,
                     XIEvent     *input_event,
                     const char **name_p,
                     char       **extra_p)
{
  ...
  switch (input_event->evtype)
    ...
    case XI_FocusOut:
      extra = g_strdup_printf ("detail: %s mode: %s\n",
                               cobiwm_event_detail_to_string (enter_event->detail),
                               cobiwm_event_mode_to_string (enter_event->mode));
      break;
    case XI_Enter:
    case XI_Leave:
...
static void
cobiwm_spew_core_event (CobiwmDisplay *display,
                      XEvent      *event,
                      const char **name_p,
                      char       **extra_p)
{
  ...
  switch (event->type)
    ...
    case MappingNotify:
      ...
      break;
    default:
      if (COBIWM_DISPLAY_HAS_XSYNC (display) &&
...
static char *
cobiwm_spew_event (CobiwmDisplay *display,
                 XEvent      *event)
{
  ...
  if (input_event)
    cobiwm_spew_xi2_event (display, input_event, &name, &extra);
  else
    cobiwm_spew_core_event (display, event, &name, &extra);
...
cobiwm_spew_event_print (CobiwmDisplay *display,
                       XEvent      *event)
{
  char *event_str;
  ...
  event_str = cobiwm_spew_event (display, event);
  g_print ("%s\n", event_str);
  g_free (event_str);
}

static gboolean
handle_window_focus_event (CobiwmDisplay  *display,
                           CobiwmWindow   *window,
                           XIEnterEvent *event,
                           unsigned long serial)
{
  ...
  else if (cobiwm_display_xwindow_is_a_no_focus_window (display, event->event))
  ...
  cobiwm_topic (COBIWM_DEBUG_FOCUS,
              ...
              cobiwm_event_mode_to_string (event->mode),
              cobiwm_event_detail_to_string (event->mode),
              serial);
#endif
   ...
   * and cobiwm's "pretend" focus window, and go through all
...
static gboolean
crossing_serial_is_ignored (CobiwmDisplay  *display,
                            unsigned long serial)
{
  int i;
...
static gboolean
handle_input_xevent (CobiwmDisplay  *display,
                     XIEvent      *input_event,
                     unsigned long serial)
{
  XIEnterEvent *enter_event = (XIEnterEvent *) input_event;
  ...
  window = modified != None ? cobiwm_display_lookup_x_window (display, modified) : NULL;

  /* If this is an event for a GTK+ widget, let GTK+ handle it. */
  if (cobiwm_ui_window_is_widget (display->screen->ui, modified))
    return FALSE;
  ...
  switch (input_event->evtype)
    ...
    case XI_Enter:
      ...
      if (window && !crossing_serial_is_ignored (display, serial) &&
          ...
          cobiwm_display_focus_sentinel_clear (display))
        {
          cobiwm_window_handle_enter (window,
                                    enter_event->time,
                                    enter_event->root_x,
                                    enter_event->root_y);
        }
    ...
    case XI_Leave:
      ...
      if (window != NULL &&
          ...
          enter_event->mode != XINotifyUngrab)
        {
          cobiwm_window_handle_leave (window);
        }
      break;
    case XI_FocusIn:
    case XI_FocusOut:
...
static void
process_request_frame_extents (CobiwmDisplay    *display,
                               XEvent         *event)
{
  ...
  cobiwm_verbose ("Setting frame extents for 0x%lx\n", xwindow);

  /* See if the window is decorated. */
  hints_set = cobiwm_prop_get_motif_hints (display,
                                         xwindow,
  ...
  if ((hints_set && hints->decorations) || !hints_set)
    {
      ...
      cobiwm_ui_theme_get_frame_borders (display->screen->ui,
...
static gboolean
convert_property (CobiwmDisplay *display,
                  CobiwmScreen  *screen,
                  Window       w,
                  Atom         target,
                  Atom         property)
...
#define N_TARGETS 4
  ...
  if (target == display->atom_TARGETS)
    ...
    {
      cobiwm_error_trap_pop_with_return (display);
      return FALSE;
    }
  ...
  cobiwm_topic (COBIWM_DEBUG_SYNC, "Syncing on %s\n", G_STRFUNC);
...
/* from fvwm2, Copyright Matthias Clasen, Dominik Vogt */
static void
process_selection_request (CobiwmDisplay   *display,
                           XEvent        *event)
{
  CobiwmScreen *screen = display->screen;
  ...
  if (screen->wm_sn_selection_window != event->xselectionrequest.owner ||
      screen->wm_sn_atom != event->xselectionrequest.selection)
    {
      ...
      cobiwm_verbose ("Selection request with selection %s window 0x%lx not a WM_Sn selection we recognize\n",
  ...
  cobiwm_verbose ("Handled selection request\n");
}

static gboolean
process_selection_clear (CobiwmDisplay   *display,
                         XEvent        *event)
{
  CobiwmScreen *screen = display->screen;
  ...
  if (screen->wm_sn_selection_window != event->xselectionclear.window ||
      screen->wm_sn_atom != event->xselectionclear.selection)
    {
      ...
      cobiwm_verbose ("Selection clear with selection %s window 0x%lx not a WM_Sn selection we recognize\n",
  ...
  cobiwm_verbose ("Got selection clear for screen %d on display %s\n",
                screen->number, display->name);

  cobiwm_display_unmanage_screen (display, display->screen,
                                event->xselectionclear.time);
  return TRUE;
}

static void
notify_bell (CobiwmDisplay *display,
             XkbAnyEvent *xkb_ev)
{
  XkbBellNotifyEvent *xkb_bell_event = (XkbBellNotifyEvent*) xkb_ev;
  CobiwmWindow *window;

  window = cobiwm_display_lookup_x_window (display, xkb_bell_event->window);
  if (!window && display->focus_window && display->focus_window->frame)
    window = display->focus_window;

  display->last_bell_time = xkb_ev->time;
  if (!cobiwm_bell_notify (display, window) &&
      cobiwm_prefs_bell_is_audible ())
    {
...
static gboolean
handle_other_xevent (CobiwmDisplay *display,
                     XEvent      *event)
{
  Window modified;
  CobiwmWindow *window;
  CobiwmWindow *property_for_window;
  ...
  if (COBIWM_DISPLAY_HAS_XSYNC (display) &&
      event->type == (display->xsync_event_base + XSyncAlarmNotify))
    {
      CobiwmWindow *alarm_window = cobiwm_display_lookup_sync_alarm (display,
      ...
      if (alarm_window != NULL)
        {
          ...
          cobiwm_window_x11_update_sync_request_counter (alarm_window, new_counter_value);
  ...
  if (COBIWM_DISPLAY_HAS_SHAPE (display) &&
      event->type == (display->shape_event_base + ShapeNotify))
    {
      ...
      if (window && !frame_was_receiver)
        ...
        {
          cobiwm_topic (COBIWM_DEBUG_SHAPES,
  ...
  switch (event->type)
    ...
    case CreateNotify:
      {
        if (event->xcreatewindow.parent == display->screen->xroot)
          cobiwm_stack_tracker_create_event (display->screen->stack_tracker,
                                           &event->xcreatewindow);
      }
      break;
    ...
    case DestroyNotify:
      {
        if (event->xdestroywindow.event == display->screen->xroot)
          cobiwm_stack_tracker_destroy_event (display->screen->stack_tracker,
      ...
      if (window)
        {
          ...
          if (frame_was_receiver)
            {
              cobiwm_warning ("Unexpected destruction of frame 0x%lx, not sure if this should silently fail or be considered a bug\n",
                            window->frame->xwindow);
              cobiwm_error_trap_push (display);
              cobiwm_window_destroy_frame (window->frame->window);
              cobiwm_error_trap_pop (display);
            }
          else
            {
    ...
    case MapNotify:
      ...
      if (window == NULL && event->xmap.event == display->screen->xroot)
        {
          window = cobiwm_window_x11_new (display, event->xmap.window,
                                        FALSE, COBIWM_COMP_EFFECT_CREATE);
        }
      break;
    case MapRequest:
      if (window == NULL)
        {
          window = cobiwm_window_x11_new (display, event->xmaprequest.window,
                                        FALSE, COBIWM_COMP_EFFECT_CREATE);
        }
      /* if frame was receiver it's some malicious send event or something */
      else if (!frame_was_receiver && window)
        {
          cobiwm_verbose ("MapRequest on %s mapped = %d minimized = %d\n",
          ...
          if (window->minimized)
            {
              ...
              if (window->workspace != window->screen->active_workspace)
                {
                  cobiwm_verbose ("Changing workspace due to MapRequest mapped = %d minimized = %d\n",
                                window->mapped, window->minimized);
                  cobiwm_window_change_workspace (window,
    ...
    case ReparentNotify:
      {
        if (event->xreparent.event == display->screen->xroot)
          cobiwm_stack_tracker_reparent_event (display->screen->stack_tracker,
                                             &event->xreparent);
      }
      break;
    case ConfigureNotify:
      if (event->xconfigure.event != event->xconfigure.window)
        {
          if (event->xconfigure.event == display->screen->xroot &&
            ...
            cobiwm_stack_tracker_configure_event (display->screen->stack_tracker,
                                                &event->xconfigure);
        }

      if (window && window->override_redirect)
        cobiwm_window_x11_configure_notify (window, &event->xconfigure);
      ...
      if (window == NULL)
        {
          ...
          cobiwm_verbose ("Configuring withdrawn window to %d,%d %dx%d border %d (some values may not be in mask)\n",
        ...
        {
          if (!frame_was_receiver)
            cobiwm_window_x11_configure_request (window, event);
        }
      break;
    case GravityNotify:
      break;
    ...
    case PropertyNotify:
      {
        ...
        if (window && !frame_was_receiver)
          cobiwm_window_x11_property_notify (window, event);
        else if (property_for_window && !frame_was_receiver)
          cobiwm_window_x11_property_notify (property_for_window, event);

        group = cobiwm_display_lookup_group (display,
                                           event->xproperty.window);
        if (group != NULL)
          cobiwm_group_property_notify (group, event);

        if (event->xproperty.window == display->screen->xroot)
          {
            if (event->xproperty.atom ==
                display->atom__NET_DESKTOP_LAYOUT)
              cobiwm_screen_update_workspace_layout (display->screen);
            else if (event->xproperty.atom ==
                     display->atom__NET_DESKTOP_NAMES)
              cobiwm_screen_update_workspace_names (display->screen);
            ...
            if (event->xproperty.atom ==
              ...
              {
                cobiwm_display_decrement_focus_sentinel (display);
              }
          }
      }
      break;
          ...
          if (event->xclient.message_type == display->atom_WL_SURFACE_ID)
            {
              ...
              cobiwm_xwayland_handle_wl_surface_id (window, surface_id);
            }
          else
#endif
          if (!frame_was_receiver)
            cobiwm_window_x11_client_message (window, event);
        }
      else
        {
          if (event->xclient.window == display->screen->xroot)
            {
              if (event->xclient.message_type ==
                  display->atom__NET_CURRENT_DESKTOP)
                {
                  ...
                  cobiwm_verbose ("Request to change current workspace to %d with "
                                "specified timestamp of %u\n",
                                space, time);

                  workspace = cobiwm_screen_get_workspace_by_index (display->screen, space);

                  /* Handle clients using the older version of the spec... */
                  if (time == 0 && workspace)
                    {
                      cobiwm_warning ("Received a NET_CURRENT_DESKTOP message "
                                    "from a broken (outdated) client who sent "
                                    "a 0 timestamp\n");
                      time = cobiwm_display_get_current_time_roundtrip (display);
                    }

                  if (workspace)
                    cobiwm_workspace_activate (workspace, time);
                  else
                    cobiwm_verbose ("Don't know about workspace %d\n", space);
                }
                       ...
                       display->atom__NET_NUMBER_OF_DESKTOPS)
                {
                  ...
                  cobiwm_verbose ("Request to set number of workspaces to %d\n",
                                num_spaces);

                  cobiwm_prefs_set_num_workspaces (num_spaces);
                }
              else if (event->xclient.message_type ==
                       display->atom__NET_SHOWING_DESKTOP)
                {
                  ...
                  timestamp = cobiwm_display_get_current_time_roundtrip (display);
                  cobiwm_verbose ("Request to %s desktop\n",
                                showing_desktop ? "show" : "hide");

                  if (showing_desktop)
                    cobiwm_screen_show_desktop (display->screen, timestamp);
                  else
                    {
                      cobiwm_screen_unshow_desktop (display->screen);
                      cobiwm_workspace_focus_default_window (display->screen->active_workspace, NULL, timestamp);
                    }
                }
              else if (event->xclient.message_type ==
                       display->atom_WM_PROTOCOLS)
                {
                  cobiwm_verbose ("Received WM_PROTOCOLS message\n");

                  if ((Atom)event->xclient.data.l[0] == display->atom__NET_WM_PING)
                    {
                      guint32 timestamp = event->xclient.data.l[1];

                      cobiwm_display_pong_for_serial (display, timestamp);
          ...
          if (event->xclient.message_type ==
            ...
            {
              cobiwm_verbose ("Received _NET_REQUEST_FRAME_EXTENTS message\n");
...
static gboolean
window_has_xwindow (CobiwmWindow *window,
                    Window      xwindow)
{
...
/**
 * cobiwm_display_handle_xevent:
 * @display: The CobiwmDisplay that events are coming from
 * @event: The event that just happened
 *
 * This is the most important function in the whole program. It is the heart,
 * it is the nexus, it is the Grand Central Station of Cobiwm's world.
 * When we create a #CobiwmDisplay, we ask GDK to pass *all* events for *all*
 ...
 */
static gboolean
cobiwm_display_handle_xevent (CobiwmDisplay *display,
                            XEvent      *event)
{
  Window modified;
...
#if 0
  cobiwm_spew_event_print (display, event);
#endif

  if (cobiwm_startup_notification_handle_xevent (display->startup_notification,
                                               event))
    {
  ...
  if (cobiwm_is_wayland_compositor () &&
      cobiwm_xwayland_selection_handle_event (event))
    {
      bypass_gtk = bypass_compositor = TRUE;
      goto out;
    }
  ...
  if (display->focused_by_us &&
    ...
    {
      cobiwm_topic (COBIWM_DEBUG_FOCUS, "Earlier attempt to focus %s failed\n",
                  display->focus_window->desc);
      cobiwm_display_update_focus_window (display,
                                        cobiwm_display_lookup_x_window (display, display->server_focus_window),
  ...
  if (event->xany.window == display->screen->xroot)
    {
      if (cobiwm_screen_handle_xevent (display->screen, event))
        {
          bypass_gtk = bypass_compositor = TRUE;
          goto out;
        }
  ...
  if (event->type == UnmapNotify)
    {
      if (cobiwm_ui_window_should_not_cause_focus (display->xdisplay,
                                                 modified))
        {
          cobiwm_display_add_ignored_crossing_serial (display, event->xany.serial);
          cobiwm_topic (COBIWM_DEBUG_FOCUS,
...
#ifdef HAVE_XI23
  if (cobiwm_display_process_barrier_xevent (display, input_event))
    {
      bypass_gtk = bypass_compositor = TRUE;
      goto out;
    }
  ...
  if (event->type == SelectionClear)
    {
      if (process_selection_clear (display, event))
        {
          /* This means we called cobiwm_display_unmanage_screen, which
           * means the CobiwmDisplay is effectively dead. We don't want
 ...
 out:
  if (!bypass_compositor)
    {
      CobiwmWindow *window = modified != None ? cobiwm_display_lookup_x_window (display, modified) : NULL;

      if (cobiwm_compositor_process_event (display->compositor, event, window))
        bypass_gtk = TRUE;
    }
               ...
               gpointer   data)
{
  ...
  if (cobiwm_display_handle_xevent (display, xevent))
...
void
cobiwm_display_init_events_x11 (CobiwmDisplay *display)
{
  gdk_window_add_filter (NULL, xevent_filter, display);
}

void
cobiwm_display_free_events_x11 (CobiwmDisplay *display)
{
