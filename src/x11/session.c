...
#ifndef HAVE_SM
void
cobiwm_session_init (const char *client_id,
                   const char *save_file)
{
  cobiwm_topic (COBIWM_DEBUG_SM, "Compiled without session management support\n");
}

const CobiwmWindowSessionInfo*
cobiwm_window_lookup_saved_state (CobiwmWindow *window)
{
  return NULL;
}

void
cobiwm_window_release_saved_state (const CobiwmWindowSessionInfo *info)
{
  ;
}
#else /* HAVE_SM */
...
void
cobiwm_session_init (const char *previous_client_id,
                   const char *previous_save_file)
{
  ...
  cobiwm_topic (COBIWM_DEBUG_SM, "Initializing session with save file '%s'\n",
  ...
  if (session_connection == NULL)
    ...
    {
      if (client_id == NULL)
        cobiwm_bug ("Session manager gave us a NULL client ID?");
      cobiwm_topic (COBIWM_DEBUG_SM, "Obtained session ID '%s'\n", client_id);
    }
                             ...
                             gboolean successful)
{
  ...
  if (current_state == STATE_SAVING_PHASE_1 ||
    ...
    {
      cobiwm_topic (COBIWM_DEBUG_SM, "Sending SaveYourselfDone\n");
...
save_phase_2_callback (SmcConn smc_conn, SmPointer client_data)
{
  ...
  cobiwm_topic (COBIWM_DEBUG_SM, "Phase 2 save");
                        ...
                        Bool      fast)
{
  ...
  cobiwm_topic (COBIWM_DEBUG_SM, "SaveYourself received");
...
{
  cobiwm_topic (COBIWM_DEBUG_SM, "Disconnecting from session manager");
  disconnect ();
...
{
  /* nothing */
  cobiwm_topic (COBIWM_DEBUG_SM, "SaveComplete received\n");
}

static void
shutdown_cancelled_callback (SmcConn smc_conn, SmPointer client_data)
{
  cobiwm_topic (COBIWM_DEBUG_SM, "Shutdown cancelled received\n");
...
interact_callback (SmcConn smc_conn, SmPointer client_data)
{
  ...
  cobiwm_topic (COBIWM_DEBUG_SM, "Interaction permission received\n");
...
static const char*
window_type_to_string (CobiwmWindowType type)
{
  switch (type)
    {
...
static CobiwmWindowType
window_type_from_string (const char *str)
{
  if (strcmp (str, "normal") == 0)
    return COBIWM_WINDOW_NORMAL;
  else if (strcmp (str, "desktop") == 0)
    return COBIWM_WINDOW_DESKTOP;
  else if (strcmp (str, "dock") == 0)
    return COBIWM_WINDOW_DOCK;
  else if (strcmp (str, "dialog") == 0)
    return COBIWM_WINDOW_DIALOG;
  else if (strcmp (str, "modal_dialog") == 0)
    return COBIWM_WINDOW_MODAL_DIALOG;
  else if (strcmp (str, "toolbar") == 0)
    return COBIWM_WINDOW_TOOLBAR;
  else if (strcmp (str, "menu") == 0)
    return COBIWM_WINDOW_MENU;
  else if (strcmp (str, "utility") == 0)
    return COBIWM_WINDOW_UTILITY;
  else if (strcmp (str, "splashscreen") == 0)
    return COBIWM_WINDOW_SPLASHSCREEN;
  else
    return COBIWM_WINDOW_NORMAL;
}
...
save_state (void)
{
   ...
   */
  cobiwm_dir = g_strconcat (g_get_user_config_dir (),
                              G_DIR_SEPARATOR_S "cobiwm",
                              NULL);

  session_dir = g_strconcat (cobiwm_dir,
                             G_DIR_SEPARATOR_S "sessions",
                             NULL);

  if (mkdir (cobiwm_dir, 0700) < 0 &&
      errno != EEXIST)
    {
      cobiwm_warning ("Could not create directory '%s': %s\n",
                    cobiwm_dir, g_strerror (errno));
    }
  ...
  cobiwm_topic (COBIWM_DEBUG_SM, "Saving session to '%s'\n", full_save_file ());

  outfile = fopen (full_save_file (), "w");

  if (outfile == NULL)
    {
      cobiwm_warning ("Could not open session file '%s' for writing: %s\n",
                    full_save_file (), g_strerror (errno));
      goto out;
    }

  /* The file format is:
   * <cobiwm_session id="foo">
  ...
  fprintf (outfile, "<cobiwm_session id=\"%s\">\n",
           client_id);

  windows = cobiwm_display_list_windows (cobiwm_get_display (), COBIWM_LIST_DEFAULT);
  stack_position = 0;

  windows = g_slist_sort (windows, cobiwm_display_stack_cmp);
  tmp = windows;
  stack_position = 0;
      ...
      if (window->sm_client_id)
        {
          ...
          cobiwm_topic (COBIWM_DEBUG_SM, "Saving session managed window %s, client ID '%s'\n",
          ...
          if (window->on_all_workspaces_requested)
            {
              ...
              int n;
              n = cobiwm_workspace_index (window->workspace);
              fprintf (outfile,
                       "    <workspace index=\"%d\"/>\n", n);
            }
          ...
          {
            int x, y, w, h;
            cobiwm_window_get_session_geometry (window, &x, &y, &w, &h);

            fprintf (outfile,
                     "    <geometry x=\"%d\" y=\"%d\" width=\"%d\" height=\"%d\" gravity=\"%s\"/>\n",
                     x, y, w, h,
                     cobiwm_gravity_to_string (window->size_hints.win_gravity));
          }
        ...
        {
          cobiwm_topic (COBIWM_DEBUG_SM, "Not saving window '%s', not session managed\n",
                      window->desc);
        }
  ...
  if (outfile)
    {
      ...
      if (ferror (outfile))
        {
          cobiwm_warning ("Error writing session file '%s': %s\n",
                        full_save_file (), g_strerror (errno));
        }
      if (fclose (outfile))
        {
          cobiwm_warning ("Error closing session file '%s': %s\n",
                        full_save_file (), g_strerror (errno));
        }
    }
...
typedef struct
{
  CobiwmWindowSessionInfo *info;
  char *previous_id;
} ParseData;

static void                   session_info_free (CobiwmWindowSessionInfo *info);
static CobiwmWindowSessionInfo* session_info_new  (void);
...
static GMarkupParser cobiwm_session_parser = {
...
load_state (const char *previous_save_file)
{
  ...
  if (!g_file_get_contents (session_file,
                            ...
                            &error))
    {
      ...
      /* Maybe they were doing it the old way, with ~/.cobiwm */
  ...
  cobiwm_topic (COBIWM_DEBUG_SM, "Parsing saved session file %s\n", session_file);
  g_free (session_file);
  session_file = NULL;
  ...
  context = g_markup_parse_context_new (&cobiwm_session_parser,
                                        0, &parse_data, NULL);
 ...
 error:
  ...
  cobiwm_warning ("Failed to parse saved session file: %s\n",
                error->message);
  g_error_free (error);
                        ...
                        GError             **error)
{
  ...
  if (strcmp (element_name, "cobiwm_session") == 0)
    {
      /* Get previous ID */
      int i;
          ...
          if (pd->previous_id)
            {
              g_set_error (error,
                       ...
                       G_MARKUP_ERROR_PARSE,
                           "<cobiwm_session> attribute seen but we already have the session ID");
              return;
            }
  ...
  else if (strcmp (element_name, "maximized") == 0)
    {
      ...
      if (pd->info->saved_rect_set)
        cobiwm_topic (COBIWM_DEBUG_SM, "Saved unmaximized size %d,%d %dx%d \n",
  ...
  else if (strcmp (element_name, "geometry") == 0)
    {
      ...
      cobiwm_topic (COBIWM_DEBUG_SM, "Loaded geometry %d,%d %dx%d gravity %s\n",
                  pd->info->rect.x,
                  pd->info->rect.y,
                  pd->info->rect.width,
                  pd->info->rect.height,
                  cobiwm_gravity_to_string (pd->info->gravity));
    }
  else
    {
      g_set_error (error,
  ...
  if (strcmp (element_name, "window") == 0)
    {
      ...
      cobiwm_topic (COBIWM_DEBUG_SM, "Loaded window info from session with class: %s name: %s role: %s\n",
...
static GSList*
get_possible_matches (CobiwmWindow *window)
{
  ...
  ignore_client_id = g_getenv ("COBIWM_DEBUG_SM") != NULL;
  ...
  while (tmp != NULL)
    {
      ...
      if ((ignore_client_id ||
        ...
        {
          cobiwm_topic (COBIWM_DEBUG_SM, "Window %s may match saved window with class: %s name: %s role: %s\n",
                      window->desc,
          ...
          if (cobiwm_is_verbose ())
            {
              if (!both_null_or_matching (info->id, window->sm_client_id))
                cobiwm_topic (COBIWM_DEBUG_SM, "Window %s has SM client ID %s, saved state has %s, no match\n",
                            window->desc,
                            window->sm_client_id ? window->sm_client_id : "(none)",
                            info->id ? info->id : "(none)");
              else if (!both_null_or_matching (info->res_class, window->res_class))
                cobiwm_topic (COBIWM_DEBUG_SM, "Window %s has class %s doesn't match saved class %s, no match\n",
                            window->desc,
                            window->res_class ? window->res_class : "(none)",
                            info->res_class ? info->res_class : "(none)");

              else if (!both_null_or_matching (info->res_name, window->res_name))
                cobiwm_topic (COBIWM_DEBUG_SM, "Window %s has name %s doesn't match saved name %s, no match\n",
                            window->desc,
                            window->res_name ? window->res_name : "(none)",
                            info->res_name ? info->res_name : "(none)");
              else if (!both_null_or_matching (info->role, window->role))
                cobiwm_topic (COBIWM_DEBUG_SM, "Window %s has role %s doesn't match saved role %s, no match\n",
                            window->desc,
                            window->role ? window->role : "(none)",
                            info->role ? info->role : "(none)");
              else
                cobiwm_topic (COBIWM_DEBUG_SM, "???? should not happen - window %s doesn't match saved state %s for no good reason\n",
                            window->desc, info->id);
            }
        }
...
static const CobiwmWindowSessionInfo*
find_best_match (GSList     *infos,
                 CobiwmWindow *window)
{
  GSList *tmp;
  const CobiwmWindowSessionInfo *matching_title;
  const CobiwmWindowSessionInfo *matching_type;
...
cobiwm_window_lookup_saved_state (CobiwmWindow *window)
{
  ...
  if (possibles == NULL)
    {
      cobiwm_topic (COBIWM_DEBUG_SM, "Window %s has no possible matches in the list of saved window states\n",
                  window->desc);
      return NULL;
    }
...
cobiwm_window_release_saved_state (const CobiwmWindowSessionInfo *info)
{
  ...
  session_info_free ((CobiwmWindowSessionInfo*) info);
}
...
static CobiwmWindowSessionInfo*
session_info_new (void)
{
  CobiwmWindowSessionInfo *info;

  info = g_new0 (CobiwmWindowSessionInfo, 1);

  info->type = COBIWM_WINDOW_NORMAL;
...
static int
windows_cmp_by_title (CobiwmWindow *a,
                      CobiwmWindow *b)
{
...
warn_about_lame_clients_and_finish_interact (gboolean shutdown)
{
  ...
  pid = cobiwm_show_dialog("--list",
                         _("These windows do not support &quot;save current setup&quot; "
                           "and will have to be restarted manually next time "
                           "you log in."),
                         "240",
                         cobiwm_get_display()->screen->screen_name,
                         NULL, NULL, NULL,
                         None,
                         columns,
                         lame_details);
