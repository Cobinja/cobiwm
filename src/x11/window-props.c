 ...
 * @short_description: #CobiwmWindow property handling
 *
 * A system which can inspect sets of properties of given windows
 * and take appropriate action given their values.
 *
 * Note that all the cobiwm_window_reload_propert* functions require a
 * round trip to the server.
 *
 * The guts of this system are in cobiwm_display_init_window_prop_hooks().
...
typedef void (* ReloadValueFunc) (CobiwmWindow    *window,
...
struct _CobiwmWindowPropHooks
{
  Atom property;
  CobiwmPropValueType type;
  ReloadValueFunc reload_func;
  CobiwmPropHookFlags flags;
};

static void init_prop_value            (CobiwmWindow          *window,
                                        CobiwmWindowPropHooks *hooks,
                                        CobiwmPropValue       *value);
static void reload_prop_value          (CobiwmWindow          *window,
                                        CobiwmWindowPropHooks *hooks,
                                        CobiwmPropValue       *value,
                                        gboolean             initial);
static CobiwmWindowPropHooks* find_hooks (CobiwmDisplay *display,
                                        Atom         property);


void
cobiwm_window_reload_property_from_xwindow (CobiwmWindow      *window,
                                          Window           xwindow,
                                          Atom             property,
                                          gboolean         initial)
{
  CobiwmPropValue value = { 0, };
  CobiwmWindowPropHooks *hooks;
  ...
  cobiwm_prop_get_values (window->display, xwindow,
                        &value, 1);

  reload_prop_value (window, hooks, &value,
                     initial);

  cobiwm_prop_free_values (&value, 1);
}

static void
cobiwm_window_reload_property (CobiwmWindow      *window,
                             Atom             property,
                             gboolean         initial)
{
  cobiwm_window_reload_property_from_xwindow (window,
                                            window->xwindow,
                                            property,
                                            initial);
}
...
void
cobiwm_window_load_initial_properties (CobiwmWindow *window)
{
  int i, j;
  CobiwmPropValue *values;
  int n_properties = 0;

  values = g_new0 (CobiwmPropValue, window->display->n_prop_hooks);

  j = 0;
  for (i = 0; i < window->display->n_prop_hooks; i++)
    {
      CobiwmWindowPropHooks *hooks = &window->display->prop_hooks_table[i];
  ...
  cobiwm_prop_get_values (window->display, window->xwindow,
                        values, n_properties);
      ...
      if (hooks->flags & LOAD_INIT)
        {
           ...
           */
          if (values[j].type != COBIWM_PROP_VALUE_INVALID ||
...
static void
init_prop_value (CobiwmWindow          *window,
                 CobiwmWindowPropHooks *hooks,
                 CobiwmPropValue       *value)
{
  if (!hooks || hooks->type == COBIWM_PROP_VALUE_INVALID ||
...
static void
reload_prop_value (CobiwmWindow          *window,
                   CobiwmWindowPropHooks *hooks,
                   CobiwmPropValue       *value,
                   gboolean             initial)
{
...
static void
reload_wm_client_machine (CobiwmWindow    *window,
                          CobiwmPropValue *value,
                          gboolean       initial)
{
  ...
  cobiwm_verbose ("Window has client machine \"%s\"\n",
...
static void
complain_about_broken_client (CobiwmWindow    *window,
                              CobiwmPropValue *value,
                              gboolean       initial)
{
  cobiwm_warning ("Broken client! Window %s changed client leader window or SM client ID\n",
                window->desc);
}

static void
reload_net_wm_window_type (CobiwmWindow    *window,
                           CobiwmPropValue *value,
                           gboolean       initial)
{
...
static void
reload_icon (CobiwmWindow    *window,
             Atom           atom)
{
  CobiwmWindowX11 *window_x11 = COBIWM_WINDOW_X11 (window);
  CobiwmWindowX11Private *priv = window_x11->priv;

  cobiwm_icon_cache_property_changed (&priv->icon_cache,
                                    window->display,
                                    atom);
  cobiwm_window_queue(window, COBIWM_QUEUE_UPDATE_ICON);
}

static void
reload_net_wm_icon (CobiwmWindow    *window,
                    CobiwmPropValue *value,
                    gboolean       initial)
{
...
static void
reload_kwm_win_icon (CobiwmWindow    *window,
                     CobiwmPropValue *value,
                     gboolean       initial)
{
...
static void
reload_icon_geometry (CobiwmWindow    *window,
                      CobiwmPropValue *value,
                      gboolean       initial)
{
  if (value->type != COBIWM_PROP_VALUE_INVALID)
    {
      if (value->v.cardinal_list.n_cardinals != 4)
        {
          cobiwm_verbose ("_NET_WM_ICON_GEOMETRY on %s has %d values instead of 4\n",
      ...
      else
        {
          ...
          cobiwm_window_set_icon_geometry (window, &geometry);
        }
    }
  else
    {
      cobiwm_window_set_icon_geometry (window, NULL);
    }
}

static void
cobiwm_window_set_custom_frame_extents (CobiwmWindow *window,
                                      GtkBorder  *extents,
                                      gboolean    is_initial)
{
  if (extents)
    {
      ...
      if (is_initial)
        {
          cobiwm_window_client_rect_to_frame_rect (window, &window->rect, &window->rect);
          cobiwm_window_client_rect_to_frame_rect (window, &window->unconstrained_rect, &window->unconstrained_rect);
        }
    }
  else
    {
...
static void
reload_gtk_frame_extents (CobiwmWindow    *window,
                          CobiwmPropValue *value,
                          gboolean       initial)
{
  if (value->type != COBIWM_PROP_VALUE_INVALID)
    {
      if (value->v.cardinal_list.n_cardinals != 4)
        {
          cobiwm_verbose ("_GTK_FRAME_EXTENTS on %s has %d values instead of 4\n",
                        window->desc, value->v.cardinal_list.n_cardinals);
        }
      else
        {
          ...
          cobiwm_window_set_custom_frame_extents (window, &extents, initial);
        }
    }
  else
    {
      cobiwm_window_set_custom_frame_extents (window, NULL, initial);
    }
}

static void
reload_struts (CobiwmWindow    *window,
               CobiwmPropValue *value,
               gboolean       initial)
{
...
static void
reload_wm_window_role (CobiwmWindow    *window,
                       CobiwmPropValue *value,
                       gboolean       initial)
{
...
static void
reload_net_wm_pid (CobiwmWindow    *window,
                   CobiwmPropValue *value,
                   gboolean       initial)
{
  if (value->type != COBIWM_PROP_VALUE_INVALID)
    {
      ...
      if (cardinal <= 0)
        cobiwm_warning ("Application set a bogus _NET_WM_PID %u\n",
                      cardinal);
      else
        {
          window->net_wm_pid = cardinal;
          cobiwm_verbose ("Window has _NET_WM_PID %d\n",
                        window->net_wm_pid);
        }
    }
}
...
static void
reload_net_wm_user_time (CobiwmWindow    *window,
                         CobiwmPropValue *value,
                         gboolean       initial)
{
  if (value->type != COBIWM_PROP_VALUE_INVALID)
    {
      ...
      cobiwm_window_set_user_time (window, cardinal);
    }
}

static void
reload_net_wm_user_time_window (CobiwmWindow    *window,
                                CobiwmPropValue *value,
                                gboolean       initial)
{
  if (value->type != COBIWM_PROP_VALUE_INVALID)
    {
      ...
      if (window->user_time_window != None)
        {
          /* See the comment to the cobiwm_display_register_x_window call below. */
      ...
      if (window->user_time_window != None)
        {
           ...
           */
          cobiwm_display_register_x_window (window->display,
                                          &window->user_time_window,
                                          window);
           ...
           */
          cobiwm_window_reload_property_from_xwindow (
            window,
            window->user_time_window,
 ...
 */
static gboolean
set_title_text (CobiwmWindow  *window,
                ...
                char       **target)
{
  ...
  if (modified && atom != None)
    cobiwm_prop_set_utf8_string_hint (window->display,
                                    window->xwindow,
                                    atom, *target);
...
static void
set_window_title (CobiwmWindow *window,
                  const char *title)
{
  CobiwmWindowX11 *window_x11 = COBIWM_WINDOW_X11 (window);
  CobiwmWindowX11Private *priv = window_x11->priv;
  ...
  cobiwm_window_set_title (window, new_title);

  g_free (new_title);
}

static void
reload_net_wm_name (CobiwmWindow    *window,
                    CobiwmPropValue *value,
                    gboolean       initial)
{
  CobiwmWindowX11 *window_x11 = COBIWM_WINDOW_X11 (window);
  ...
  if (value->type != COBIWM_PROP_VALUE_INVALID)
    {
      ...
      cobiwm_verbose ("Using _NET_WM_NAME for new title of %s: \"%s\"\n",
  ...
  else
    {
      ...
      if (!initial)
        cobiwm_window_reload_property (window, XA_WM_NAME, FALSE);
    }
}

static void
reload_wm_name (CobiwmWindow    *window,
                CobiwmPropValue *value,
                gboolean       initial)
{
  CobiwmWindowX11 *window_x11 = COBIWM_WINDOW_X11 (window);
  ...
  if (priv->using_net_wm_name)
    {
      cobiwm_verbose ("Ignoring WM_NAME \"%s\" as _NET_WM_NAME is set\n",
                    value->v.str);
      return;
    }
  ...
  if (value->type != COBIWM_PROP_VALUE_INVALID)
    {
      ...
      cobiwm_verbose ("Using WM_NAME for new title of %s: \"%s\"\n",
                    window->desc, window->title);
    }
  else
    {
...
static void
cobiwm_window_set_opaque_region (CobiwmWindow     *window,
...
static void
reload_opaque_region (CobiwmWindow    *window,
                      CobiwmPropValue *value,
                      gboolean       initial)
{
  ...
  if (value->type != COBIWM_PROP_VALUE_INVALID)
    {
      ...
      if (nitems % 4 != 0)
        {
          cobiwm_verbose ("_NET_WM_OPAQUE_REGION does not have a list of 4-tuples.");
          goto out;
        }
 ...
 out:
  cobiwm_window_set_opaque_region (window, opaque_region);
  cairo_region_destroy (opaque_region);
}

static void
reload_cobiwm_hints (CobiwmWindow    *window,
                     CobiwmPropValue *value,
                     gboolean       initial)
{
  if (value->type != COBIWM_PROP_VALUE_INVALID)
    {
      ...
      char     *old_hints = window->cobiwm_hints;
      ...
      if (changed)
        {
          ...
          if (new_hints)
            window->cobiwm_hints = g_strdup (new_hints);
          else
            window->cobiwm_hints = NULL;

          g_object_notify (G_OBJECT (window), "cobiwm-hints");
        }
    }
  else if (window->cobiwm_hints)
    {
      g_free (window->cobiwm_hints);
      window->cobiwm_hints = NULL;
...
static void
reload_net_wm_state (CobiwmWindow    *window,
                     CobiwmPropValue *value,
                     gboolean       initial)
{
  CobiwmWindowX11 *window_x11 = COBIWM_WINDOW_X11 (window);
  ...
  if (!initial) {
    ...
    cobiwm_verbose ("Ignoring _NET_WM_STATE: we should be the one who set "
  ...
  cobiwm_verbose ("Reloaded _NET_WM_STATE for %s\n",
                window->desc);
...
static void
reload_mwm_hints (CobiwmWindow    *window,
                  CobiwmPropValue *value,
                  gboolean       initial)
{
  MotifWmHints *hints;
  ...
  if (value->type == COBIWM_PROP_VALUE_INVALID)
    {
      cobiwm_verbose ("Window %s has no MWM hints\n", window->desc);
      cobiwm_window_recalc_features (window);
      return;
    }
  ...
  cobiwm_verbose ("Window %s has MWM hints\n",
                window->desc);

  if (hints->flags & MWM_HINTS_DECORATIONS)
    {
      cobiwm_verbose ("Window %s sets MWM_HINTS_DECORATIONS 0x%x\n",
    ...
    cobiwm_verbose ("Decorations flag unset\n");

  if (hints->flags & MWM_HINTS_FUNCTIONS)
    {
      gboolean toggle_value;

      cobiwm_verbose ("Window %s sets MWM_HINTS_FUNCTIONS 0x%x\n",
      ...
      if ((hints->functions & MWM_FUNC_ALL) == 0)
        {
          ...
          cobiwm_verbose ("Window %s disables all funcs then reenables some\n",
                        window->desc);
        ...
        {
          cobiwm_verbose ("Window %s enables all funcs then disables some\n",
                        window->desc);
          toggle_value = FALSE;
        }
      ...
      if ((hints->functions & MWM_FUNC_CLOSE) != 0)
        {
          cobiwm_verbose ("Window %s toggles close via MWM hints\n",
                        window->desc);
          window->mwm_has_close_func = toggle_value;
        }
      if ((hints->functions & MWM_FUNC_MINIMIZE) != 0)
        {
          cobiwm_verbose ("Window %s toggles minimize via MWM hints\n",
                        window->desc);
          window->mwm_has_minimize_func = toggle_value;
        }
      if ((hints->functions & MWM_FUNC_MAXIMIZE) != 0)
        {
          cobiwm_verbose ("Window %s toggles maximize via MWM hints\n",
                        window->desc);
          window->mwm_has_maximize_func = toggle_value;
        }
      if ((hints->functions & MWM_FUNC_MOVE) != 0)
        {
          cobiwm_verbose ("Window %s toggles move via MWM hints\n",
                        window->desc);
          window->mwm_has_move_func = toggle_value;
        }
      if ((hints->functions & MWM_FUNC_RESIZE) != 0)
        {
          cobiwm_verbose ("Window %s toggles resize via MWM hints\n",
                        window->desc);
          window->mwm_has_resize_func = toggle_value;
        }
    }
  else
    cobiwm_verbose ("Functions flag unset\n");

  cobiwm_window_recalc_features (window);

  /* We do all this anyhow at the end of cobiwm_window_x11_new() */
...
static void
reload_wm_class (CobiwmWindow    *window,
                 CobiwmPropValue *value,
                 gboolean       initial)
{
  if (value->type != COBIWM_PROP_VALUE_INVALID)
    {
      ...
      g_autofree gchar *res_name = g_convert (value->v.class_hint.res_name, -1,
                                              "UTF-8", "LATIN1",
                                              NULL, NULL, NULL);
      cobiwm_window_set_wm_class (window, res_class, res_name);
    }
  else
    {
      cobiwm_window_set_wm_class (window, NULL, NULL);
    }

  cobiwm_verbose ("Window %s class: '%s' name: '%s'\n",
      window->desc,
...
static void
reload_net_wm_desktop (CobiwmWindow    *window,
                       CobiwmPropValue *value,
                       gboolean       initial)
{
  if (value->type != COBIWM_PROP_VALUE_INVALID)
...
static void
reload_net_startup_id (CobiwmWindow    *window,
                       CobiwmPropValue *value,
                       gboolean       initial)
{
  ...
  if (!window->constructing)
  {
    ...
    if (cobiwm_screen_apply_startup_properties (window->screen, window))
      {

        if (window->initial_timestamp_set)
          timestamp = window->initial_timestamp;
        if (window->initial_workspace_set)
          workspace = cobiwm_screen_get_workspace_by_index (window->screen, window->initial_workspace);

        cobiwm_window_activate_with_workspace (window, timestamp, workspace);
      }
  }

  cobiwm_verbose ("New _NET_STARTUP_ID \"%s\" for %s\n",
                window->startup_id ? window->startup_id : "unset",
                window->desc);
}

static void
reload_update_counter (CobiwmWindow    *window,
                       CobiwmPropValue *value,
                       gboolean       initial)
{
  if (value->type != COBIWM_PROP_VALUE_INVALID)
    {
      ...
      if (value->v.xcounter_list.n_counters == 0)
        {
          cobiwm_warning ("_NET_WM_SYNC_REQUEST_COUNTER is empty\n");
          return;
        }
      ...
      cobiwm_verbose ("Window has _NET_WM_SYNC_REQUEST_COUNTER 0x%lx (extended=%s)\n",
                             ...
                             const XSizeHints *new)
{
  if (FLAG_CHANGED (old, new, USPosition))
    cobiwm_topic (COBIWM_DEBUG_GEOMETRY, "XSizeHints: USPosition now %s\n",
                FLAG_TOGGLED_ON (old, new, USPosition) ? "set" : "unset");
  if (FLAG_CHANGED (old, new, USSize))
    cobiwm_topic (COBIWM_DEBUG_GEOMETRY, "XSizeHints: USSize now %s\n",
                FLAG_TOGGLED_ON (old, new, USSize) ? "set" : "unset");
  if (FLAG_CHANGED (old, new, PPosition))
    cobiwm_topic (COBIWM_DEBUG_GEOMETRY, "XSizeHints: PPosition now %s\n",
                FLAG_TOGGLED_ON (old, new, PPosition) ? "set" : "unset");
  if (FLAG_CHANGED (old, new, PSize))
    cobiwm_topic (COBIWM_DEBUG_GEOMETRY, "XSizeHints: PSize now %s\n",
                FLAG_TOGGLED_ON (old, new, PSize) ? "set" : "unset");
  if (FLAG_CHANGED (old, new, PMinSize))
    cobiwm_topic (COBIWM_DEBUG_GEOMETRY, "XSizeHints: PMinSize now %s (%d x %d -> %d x %d)\n",
  ...
  if (FLAG_CHANGED (old, new, PMaxSize))
    cobiwm_topic (COBIWM_DEBUG_GEOMETRY, "XSizeHints: PMaxSize now %s (%d x %d -> %d x %d)\n",
  ...
  if (FLAG_CHANGED (old, new, PResizeInc))
    cobiwm_topic (COBIWM_DEBUG_GEOMETRY, "XSizeHints: PResizeInc now %s (width_inc %d -> %d height_inc %d -> %d)\n",
  ...
  if (FLAG_CHANGED (old, new, PAspect))
    cobiwm_topic (COBIWM_DEBUG_GEOMETRY, "XSizeHints: PAspect now %s (min %d/%d -> %d/%d max %d/%d -> %d/%d)\n",
  ...
  if (FLAG_CHANGED (old, new, PBaseSize))
    cobiwm_topic (COBIWM_DEBUG_GEOMETRY, "XSizeHints: PBaseSize now %s (%d x %d -> %d x %d)\n",
  ...
  if (FLAG_CHANGED (old, new, PWinGravity))
    cobiwm_topic (COBIWM_DEBUG_GEOMETRY, "XSizeHints: PWinGravity now %s  (%d -> %d)\n",
                FLAG_TOGGLED_ON (old, new, PWinGravity) ? "set" : "unset",
                old->win_gravity, new->win_gravity);
}

void
cobiwm_set_normal_hints (CobiwmWindow *window,
                       XSizeHints *hints)
{
  int x, y, w, h;
  double minr, maxr;
  ...
  if (window->size_hints.flags & PBaseSize)
    {
      cobiwm_topic (COBIWM_DEBUG_GEOMETRY, "Window %s sets base size %d x %d\n",
                  window->desc,
  ...
  if (window->size_hints.flags & PMinSize)
    {
      cobiwm_topic (COBIWM_DEBUG_GEOMETRY, "Window %s sets min size %d x %d\n",
                  window->desc,
  ...
  if (window->size_hints.flags & PMaxSize)
    {
      cobiwm_topic (COBIWM_DEBUG_GEOMETRY, "Window %s sets max size %d x %d\n",
                  window->desc,
  ...
  if (window->size_hints.flags & PWinGravity)
    {
      cobiwm_topic (COBIWM_DEBUG_GEOMETRY, "Window %s sets gravity %d\n",
                  window->desc,
  ...
  if (window->size_hints.width_inc < 1)
    {
      ...
      window->size_hints.width_inc = 1;
      cobiwm_topic (COBIWM_DEBUG_GEOMETRY, "Corrected 0 width_inc to 1\n");
    }
  if (window->size_hints.height_inc < 1)
    {
      /* another cracksmoker */
      window->size_hints.height_inc = 1;
      cobiwm_topic (COBIWM_DEBUG_GEOMETRY, "Corrected 0 height_inc to 1\n");
    }
...
static void
reload_normal_hints (CobiwmWindow    *window,
                     CobiwmPropValue *value,
                     gboolean       initial)
{
  if (value->type != COBIWM_PROP_VALUE_INVALID)
    {
      ...
      cobiwm_topic (COBIWM_DEBUG_GEOMETRY, "Updating WM_NORMAL_HINTS for %s\n", window->desc);

      old_hints = window->size_hints;

      cobiwm_set_normal_hints (window, value->v.size_hints.hints);
...
static void
reload_wm_protocols (CobiwmWindow    *window,
                     CobiwmPropValue *value,
                     gboolean       initial)
{
  int i;
...
static void
reload_wm_hints (CobiwmWindow    *window,
                 CobiwmPropValue *value,
                 gboolean       initial)
{
  CobiwmWindowX11 *window_x11 = COBIWM_WINDOW_X11 (window);
  ...
  if (value->type != COBIWM_PROP_VALUE_INVALID)
    {
      ...
      cobiwm_verbose ("Read WM_HINTS input: %d iconic: %d group leader: 0x%lx pixmap: 0x%lx mask: 0x%lx\n",
  ...
  if (window->xgroup_leader != old_group_leader)
    {
      cobiwm_verbose ("Window %s changed its group leader to 0x%lx\n",
                    window->desc, window->xgroup_leader);

      cobiwm_window_group_leader_changed (window);
    }

  cobiwm_window_set_urgent (window, urgent);

  cobiwm_icon_cache_property_changed (&priv->icon_cache,
                                    window->display,
                                    XA_WM_HINTS);

  cobiwm_window_queue (window, COBIWM_QUEUE_UPDATE_ICON | COBIWM_QUEUE_MOVE_RESIZE);
}

static void
reload_transient_for (CobiwmWindow    *window,
                      CobiwmPropValue *value,
                      gboolean       initial)
{
  CobiwmWindow *parent = NULL;
  ...
  if (value->type != COBIWM_PROP_VALUE_INVALID)
    {
      ...
      parent = cobiwm_display_lookup_x_window (window->display, transient_for);
      if (!parent)
        {
          cobiwm_warning ("Invalid WM_TRANSIENT_FOR window 0x%lx specified for %s.\n",
      ...
      while (parent)
        {
          if (parent == window)
            {
              cobiwm_warning ("WM_TRANSIENT_FOR window 0x%lx for %s would create loop.\n",
                            transient_for, window->desc);
              transient_for = None;
              break;
            }

          parent = cobiwm_display_lookup_x_window (parent->display,
  ...
  if (window->xtransient_for != None)
    cobiwm_verbose ("Window %s transient for 0x%lx\n", window->desc, window->xtransient_for);
  else
    cobiwm_verbose ("Window %s is not transient\n", window->desc);

  if (window->xtransient_for == None || window->xtransient_for == window->screen->xroot)
    cobiwm_window_set_transient_for (window, NULL);
  else
    {
      parent = cobiwm_display_lookup_x_window (window->display,
                                             window->xtransient_for);
      cobiwm_window_set_transient_for (window, parent);
    }
}

static void
reload_gtk_theme_variant (CobiwmWindow    *window,
                          CobiwmPropValue *value,
                          gboolean       initial)
{
  ...
  if (value->type != COBIWM_PROP_VALUE_INVALID)
    {
      ...
      cobiwm_verbose ("Requested \"%s\" theme variant for window %s.\n",
  ...
  if (g_strcmp0 (requested_variant, current_variant) != 0)
    {
      ...
      if (window->frame)
        cobiwm_frame_update_style (window->frame);
    }
}

static void
reload_gtk_hide_titlebar_when_maximized (CobiwmWindow    *window,
                                         CobiwmPropValue *value,
                                         gboolean       initial)
{
  ...
  if (!cobiwm_prefs_get_ignore_request_hide_titlebar () && value->type != COBIWM_PROP_VALUE_INVALID)
    {
      requested_value = ((int) value->v.cardinal == 1);
      cobiwm_verbose ("Request to hide titlebar for window %s.\n", window->desc);
    }
...
static void
reload_bypass_compositor (CobiwmWindow    *window,
                          CobiwmPropValue *value,
                          gboolean       initial)
{
  ...
  if (requested_value == _NET_WM_BYPASS_COMPOSITOR_HINT_ON)
    cobiwm_verbose ("Request to bypass compositor for window %s.\n", window->desc);
  else if (requested_value == _NET_WM_BYPASS_COMPOSITOR_HINT_OFF)
    cobiwm_verbose ("Request to don't bypass compositor for window %s.\n", window->desc);
...
static void
reload_window_opacity (CobiwmWindow    *window,
                       CobiwmPropValue *value,
                       gboolean       initial)

{
  ...
  cobiwm_window_set_opacity (window, opacity);
}

#define RELOAD_STRING(var_name, propname) \
  static void                                       \
  reload_ ## var_name (CobiwmWindow    *window,       \
                       CobiwmPropValue *value,        \
                       gboolean       initial)      \
  {                                                 \
...
/**
 * cobiwm_display_init_window_prop_hooks:
 ...
 * COBIWM_PROP_VALUE_INVALID, in which case nothing will be queried.
 ...
 * was COBIWM_PROP_VALUE_INVALID, the callback still gets called anyway.)
 * This value may be NULL, in which case no callback will be called.
 */
void
cobiwm_display_init_window_prop_hooks (CobiwmDisplay *display)
{
   ...
   */
  CobiwmWindowPropHooks hooks[] = {
    { display->atom_WM_CLIENT_MACHINE, COBIWM_PROP_VALUE_STRING,   reload_wm_client_machine, LOAD_INIT | INCLUDE_OR },
    { display->atom__NET_WM_NAME,      COBIWM_PROP_VALUE_UTF8,     reload_net_wm_name,       LOAD_INIT | INCLUDE_OR },
    { XA_WM_CLASS,                     COBIWM_PROP_VALUE_CLASS_HINT, reload_wm_class,        LOAD_INIT | INCLUDE_OR },
    { display->atom__NET_WM_PID,       COBIWM_PROP_VALUE_CARDINAL, reload_net_wm_pid,        LOAD_INIT | INCLUDE_OR },
    { XA_WM_NAME,                      COBIWM_PROP_VALUE_TEXT_PROPERTY, reload_wm_name,      LOAD_INIT | INCLUDE_OR },
    { display->atom__COBIWM_HINTS,     COBIWM_PROP_VALUE_TEXT_PROPERTY, reload_cobiwm_hints, LOAD_INIT | INCLUDE_OR },
    { display->atom__NET_WM_OPAQUE_REGION, COBIWM_PROP_VALUE_CARDINAL_LIST, reload_opaque_region, LOAD_INIT | INCLUDE_OR },
    { display->atom__NET_WM_DESKTOP,   COBIWM_PROP_VALUE_CARDINAL, reload_net_wm_desktop,    LOAD_INIT | INIT_ONLY },
    { display->atom__NET_STARTUP_ID,   COBIWM_PROP_VALUE_UTF8,     reload_net_startup_id,    LOAD_INIT },
    { display->atom__NET_WM_SYNC_REQUEST_COUNTER, COBIWM_PROP_VALUE_SYNC_COUNTER_LIST, reload_update_counter, LOAD_INIT | INCLUDE_OR },
    { XA_WM_NORMAL_HINTS,              COBIWM_PROP_VALUE_SIZE_HINTS, reload_normal_hints,    LOAD_INIT },
    { display->atom_WM_PROTOCOLS,      COBIWM_PROP_VALUE_ATOM_LIST, reload_wm_protocols,     LOAD_INIT },
    { XA_WM_HINTS,                     COBIWM_PROP_VALUE_WM_HINTS,  reload_wm_hints,         LOAD_INIT },
    { display->atom__NET_WM_USER_TIME, COBIWM_PROP_VALUE_CARDINAL, reload_net_wm_user_time,  LOAD_INIT },
    { display->atom__NET_WM_STATE,     COBIWM_PROP_VALUE_ATOM_LIST, reload_net_wm_state,     LOAD_INIT | INIT_ONLY },
    { display->atom__MOTIF_WM_HINTS,   COBIWM_PROP_VALUE_MOTIF_HINTS, reload_mwm_hints,      LOAD_INIT },
    { XA_WM_TRANSIENT_FOR,             COBIWM_PROP_VALUE_WINDOW,    reload_transient_for,    LOAD_INIT },
    { display->atom__GTK_THEME_VARIANT, COBIWM_PROP_VALUE_UTF8,     reload_gtk_theme_variant, LOAD_INIT },
    { display->atom__GTK_HIDE_TITLEBAR_WHEN_MAXIMIZED, COBIWM_PROP_VALUE_CARDINAL,     reload_gtk_hide_titlebar_when_maximized, LOAD_INIT },
    { display->atom__GTK_APPLICATION_ID,               COBIWM_PROP_VALUE_UTF8,         reload_gtk_application_id,               LOAD_INIT },
    { display->atom__GTK_UNIQUE_BUS_NAME,              COBIWM_PROP_VALUE_UTF8,         reload_gtk_unique_bus_name,              LOAD_INIT },
    { display->atom__GTK_APPLICATION_OBJECT_PATH,      COBIWM_PROP_VALUE_UTF8,         reload_gtk_application_object_path,      LOAD_INIT },
    { display->atom__GTK_WINDOW_OBJECT_PATH,           COBIWM_PROP_VALUE_UTF8,         reload_gtk_window_object_path,           LOAD_INIT },
    { display->atom__GTK_APP_MENU_OBJECT_PATH,         COBIWM_PROP_VALUE_UTF8,         reload_gtk_app_menu_object_path,         LOAD_INIT },
    { display->atom__GTK_MENUBAR_OBJECT_PATH,          COBIWM_PROP_VALUE_UTF8,         reload_gtk_menubar_object_path,          LOAD_INIT },
    { display->atom__GTK_FRAME_EXTENTS,                COBIWM_PROP_VALUE_CARDINAL_LIST,reload_gtk_frame_extents,                LOAD_INIT },
    { display->atom__NET_WM_USER_TIME_WINDOW, COBIWM_PROP_VALUE_WINDOW, reload_net_wm_user_time_window, LOAD_INIT },
    { display->atom__NET_WM_ICON,      COBIWM_PROP_VALUE_INVALID,  reload_net_wm_icon,  NONE },
    { display->atom__KWM_WIN_ICON,     COBIWM_PROP_VALUE_INVALID,  reload_kwm_win_icon, NONE },
    { display->atom__NET_WM_ICON_GEOMETRY, COBIWM_PROP_VALUE_CARDINAL_LIST, reload_icon_geometry, LOAD_INIT },
    { display->atom_WM_CLIENT_LEADER,  COBIWM_PROP_VALUE_INVALID, complain_about_broken_client, NONE },
    { display->atom_SM_CLIENT_ID,      COBIWM_PROP_VALUE_INVALID, complain_about_broken_client, NONE },
    { display->atom_WM_WINDOW_ROLE,    COBIWM_PROP_VALUE_STRING, reload_wm_window_role, LOAD_INIT | FORCE_INIT },
    { display->atom__NET_WM_WINDOW_TYPE, COBIWM_PROP_VALUE_ATOM_LIST, reload_net_wm_window_type, LOAD_INIT | INCLUDE_OR | FORCE_INIT },
    { display->atom__NET_WM_STRUT,         COBIWM_PROP_VALUE_INVALID, reload_struts, NONE },
    { display->atom__NET_WM_STRUT_PARTIAL, COBIWM_PROP_VALUE_INVALID, reload_struts, NONE },
    { display->atom__NET_WM_BYPASS_COMPOSITOR, COBIWM_PROP_VALUE_CARDINAL,  reload_bypass_compositor, LOAD_INIT | INCLUDE_OR },
    { display->atom__NET_WM_WINDOW_OPACITY, COBIWM_PROP_VALUE_CARDINAL, reload_window_opacity, LOAD_INIT | INCLUDE_OR },
    { 0 },
  };

  CobiwmWindowPropHooks *table = g_memdup (hooks, sizeof (hooks)),
    *cursor = table;
  ...
  while (cursor->property)
    {
      ...
      g_assert (!((cursor->flags & LOAD_INIT) && cursor->type == COBIWM_PROP_VALUE_INVALID));
...
void
cobiwm_display_free_window_prop_hooks (CobiwmDisplay *display)
{
